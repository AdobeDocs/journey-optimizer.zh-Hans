...
在[!DNL Adobe Experience Manager]中使用[!DNL Journey Optimizer]中的内容片段时，请记住以下事项：
...
* **内容片段状态**
   ...
   * 在[!DNL Journey Optimizer]中重新发布片段之前，发布后所做的更改不会反映在[!DNL Adobe Experience Manager]中。 这两种产品之间没有自动版本协调。
...
* **缓存和校对**
   * 首次将片段添加到营销活动或历程时，[!DNL Journey Optimizer]将缓存该片段。 如果您选择的片段已通过&#x200B;**[!UICONTROL 打开AEM CF选择器]**&#x200B;在其他位置使用，则会从[!DNL Journey Optimizer]缓存加载该片段。
