...
## 通过MCP的人工智能工具(Beta) {#integration-mcp}

[!DNL Adobe Journey Optimizer]提供模型上下文协议(MCP)服务器，该服务器允许您直接从MCP兼容的AI工具（如Claude Web和Claude Desktop）中使用纯语言提示来查询促销活动并提供数据，而无需编写API调用或导航产品屏幕。

了解有关[Journey Optimizer + MCP](../integrations/ajo-mcp.md)的更多信息。
