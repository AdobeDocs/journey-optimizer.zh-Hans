...
* **验证渠道配置详细信息** — 在历程或营销活动中使用渠道配置之前，检查发件人域、取消订阅设置和IP池。
* **确认治理策略** — 查看哪些营销操作和治理策略已附加到渠道配置。
...
| 工具 | 描述 |
|---|---|
| **列出渠道配置** | 跨所有AJO渠道按名称、状态（草稿、活动、已存档、已停用）或渠道类型筛选渠道配置：电子邮件、移动消息、推送通知、WhatsApp、直邮、应用程序内消息传送、Web、基于代码的体验、内容卡、LINE、实时活动。 |
| **获取渠道配置** | 检索特定渠道配置的完整配置详细信息，包括发件人/回复地址、子域、IP池和取消订阅设置。 |
| **列出配置资源** | 列出渠道配置引用的支持资源，如推送凭据、电子邮件子域、IP池、SMS凭据、WhatsApp凭据、直邮路由、LINE渠道设置和Live Activity注册表。 |
| **获取配置资源** | 按类型和ID检索单个配置资源的完整详细信息。 |
| **列出营销操作** | 列出用于实施数据治理策略的可用营销操作。 |
...
| **渠道配置** | 我的沙盒中有哪些渠道预设可用？ /显示我的所有电子邮件渠道配置。 /我是否设置了任何WhatsApp配置？ /我的营销电子邮件配置配置了哪些发件人地址和回复？ |
...
您可以访问营销活动、历程、优惠、渠道配置、配置资源和沙盒信息。 操作是只读的（检索API）；当前版本不支持写入操作。
+++
