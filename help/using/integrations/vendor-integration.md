...
1. 在Journey Optimizer中，转到配置>管理，然后选择创建集成。
...
授权：持有者&lt;CONTENTABLE_DELIVERY_TOKEN>
...
1. 关注[使用集成](external-sources.md)。 在下载订单路径上配置&#x200B;**GET**，为每个Sitecore设置授权标头，从上下文映射`id`，粘贴示例JSON，映射字段，以及针对资源延迟调整超时。
...
1. 选择配置表中显示的HTTP方法（除非另有说明，否则通常为GET）。
...
1. 关注[使用集成](external-sources.md)。 与批量目录调用、设置持有者身份验证、粘贴示例JSON、映射字段、测试、激活相比，偏好使用单产品检索。
...
1. 关注[使用集成](external-sources.md)。 按Contentstack的要求添加`api_key`和`access_token`标头，包括`environment`查询参数，粘贴示例JSON，映射字段，测试，激活。
...
| 键名称 | 键值 | 添加至 |
| --- | --- | --- |
| `api_key` | `<YOUR_STACK_API_KEY>` | 标头 |
| `access_token` | `<YOUR_DELIVERY_TOKEN>` | 标头 |
...
1. 关注[使用集成](external-sources.md)。 将&#x200B;**GET**&#x200B;与持有者令牌一起使用，在查询标记中仅请求所需的属性选项，粘贴示例JSON，映射最小属性集，测试，激活。
...
示例模式：带有`https://{pim-host}/api/rest/v1/products-uuid/{uuid}`的`Accept: application/json`。 查看[Akeneo API](https://api.akeneo.com/){target="_blank"}。
...
| 类型 | API密钥名称 | API密钥值 | 位置 |
| --- | --- | --- | --- |
| API密钥 | Authorization | `Bearer <YOUR_ACCESS_TOKEN>` | 标头 |
...
1. 关注[使用集成](external-sources.md)。 使用模块公开的公共投放URL模式，根据Magnolia指南进行身份验证（匿名投放与受保护内容的令牌），粘贴示例JSON，映射字段，测试，激活。
...
| 类型 | API密钥名称 | API密钥值 | 位置 |
| --- | --- | --- | --- |
| API密钥 | Authorization | `<bearer_token>` | 标头 |
...
1. 关注[使用集成](external-sources.md)。 为群集设置基本URL，添加必需的标头(`X-APP-ID`， `X-APP-TOKEN`)，使用过滤器或ID约束列表端点，粘贴示例JSON，映射字段，测试，激活。

1. 在Journey Optimizer中，转到配置>管理，然后选择创建集成。

1. 输入不带空格的集成名称。

1. 使用忠诚度/REST API配置端点。 示例URL模式：

1. 按验证您所在地区的OpenAPI基本URL
...
| 类型 | API密钥名称 | API密钥值 | 位置 |
| --- | --- | --- | --- |
| API密钥 | X-APP-TOKEN | `<YOUR-APP-TOKEN>` | 标头 |
...
1. 关注[使用集成](external-sources.md)。 在所需的配置文件或成就路径上使用&#x200B;**GET**，按文档设置`Authorization: ApiKey-v1 <key>`，粘贴示例JSON，映射字段，测试，激活。
...
| 类型 | API密钥名称 | API密钥值 | 位置 |
| --- | --- | --- | --- |
| API密钥 | Authorization | ApiKey-v1 `<YOUR_API_KEY>` | 标头 |
...
1. 关注[使用集成](external-sources.md)。 使用供应商的身份验证（例如查询中的API密钥）配置&#x200B;**GET**，避免公开PII以违反策略，粘贴示例JSON，映射字段，测试，激活。

1. 在Journey Optimizer中，转到配置>管理，然后选择创建集成。

1. 输入不带空格的集成名称。

1. 使用Antavo Enterprise API配置端点。 示例URL模式：

1. 您的租户中记录的每个Antavo栈栈基本URL
...
1. 关注[使用集成](external-sources.md)。 使用您的团队批准的忠诚度集成端点，完成Salesforce OAuth，粘贴示例JSON，映射字段，遵守复合API限制，测试，激活。
...
| 类型 | API密钥名称 | API密钥值 | 位置 |
| --- | --- | --- | --- |
| API密钥 | Authorization | `<access_token>` | 标头 |
...
1. 关注[使用集成](external-sources.md)。 根据需要配置标头，如`CAP-API-ACCESS-TOKEN`，粘贴示例JSON，映射字段，测试，激活。

1. 在Journey Optimizer中，转到配置>管理，然后选择创建集成。

1. 输入不带空格的集成名称。

1. 使用毛细管API配置端点。 示例URL模式：
...
| 类型 | API密钥名称 | API密钥值 | 位置 |
| --- | --- | --- | --- |
| API密钥 | CAP-API-ACCESS-TOKEN | `<YOUR_ACCESS_TOKEN>` | 标头 |
...
1. 使用模板模板API URL配置端点（示例模式）： `https://api.stensul.com/v1/templates/{template_id}`
...
1. 关注[使用集成](external-sources.md)。 指向您所在地区的Marigold主机，设置身份验证（下面的示例使用带有密钥和密码的`X-Api-Key`），粘贴示例JSON，映射字段，测试，激活。

1. 在Journey Optimizer中，转到配置>管理，然后选择创建集成。

1. 输入不带空格的集成名称。

1. 使用Marigold REST API配置端点（根据集成指南配置端点）。 示例URL模式：

1. 使用Marigold API文档中提供的基本URL和路径
...
1. 欧洲： https://{{customername}}.module.slgnt.eu
...
| 类型 | API密钥名称 | API密钥值 | 位置 |
| --- | --- | --- | --- |
| API密钥 | X-Api-Key | `<apiKey>:<apiSecret>` | 标头 |
...
1. 关注[使用集成](external-sources.md)。 传递调用通常为带有JSON正文的&#x200B;**POST**。 按[目标身份验证](https://experienceleague.adobe.com/zh-hans/docs/target-dev/developer/api/configure-authentication){target="_blank"}配置OAuth，粘贴示例响应，映射字段，在预期卷下测试。

1. 在Journey Optimizer中，转到配置>管理，然后选择创建集成。

1. 输入不带空格的集成名称。

1. 使用Target推荐/交付API配置端点（根据Adobe文档了解您的集成模式）。 示例URL模式：

1. 请参阅用例的Adobe Target推荐API文档
...
1. 关注[使用集成](external-sources.md)。 除非您的订阅另有要求，否则请使用&#x200B;**GET**，附加`apiKey`查询参数（或如文档中所述），映射`locationKey`和其他来自配置文件/上下文的变量，粘贴示例JSON，映射字段，然后进行测试。
...
集成字段示例。 [AccuWeather API](https://developer.accuweather.com/apis){target="_blank"}中介绍了详细信息和层。 您通常通过单独的位置搜索调用（例如`locationKey`）解析`.../locations/v1/cities/search?q={{cityName}}`。
...
1. 关注[使用集成](external-sources.md)。 定位您需要的资源（订单与装运），根据[ShipStation API](https://www.shipstation.com/docs/api/){target="_blank"}进行身份验证，粘贴示例JSON，映射字段，测试，激活。
...
1. 关注[使用集成](external-sources.md)。 调用下面建模的REST **GET**，使用密钥标头进行身份验证，粘贴示例JSON，映射字段，测试，激活。
...
示例模式：将RevenueCat的&#x200B;**从** RevenueCat文档[获取产品](https://docs.revenuecat.com/){target="_blank"} （或等效产品/权利GET）与项目的基本URL和版本一起使用。
...
| 类型 | API密钥名称 | API密钥值 | 位置 |
| --- | --- | --- | --- |
| API密钥 | Authorization | `Bearer <token>` | 标头 |
...
1. 关注[使用集成](external-sources.md)。 首选较窄的读取路径；如果您使用&#x200B;**POST**&#x200B;语句执行，请包括API所需的JSON主体，粘贴映射成功响应示例，仔细测试延迟，激活。
...
1. 关注[使用集成](external-sources.md)。 在所选端点上配置&#x200B;**GET**（一种常见模式是用户列表），按[Bynder](https://developer.bynder.com/){target="_blank"}完成OAuth，避免提取不必要的数据页面，映射字段，测试，然后激活。

1. 在Journey Optimizer中，转到配置>管理，然后选择创建集成。

1. 输入不带空格的集成名称。

1. 使用Bynder API v4配置端点（示例：用户列表模式）。 示例URL模式：
...
1. 关注[使用集成](external-sources.md)。 使用所需的查询身份验证配置&#x200B;**GET**，从配置文件或上下文映射标识符，粘贴示例JSON，映射字段，测试，激活。
...
1. 关注[使用集成](external-sources.md)。 使用带有&#x200B;**的** GET`passkey`作为对话API上的查询参数，设置`Accept: application/json`，粘贴示例JSON，映射字段，测试，激活。
...
1. 关注[使用集成](external-sources.md)。 使用发布的架构或首选项中心路径访问您的订阅文档，根据需要完成OAuth，粘贴示例JSON，映射字段，测试，激活。

1. 在Journey Optimizer中，转到配置>管理，然后选择创建集成。

1. 输入不带空格的集成名称。

1. 使用OneTrust API配置端点。 示例URL模式：

1. 每个OneTrust开发人员门户基本URL
...
1. 关注[使用集成](external-sources.md)。 图形调用通常是带有版本化路径的&#x200B;**GET**；处理令牌过期、粘贴示例JSON、映射字段、测试、激活。
...
1. 关注[使用集成](external-sources.md)。 在所需的记录路径上使用&#x200B;**GET**，发送所需的标头（如`API-VERSION`），粘贴示例JSON（返回的HAL或JSON），映射最小字段集，测试，激活。

1. 在Journey Optimizer中，转到配置>管理，然后选择创建集成。

1. 输入不带空格的集成名称。

1. 使用Aprimo DAM /记录API配置端点。 示例URL模式：

1. 您的租户的每个Aprimo API基本URL和资源路径
...
| 类型 | API密钥名称 | API密钥值 | 位置 |
| --- | --- | --- | --- |
| API密钥 | Authorization | 持有人`<token>` | 标头 |
...
1. 关注[使用集成](external-sources.md)。 不要猜测公共URL。 使用Epsilon中的规范，粘贴示例JSON，映射字段，测试，激活。

1. 在Journey Optimizer中，转到配置>管理，然后选择创建集成。

1. 输入不带空格的集成名称。

1. 使用Epsilon API配置端点（根据集成规范）。 示例URL模式：
...
示例模式：带有`https://{your-instance}.epsilon3.io/api/v1/planning/events`和`start`查询参数以及基于标头的API密钥的`end`。 在生产之前使用Epsilon确认。
...
| 类型 | API密钥名称 | API密钥值 | 位置 |
| --- | --- | --- | --- |
| API密钥 | `<your_username>` | `<EPSILON3_API_KEY>` | 标头 |
