...
* 您可以在此处下载空白Excel [IP预热计划模板](assets/IPWarmupPlan-Template.zip) 以填写。

* 您也可以下载 [示例IP预热计划](assets/IPWarmupPlan-Sample.zip) 已填入了一些可用作示例的数据。
...
1. 上载包含IP预热计划的Excel文件。 [了解详情](#prepare-file)
   ...
   >[!NOTE]
   >
   >如果上传失败，请确保您使用的是正确的格式和文件格式(.xls或.xlsx)。 使用 [模板](assets/IPWarmupPlan-Template.zip) 由Adobe提供。
