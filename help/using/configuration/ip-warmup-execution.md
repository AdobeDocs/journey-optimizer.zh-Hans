...
1. 为当前阶段选择营销策划后，将显示排除用户档案、营销策划受众和域组的部分。
   ...
   1. 从&#x200B;**[!UICONTROL 排除的域组]**&#x200B;部分，选择要从该阶段排除的域。
      ...
      >[!CAUTION]
      >
      >在IP预热计划运行后，如果更新用于IP预热营销活动的电子邮件渠道[配置](../email/email-settings.md#execution-address)中的[执行地址](channel-surfaces.md)，域排除可能会失败。 在IP预热计划启动后，请勿编辑电子邮件渠道配置。
...
>title="查看合格的轮廓"
>abstract="此列显示合格的轮廓数。为某次运行评估受众后，如果作为目标的轮廓比合格的轮廓多，则除非启用了&#x200B;**因错误取消已激活的运行**&#x200B;选项，否则仍执行该运行。在这种情况下，将取消运行。"
...
>[!CAUTION]
>
>每次运行必须在实际发送时间之前至少12小时以及在每日批量分段作业之前激活。 否则，可能无法完成受众评估。
...
您还可以从[促销活动菜单](../campaigns/manage-campaigns.md#access)访问报告，因为您的计划可能使用不同的促销活动。 [在日历→](../campaigns/manage-campaigns.md#calendar)中查看
