...
>title="子域委派"
>abstract="Journey Optimizer允许您将子域委派给Adobe。 您可以将子域完全委派给Adobe，这是推荐的方法。 您还可以使用CNAME创建子域以指向特定于Adobe的记录，但此方法要求您自行维护和管理DNS记录。"
...
>title="子域委派"
>abstract="要开始发送电子邮件，您需要将子域委派给Adobe。 完成后，将为您配置DNS记录、收件箱、发件人、回复地址和退回地址。"
...
>title="生成匹配的DNS记录"
>abstract="要将新子域完全委派给Adobe，您需要将Journey Optimizer界面中显示的Adobe名称服务器信息复制粘贴到域托管解决方案中，以生成匹配的DNS记录。 要使用CNAME委派子域，您还需要复制并粘贴SSL CDN URL验证记录。 检查成功后，子域即可用于投放消息。"
...
1. 指定要委派的子域的名称。
   ...
   >
   >请注意，当前不支持email.marketing.yourcompany.com等多级别子域。
