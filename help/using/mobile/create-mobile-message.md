...
>title="移动消息操作"
>abstract="当轮廓到达历程中的此步骤时，移动消息通信渠道操作会向其发送短信（SMS）、多媒体消息（MMS）或富通信消息（RCS）。 标签用于在历程画布中标识该活动，而该操作会引用定义投放内容的移动消息配置。 **优化**&#x200B;部分可包含内容试验或目标定位规则；**多语言**&#x200B;部分可使用多种语言投放内容；**超时或错误**&#x200B;部分则可在操作失败时定义备用路径。"
