...
当用户正在使用您的应用程序，并且注意力十分集中时，应用程序内消息最适合用于引导或影响用户。
...
| 不依赖于外部渠道 | 完全在应用程序中工作，与其他渠道的选择启用状态无关 | 联系选择退出推送通知的用户 |
| 更高的转化潜力 | 在积极关注的时刻提供，提高响应率 | 追加销售或交叉销售优惠，调查提示 |
| 自定义和分段 | 布局、文本和按钮可以根据特定受众进行定制 | 不同用户区段的个性化入门流程 |
| 非侵入式设计 | 可以设计为补充而不是中断用户体验 | 与应用程序 UI 一致的横幅或模态框 |
...
* 消息涉及紧急或时间敏感问题，需要联系应用程序外的用户，例如中断或安全警报
* 通信涉及监管或法律要求，需要应用程序内消息无法提供的读取确认
* 目标是为不太可能打开应用程序的非活动用户重新激活帐户或开展挽回活动
