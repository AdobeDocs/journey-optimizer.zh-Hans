...
**在此页面上：**&#x200B;查看您在访问、创作、交付和报告Adobe Journey Optimizer中的Web体验时必须满足的实施、可视化创作、交付和报告先决条件和护栏。
...
>title="生成页面匹配规则"
>abstract="要访问 [!DNL Journey Optimizer] 网页设计器，必须安装特定的浏览器扩展：Adobe Experience Cloud 可视化编辑助手 - 仅在 Google Chrome 或 Microsoft Edge 上可用。"
...
>* 添加必填字段组仅是附加的 — 它适用于运行营销活动或历程的页面，并保持所有其他跟踪不变。
