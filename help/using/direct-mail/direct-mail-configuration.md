...
1. [引用文件路由配置的直邮配置](#direct-mail-configuration)。 如果尚未配置任何文件路由选项，您将无法创建直邮配置。
...
* **[!UICONTROL 身份验证类型]**：选择用于连接到服务器的身份验证类型（密码或SSH密钥）。

* **[!UICONTROL 帐户]**：用于连接到SFTP服务器的帐户名称。

* **[!UICONTROL 服务器地址]**： &#x200B;SFTP服务器的URL。

* **[!UICONTROL 端口]**： SFTP连接端口号。
