...
* **在等待活动（或读取受众历程）之前：** Journey Optimizer从用户档案的批量投影中读取。 此投影中的数据在摄取后&#x200B;**2小时**&#x200B;内刷新。 依赖于基于天或基于时间的条件的受众可能会遇到额外的延迟。 在历程开始时添加短的[等待活动](../wait-activity.md)，或允许缓冲时间以确保反映最新的区段成员资格。
* **等待活动（或单一事件历程中）后：**&#x200B;从流式传输（单一）投影中读取区段成员资格。 有关预期延迟，请参阅[Adobe Experience Platform流式摄取文档](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/ingestion/streaming/overview){target="_blank"}。 此路径通常对最近的配置文件更改响应更灵敏。
