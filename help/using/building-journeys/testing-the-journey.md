...
使用业务事件（请参阅[此部分](../event/about-events.md)）时，测试模式允许您触发历程中的单个测试用户档案入口，模拟该事件并传递正确的用户档案ID。 您必须传递将在测试中进入旅程的测试用户档案的事件参数和标识符。 您不能使用存在于其他基于区段的历程的&#x200B;**[!UICONTROL Up to 100 profiles at once]**&#x200B;选项。 在测试模式下，没有“代码视图”模式可用于基于业务事件的历程。
