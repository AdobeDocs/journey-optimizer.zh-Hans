...
>id="ajo_journey_test"
>title="测试您的历程"
>abstract="测试轮廓可让您在发布历程之前进行测试。 这使您可以分析个人如何在历程中流动，并在发布前进行故障排除。"
...
* **自动停用** — 在测试模式下持续停用&#x200B;**一周**&#x200B;的历程会自动恢复为草稿状态，以优化性能并防止过时资源的使用。
...
当您想要使用已在Adobe Experience Platform中创建的预先存在的测试配置文件测试您的历程时，请使用&#x200B;**[!UICONTROL 测试模式]**&#x200B;方法。

1. 要激活测试模式，请单击&#x200B;**[!UICONTROL 模拟]**&#x200B;按钮，然后选择&#x200B;**[!UICONTROL 测试模式]**。
...
>title="配置测试模式"
>abstract="如果历程包含多个事件，则可使用下拉列表选择事件。 对于每个事件，都需要配置传递的字段以及事件发送的执行方式。"
