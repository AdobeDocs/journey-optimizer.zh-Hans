...
* 通过上传创建测试用户档案 [csv文件](#create-test-profiles-csv) 或使用 [API调用](#create-test-profiles-api).

   除了这两种方法之外，Adobe Journey Optimizer还随附了特定的 [产品内用例](#use-case-1) 以便于创建测试用户档案。

您还可以在现有数据集中上传json文件。 有关详情，请参阅 [数据引入文档](https://experienceleague.adobe.com/docs/experience-platform/ingestion/tutorials/ingest-batch-data.html#add-data-to-dataset){target="_blank"}.
...
完成后，单击 **[!UICONTROL 添加字段组]**：字段组的列表显示在架构概述屏幕上。
   ...
   >* 单击架构名称可更改并更新其属性。
   >
   >* 单击 **[!UICONTROL 添加]** 字段组部分中的按钮，以选择要添加到架构中的其他字段组


1. 在字段列表中，单击要定义为主要标识的字段。
   ![](assets/test-profiles-3.png)
1. 在 **[!UICONTROL 字段属性]** 右窗格，检查 **[!UICONTROL 身份]** 和 **[!UICONTROL 主要身份]** 选项并选择命名空间。 如果您希望主标识是电子邮件地址，请选择 **[!UICONTROL 电子邮件]** 命名空间。 单击&#x200B;**[!UICONTROL 应用]**。
...
从Adobe Journey Optimizer主页中，您可以利用测试配置文件在产品中的用例。 此用例有助于创建测试用户档案，用于在发布之前测试历程。
...
1. **身份命名空间**：此 [身份命名空间](../segment/get-started-identity.md) 用于唯一标识测试用户档案。 例如，如果电子邮件用于标识测试用户档案，则标识命名空间 **电子邮件** 应选择。 如果唯一标识符是电话号码，则身份命名空间 **电话** 应选择。

2. **CSV文件**：包含要创建的测试配置文件列表的逗号分隔文件。 用例需要CSV文件的预定义格式，该文件包含要创建的测试配置文件列表。 文件中的每一行应按正确的顺序包含以下字段，如下所示：

   1. **人员Id**：测试配置文件的唯一标识符。 此字段的值应当反映所选的身份命名空间。 (例如，如果 **电话** 选择作为身份命名空间，此字段的值应为电话号码。 同样地，如果 **电子邮件** （则此字段的值应为电子邮件）
   1. **电子邮件地址**：测试用户档案电子邮件地址。 (此 **人员Id** 字段和 **电子邮件地址** 字段可能包含相同的值，如果 **电子邮件** 被选为身份命名空间)
   1. **名字**：测试配置文件名字。
   1. **姓氏**：测试配置文件姓氏。
   1. **城市**：测试用户档案居住城市
   1. **国家/地区**：测试用户档案居住国家/地区
   1. **性别**：测试用户档案性别。 可用的值包括 **男性**， **女** 和 **非指定**

选择身份命名空间并根据上述格式提供CSV文件后，单击 **[!UICONTROL 运行]** 按钮。 用例可能需要几分钟才能完成。 一旦用例完成处理和创建测试用户档案，将发送通知以通知用户。

>[!NOTE]
>
>测试配置文件可能会覆盖现有配置文件。 在执行用例之前，请确保CSV仅包含测试用户档案，并且针对正确的沙盒执行。
...
您的历程将由 **[!UICONTROL 读取区段]** 和 **[!UICONTROL 更新配置文件]** 活动。 您首先需要创建一个以要转换为测试用户档案的用户档案为目标的区段。

>[!NOTE]
>
> 因为您将更新 **testProfile** 字段，所选配置文件必须包含此字段。 相关架构必须具有 **配置文件测试详细信息** 字段组。 请参阅[此小节](../segment/creating-test-profiles.md#test-profiles-prerequisites)。

1. 浏览到 **区段**，则 **创建区段**，位于右上角。
   ![](assets/test-profiles-22.png)
1. 定义区段的名称并构建区段：选择字段和值以定向所需的配置文件。
   ![](assets/test-profiles-23.png)
1. 单击 **保存** 和检查用户档案是否被区段正确定向。
   ![](assets/test-profiles-24.png)

   >[!NOTE]
   >
   > 区段计算可能需要一些时间。 了解中有关区段的更多信息 [本节](../segment/about-segments.md).

1. 现在，创建一个新历程并从开始 **[!UICONTROL 读取区段]** 编排活动。
1. 选择之前创建的区段和您的配置文件使用的命名空间。
   ![](assets/test-profiles-25.png)
1. 添加 **[!UICONTROL 更新配置文件]** 操作活动。
1. 选择架构，然后 **testProfile** 字段、数据集并将值设置为 **True**. 要执行此操作，请在 **[!UICONTROL 值]** 字段中，单击 **笔** 图标，选择 **[!UICONTROL 高级模式]** 并输入 **true**.
   ![](assets/test-profiles-26.png)
1. 单击&#x200B;**[!UICONTROL 发布]**。
1. 在 **[!UICONTROL 区段]** 部分，检查配置文件是否已正确更新。
...
1. 单击 **选择文件** 并选择您的csv文件。 上传文件后，单击 **下一个**.
   ![](assets/test-profiles-18.png)
1. 将源csv字段映射到架构字段，然后单击 **完成**.
   ![](assets/test-profiles-19.png)
1. 数据导入开始。 状态将从 **正在处理** 到 **成功**. 单击 **预览数据集**，位于右上角。
