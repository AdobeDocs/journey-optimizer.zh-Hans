...
| 错误代码 | 描述 | 根本原因 | 解决方法 |
|------------|-------------|-----------|-----------|
| **CJMPTS-1410-500** | 推送/渠道发送操作出现内部服务器错误 | 渠道后端中断、凭据过期、配置错误或提供程序错误 | &#x200B;1. 在延迟<br/>2后重试。 检查提供程序配置和配额<br/>3。 验证推送凭据是否有效<br/>4。 使用备用通道<br/>5进行测试。 如果是永久性的，请使用请求ID <br/><br/>**相关文档**&#x200B;联系Adobe支持： [推送配置](../push/push-configuration.md) |
| **CJMPTS-1006-404** | 推送/短信失败，显示“未找到资源” | 引用的提供程序/渠道不存在、拼写错误或已取消设置 | &#x200B;1. 查看并更正提供程序/渠道引用和ID<br/>2。 审核沙盒/组织配置<br/>3。 验证通道配置是否处于活动状态<br/>4。 如有必要，重新创建渠道配置&#x200B;<br/><br/>**相关文档**： [渠道平面](../configuration/channel-surfaces.md) |
| **CJMPTS-1510-500** | 推送渠道发送时出现内部服务器错误 | 后端推送/传输故障；提供商或基础设施错误 | &#x200B;1. 检查渠道设置设置<br/>2。 验证推送凭据是否有效<br/>3。 请重试操作<br/>4。 如果是永久性的，请使用请求ID <br/><br/>**相关文档**&#x200B;联系Adobe支持： [推送配置](../push/push-configuration.md) |
...
| 错误代码 | 描述 | 根本原因 | 解决方法 |
|------------|-------------|-----------|-----------|
| **CJMRT-110001-500** | 超出工作流步骤的最大运行次数（例如，IP关联配置步骤超时） | 工作流/配置作业未在允许的重试/时间内完成，通常是因为基础架构/服务滞后或临时后端问题 | &#x200B;1. 请稍后重试<br/>2。 检查[Adobe Status](https://status.adobe.com/zh-cn/)是否有中断<br/>3。 通过工作流/作业/组织详细信息<br/>4上报到Adobe支持。 提供日志和网络捕获（如果可用）<br/><br/>**相关文档**： [历程疑难解答](troubleshooting.md) |
...
| **CJMRT-000032-409** | 冲突 — 资源已存在 | 尝试创建具有重复ID或名称的资源 | &#x200B;1. 对所有资源<br/>2使用唯一ID和名称。 检查具有相同标识符<br/>3的现有资源。 删除或重命名冲突的对象<br/>4。 查看命名惯例&#x200B;<br/><br/>**相关文档**： [历程创建](journey-gs.md) |
...
| **CJMRT-110100-500** | 有效负载上的内部错误 | 后端数据/配置错误或不支持的配置 | &#x200B;1. 请重试操作<br/>2。 如果使用高级功能，请简化配置<br/>3。 使用请求ID和确切有效负载<br/>4上报给Adobe支持。 查看发行说明&#x200B;<br/><br/>**相关文档**&#x200B;中的已知问题： [历程疑难解答](troubleshooting.md) |
...
| 错误代码 | 描述 | 根本原因 | 解决方法 |
|------------|-------------|-----------|-----------|
| **CJMMAS-1732-500** | 验证失败 — 使用AEM资源发送验证/测试时，所有资源未发布 | 最近发布的资产尚未在AJO中发布；资产ID不匹配；跨存储库使用；AEM同步延迟 | &#x200B;1. 仅使用来自正确存储库/环境<br/>2的已发布资产ID。 留出时间在AEM和AJO<br/>3之间同步。 请使用已知良好的资产<br/>4重试。 在AEM <br/><br/>**相关文档**&#x200B;中验证资产发布状态： [Assets集成](../integrations/assets.md) |
| **CJMMAS-1069-500** | 保存或发布消息模板时发生内部错误 | 后端异常（基础架构/服务错误或内容问题）；不支持的标记/功能 | &#x200B;1. 简化或降低模板复杂度<br/>2。 在增量步骤中重新添加内容以标识问题<br/>3。 检查[Adobe Status页面](https://status.adobe.com/zh-cn/)<br/>4。 删除不支持的功能或标记&#x200B;<br/><br/>**相关文档**： [内容模板](../content-management/content-templates.md) |
| **CJMMAS-1149-400** | 保存消息、预设或变体时请求无效 | 消息中缺少必填字段或配置错误 | &#x200B;1. 填写所有必填字段（标有星号）<br/>2。 验证消息/预设配置<br/>3。 检查字段值格式和约束<br/>4。 查看UI <br/><br/>**相关文档**&#x200B;中的验证消息： [电子邮件渠道](../email/get-started-email.md)，[渠道界面](../configuration/channel-surfaces.md) |
| **CJMMAS-2073-422** | 消息预设编辑中无法处理的实体 | 验证错误、不支持的字段或不正确的语法 | &#x200B;1. 请更正语法/字段错误，如<br/>2所示。 与已知良好的配置<br/>3比较。 在保存<br/>4之前使用消息UI验证。 查看文档&#x200B;<br/><br/>**相关文档**&#x200B;中的字段要求： [消息预设](../configuration/channel-surfaces.md)，[电子邮件设置](../email/email-settings.md) |
...
| 错误代码 | 描述 | 根本原因 | 解决方法 |
|------------|-------------|-----------|-----------|
| **CJMCMP-6003-400** | 发布/测试模式历程/消息时，“至少有一个不正确的营销活动” | 节点引用缺失、未发布或无效的营销活动；旧版或克隆的历程未创建内联 | &#x200B;1. 打开每个消息节点并验证配置<br/>2。 重新链接或重新添加消息节点<br/>3。 激活测试模式以强制创建内联营销活动<br/>4。 如果问题频繁，请移至新历程向导&#x200B;<br/><br/>**相关文档**： [历程创建](journey-gs.md)，[测试历程](testing-the-journey.md) |
| **CJMCMP-2003-400** | UI横幅： Email Designer中的“试验不正确” | 试验/数据提供程序失效或缺失；试验清理失败、架构不匹配或UI验证错误 | &#x200B;1. 删除未使用的试验字段<br/>2。 验证架构和数据提供程序连接<br/>3。 重新加载UI并清除浏览器缓存<br/>4。 如果问题未解决，则重新创建节点/电子邮件&#x200B;<br/><br/>**相关文档**： [内容实验](../content-management/content-experiment.md) |
| **CJMCMP-3001-400** | 模拟/预览“不正确的表面类型过滤器” | 使用旧结构构建的节点发送type=surfaceId，后端应接收brandingPresetId | &#x200B;1. 删除并重新创建受影响的节点<br/>2。 使用新历程版本/模板<br/>3。 使用测试模式清除配置<br/>4。 如果问题普遍存在，则批量重新创建节点&#x200B;<br/><br/>**相关文档**： [渠道平面](../configuration/channel-surfaces.md)，[消息模拟](../content-management/preview.md) |
...
2. **查找服务**：使用服务前缀(CJMPTS、CJMRT、CJMMAS、CJMCMP、CJMTL、CJMRPS)来标识受影响的组件。
...
7. **需要时上报**：如果在执行解决步骤后错误仍然存在，[请联系Adobe支持](../start/user-interface.md#support-ticket-guidelines)，并提供完整的错误代码、请求ID（如果有）、要再现的步骤和相关配置详细信息。
...
2. **检查系统状态**：访问[Adobe Status](https://status.adobe.com/zh-cn/){target="_blank"}以了解已知的服务问题
