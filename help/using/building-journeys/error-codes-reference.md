...
| 错误代码 | 描述 | 根本原因 | 解决方法 |
|------------|-------------|-----------|-----------|
| **CJMPTS-1410-500** | 推送/渠道发送操作出现内部服务器错误 | 渠道后端中断、凭据过期、配置错误或提供程序错误 | 1.在延迟后重试<br/>2。 检查提供程序配置和配额<br/>3。 验证推送凭据是否有效<br/>4。 使用备用通道<br/>5进行测试。 如果是永久性的，请使用请求ID <br/><br/>**相关文档**&#x200B;联系Adobe支持： [推送配置](../push/push-configuration.md) |
