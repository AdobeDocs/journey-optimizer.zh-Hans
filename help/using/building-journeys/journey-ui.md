...
title: 浏览和筛选您的历程
description: 在 [!DNL Adobe Journey Optimizer]中浏览并筛选您的历程
...
**在此页面上：**&#x200B;了解如何使用Adobe Journey Optimizer中的历程仪表板、列表和日历视图浏览、搜索和筛选您的历程。
...
>id="ajo_journey_view"
>title="历程列表和日程表视图"
>abstract="除了历程列表之外，[!DNL Journey Optimizer] 还提供历程的日程表视图，清晰展示历程的日程安排。 这些按钮可随时在列表视图和日历视图之间切换。"
...
>[!NOTE]
>
>* 历程创作生命周期还包括一组不可用于筛选的中间状态： **发布**（在“草稿”和“实时”之间）、**激活测试模式**&#x200B;或&#x200B;**停用测试模式**(在&#x200B;**草稿**&#x200B;和&#x200B;**草稿（测试）**&#x200B;之间)、**停止**（在&#x200B;**实时**&#x200B;和&#x200B;**停止**&#x200B;之间）、**继续**（在&#x200B;**之间）已暂停**&#x200B;和&#x200B;**Live**)，**正在暂停** （介于&#x200B;**Live**&#x200B;和&#x200B;**Paused**&#x200B;之间）当历程处于中间状态时，它是只读的。
>
>* 如果需要修改为&#x200B;**实时**&#x200B;历程，请[创建历程的新版本](#journey-versions)。 您还可以暂停实时历程，执行所有需要的更改，然后随时重新恢复它们。 [了解有关暂停历程的更多信息](../building-journeys/journey-pause.md)
...
从历程列表中，您可以使用复选框一次性选择多个项目，并通过屏幕底部显示的批量操作栏将操作应用于所有项目。

![选择了多个历程的批量操作栏](assets/bulk-actions-journeys.png)

可用的操作如下：

* **[!UICONTROL 添加到包]** — 将选定的历程导出到另一个沙盒。 [了解如何导出对象 →](../configuration/copy-objects-to-sandbox.md)
* **[!UICONTROL 移动到文件夹]** — 将所选历程移动到文件夹中。 [了解如何使用文件夹→](#journeys-folders)
* **[!UICONTROL 编辑标记]** — 编辑分配给所选历程的标记。 [了解如何使用标签 →](../start/search-filter-categorize.md#add-tags)
* **[!UICONTROL 管理访问权限]** — 将访问标签应用于所选历程。 [详细了解对象级访问控制 →](../administration/object-based-access.md)
* **[!UICONTROL 删除]** — 永久删除选定的历程。
* **[!UICONTROL 暂停]** — 暂停选定的&#x200B;**实时**&#x200B;历程（_批量暂停_）。 仅在选择&#x200B;**实时**&#x200B;历程时可用。
* **[!UICONTROL 恢复]** — 恢复选定的&#x200B;**已暂停**&#x200B;历程（_批量恢复_）。 仅在选择&#x200B;**已暂停**&#x200B;历程时可用。

![从底部栏批量暂停两个实时历程](assets/bulk-pause-journeys.png)

[了解有关暂停/恢复历程的更多信息](journey-pause.md)。 [了解有关其他批量操作的更多信息→](../start/search-filter-categorize.md#bulk-actions)
