...
用户档案可以具有窗口外的最佳打开时间。 例如，John在下午5点时打开了最佳点击。

在用户档案级别，每周每小时都有分数。 在本例中，电子邮件将始终在窗口内发送。 在运行时，系统会检查该窗口内的得分列表（从上午10点开始的3小时窗口）。 然后，系统比较10分、11分和中午的得分，并决定三者中哪一分最高。 该时发送电子邮件。
