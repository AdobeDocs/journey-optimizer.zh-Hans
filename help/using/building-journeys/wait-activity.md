...
* 在等待活动中使用发送时间优化以确定在继续任何下游活动之前的最佳时间
...
* **发送时间优化等待**：一种等待类型，使用Adobe的发送时间优化人工智能模型选择最佳时间以继续下一个活动，与任何消息发送&#x200B;*（产品特定）*&#x200B;脱钩
...
* 等待活动中的发送时间优化不会显示免打扰时间规则；如果下游渠道操作受设置为丢弃消息的免打扰时间规则保护，则可以从消息投放中删除用户档案并将其从历程退出。
...
* **问：发送时间优化等待活动是否知道安静时间？**  — 不。 安静时间仅在消息操作时评估，因此“等待”活动可以在安静时间窗口内选取时间。 根据免打扰时间规则，消息将排队直到免打扰时间结束或被丢弃，这也会从历程中退出用户档案。
