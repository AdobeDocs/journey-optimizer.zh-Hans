...
+++ 是要在每个批次之前重新评估受众，还是修复在历程开始时？

在触发历程时，受众被评估一次&#x200B;**&#x200B;**。 此时，将生成符合条件的用户档案的快照，并用于所有波次 — 在后续每次波次之前，不会重新评估受众成员资格。

但是，在每个波次处理&#x200B;**时读取**&#x200B;配置文件属性，而不是在历程开始时读取。 这意味着对于跨越多天的波段：

* Personalization属性（例如，用户档案的名字或忠诚度级别）反映用户档案在批次运行时的状态。
* **在发送时为每个批次应用同意和隐藏检查。** 如果个人资料在两个批次之间选择退出，他们将不会接收来自后续批次的消息。

总而言之，历程中包含的&#x200B;*谁*&#x200B;是预先确定的，但&#x200B;*用于发送到这些用户档案的数据*&#x200B;在处理其批次时反映其当前状态。
