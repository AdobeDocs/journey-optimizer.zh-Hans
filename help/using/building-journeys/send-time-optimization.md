...
>[!NOTE]
>
>发送时间优化适用于历程中的内置电子邮件和推送操作，以及适用于[等待活动](wait-activity.md#sto-wait)，它确定继续下一步活动的最佳时间。 它当前不适用于通过营销活动发送的消息或其他操作类型。
...
>
>* 发送时间优化仅适用于&#x200B;**电子邮件**&#x200B;和&#x200B;**推送通知**&#x200B;渠道，以及&#x200B;**[!UICONTROL 等待]**&#x200B;活动。
>
...
## 等待活动中的发送时间优化 {#sto-wait-activity}

等待活动中的![发送时间优化切换](../building-journeys/assets/wait_sto.png)

除了电子邮件和推送操作之外，您还可以将发送时间优化应用于&#x200B;**[!UICONTROL 等待]**&#x200B;活动。 它依赖于同一发送时间优化模型来计算每个用户档案的最佳等待时间，但在这里，等待与发送是相互分离的：除了仅与电子邮件或推送操作绑定，它可以后跟任何活动，例如自定义操作。

[了解如何在等待活动中配置发送时间优化](wait-activity.md#sto-wait)。
...
* **TL；DR：**&#x200B;本页介绍如何在Adobe Journey Optimizer中配置和使用发送时间优化，该功能由AI提供支持，可预测向每个人发送电子邮件或推送消息的最佳时间，从而最大限度地提高参与度。

**意图：**

* 对历程中的电子邮件或推送操作启用发送时间优化
* 选择是优化电子邮件打开次数还是点进次数
* 设置延迟投放的最大等待时段（在下一次发送）
* 了解AI模型如何使用行为数据预测最佳发送时间
* 确定发送时间优化是否适用于给定的消息类型
* 在等待活动中使用发送时间优化可在任何下游活动之前延迟，这与消息发送分离

**术语表：**

* **发送时间优化(STO)**：AI支持的功能，它将消息发送到每个用户档案的时间延迟到配置的时间窗口&#x200B;*（产品特定）*&#x200B;内的预测最佳参与小时数
...
* 发送时间优化适用于历程中的电子邮件和推送通知渠道以及等待活动；它不适用于促销活动或自定义操作。
* “发送时间优化”不能查看无提示小时规则；“发送时间优化等待”活动可以在无提示小时窗口内为下游渠道操作选择一个时间，然后该操作可能会根据无提示小时规则配置将消息排队或丢弃。
...
* 规范名称：发送时间优化 — 缩写：STO — 变体：最佳发送时间、发送时间AI、智能发送时间
* 同义词：“发送时间优化”=“最佳发送时间”=“AI发送时间”
* 请勿混淆：“探索发送时间”≠“优化的发送时间”（探索是随机进行的模型测试；优化是模型预测的参与）

**常见问题解答：**

* **问：哪些渠道支持发送时间优化？** —历程中的电子邮件和推送通知渠道，以及等待活动；不支持营销活动和自定义操作。
* **问：发送时间优化是否知道安静时间？**  — 不。 仅当用户档案到达消息操作时才评估免打扰时间，因此发送时间优化等待活动可以在免打扰时间窗口内选取时间。 根据免打扰时间规则，消息将排队直到免打扰时间结束，或者被放弃并且用户档案退出历程。 [了解详情](wait-activity.md#sto-wait)。
* **问：我是否应该优化电子邮件的打开次数或点击次数？**  — 优化大多数电子邮件的点击次数。 当消息仅供参考而不用于驱动特定操作时，选择打开。
* **问：启用STO之前，组织需要等待多长时间？**  — 在Journey Optimizer中至少需要30天的电子邮件或推送使用时间，才能收集足够的行为数据。 在长达16周的时间内，结果持续改善。
* **问：STO可以在晚上发送推送通知吗？**  — 是，如果用户的行为表明夜间参与，或者选择了勘探发送时间。 要避免这种情况，请使用最长等待时间较短的早上发送时间。
* **问：发送时间优化的预期好处是什么？**  — 所有经过优化的消息的电子邮件点击率或推送打开率提高了约2-10%，不过单个小流量发送的消息可能无法带来明显的优势。
