...
* **TL；DR：**&#x200B;本页介绍了在Adobe Journey Optimizer中创建第一个历程的四个关键步骤（定义入口点、设计画布、使用测试模式或试运行进行测试和发布），以及选择正确入口类型的指南。

**意图：**
* 在历程管理菜单中创建新旅程并配置其属性
* 为给定用例选择正确的入口点（读取受众、受众资格、单一事件或业务事件）
* 通过将事件、编排活动和渠道操作拖放到画布上，设计多步历程
* 使用模拟、具有持久AEP测试用户档案的测试模式或在发布之前试运行，测试历程
* 执行模拟运行，在不联系客户的情况下使用实际生产数据验证受众定位
* 发布历程以使其上线并使用报告工具监控其性能

**术语表：**
* **读取受众**：一次或按计划&#x200B;*（产品特定）*&#x200B;处理批次受众中所有配置文件的进入活动
* **受众资格**：当配置文件进入或退出流受众&#x200B;*（产品特定）*&#x200B;时，实时触发进入活动
* **单一事件**：在特定操作发生&#x200B;*（产品特定）*&#x200B;时一次输入一个用户档案进入历程的实时触发器
* **业务事件**：通过自动读取受众步骤&#x200B;*（产品特定）*&#x200B;同时触发多个配置文件的历程的非配置文件事件（例如，航班取消、库存补充）
* **测试模式**：一种验证模式，使用永久性Adobe Experience Platform测试配置文件（明确标记为测试配置文件）在发布&#x200B;*（产品特定）*&#x200B;之前遍历草稿历程
* **模拟**：使用动态生成的临时模拟用户的验证模式；模拟用户不会在Adobe Experience Platform *（产品特定）*&#x200B;中持续存在
* **试运行**：一种特殊的发布模式，它使用实际生产数据来验证历程逻辑，而不联系实际客户或更新配置文件&#x200B;*（产品特定）*

**护栏：**
* 如果历程包含错误，则无法发布该历程；必须先解决所有错误
* 必须先由数据工程师完成事件配置（用于基于事件的条目），然后才能构建历程
* 历程护栏和限制单独记录，应在大规模设计之前查看
* 在Adobe Experience Platform中创建受众是基于受众的历程的先决条件

**术语：**
* 规范名称：历程 — 缩写：无 — 变体：客户历程、编排流程
* 同义词：“测试模式”=“历程测试”；“练习”=“练习模式”
* 请勿混淆：“模拟”≠“测试模式”≠“演习” — 模拟使用临时模拟用户；测试模式使用持久AEP测试配置文件；演习使用真实生产数据，而不联系客户或更新配置文件

**常见问题解答：**
* **问：在创建事件触发的历程之前，我首先需要做什么？**  — 使用数据工程师配置事件以定义触发器及其携带的数据；然后引用事件作为历程入口点。
* **问：对于Journey Optimizer的新用户，建议使用哪个入口点？**  — 从使用读取受众活动的基于受众的历程开始 — 它不需要事先配置事件，是熟悉画布的最简单方法。
* **问：我能否在旅程上线之前对其进行测试？**  — 是；对临时模拟用户使用模拟，对永久AEP测试用户档案使用测试模式，或对真实生产数据使用练习而不发送通信。
* **问：如果我尝试发布时历程出错，会发生什么情况？**  — 您无法发布包含错误的历程；必须在发布之前解决所有配置错误。
* **问：如何分步完成多个步骤的复杂历程？**  — 使用跳转活动连接较小的子历程，从而减少复杂性并使每个子历程更易于独立测试。
