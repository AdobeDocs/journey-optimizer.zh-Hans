...
1. **创建区段**：您的历程还可以侦听Adobe Experience Platform区段，以将消息批量发送到一组指定的用户档案。 为此，您需要创建区段。 [了解更多信息](../segment/about-segments.md)。
...
* **实时历程**：过去24小时内有流量的实时历程总数。 实时历程包括 **单一历程** （基于事件）和 **批量历程** （读取区段）。
* **错误率**：所有出错配置文件与过去24小时内输入的配置文件总数的比率。
* **丢弃率**：与过去24小时内输入的配置文件总数相比，所有配置文件数均出现分数的比率。
...
1. 首先，拖放一个事件或 **读取区段** 活动从面板移入画布。 要了解有关历程设计的更多信息，请参阅 [本节](using-the-journey-designer.md).
...
当 **允许重新进入** 选项处于激活状态， **重新进入等待期** 字段。 使用该字段，您可以定义允许用户档案再次进入单一历程（以事件或区段鉴别开始）之前等待的时间。这可防止同一事件多次错误触发历程。默认情况下，字段设置为 5 分钟。
...
您还可以添加 **结束日期**. 这允许用户档案在到期时自动退出。如果您未指定结束日期，配置文件可以保留到默认历程超时（使用Healthcare Shield附加产品时通常为30天、7天）。 唯一的例外是重复读取区段历程 **在重复时强制重新进入** 激活，在下一次事件的开始日期结束。
