...
* **测试模式**：一种验证模式，使用合成测试配置文件模拟历程执行，而不激活实时通信&#x200B;*（产品特定）*
...
* 请勿混淆：“测试模式”≠“练习” — 测试模式使用合成配置文件；练习使用真实的生产数据，而不与客户联系
...
* **问：我能否在旅程上线之前对其进行测试？**  — 是；将测试模式与合成测试用户档案一起使用来逐步完成历程，或使用模拟运行来针对实际生产数据执行，而不发送任何通信。
