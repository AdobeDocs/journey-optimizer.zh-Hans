...
**在此页面上：**&#x200B;了解如何在创作期间从右边栏设置历程的全局属性 — 包括其名称、进入规则、时区、开始和结束日期、超时、退出标准、冲突管理和维持配置（有限可用性）。
...
* 选择您的历程成功量度。 [了解如何配置和跟踪您的历程成功量度](success-metrics.md)
...
* [使用保持组（有限可用性）测量历程提升](#performance-management)以量化历程的增量有效性
...
用户档案入口管理取决于历程类型。 在[此页面](entry-management.md)上了解有关用户档案进入和重新进入管理的更多信息。 在[本节](entry-management.md#journey-processing-rate)中了解有关历程处理率和用户档案如何流过历程的更多信息。
...
任何配置的开始和结束日期都直接显示在状态徽章旁边的&#x200B;**历程标题**&#x200B;中。 这让您无需打开属性面板即可快速查看旅程计划。 显示的标签会根据每个日期的状态进行调整：

* **开始日期即将到来** — 标题显示配置的开始日期。
* **开始日期已过** — 标题指示历程已开始。
* **即将结束日期** — 标题显示配置的结束日期。
* **结束日期已通过** — 标题指示历程已结束。
...
此全局超时会在个人进入历程 **91 天**&#x200B;后停止个人进度。 这意味着个人的历程不能超过91天。 在此超时时段后，个人数据将被删除。 超时时间结束时仍在历程中流动的个人将被停止，并且不会在报表中考虑他们。 因此，您可能会看到进入历程的人员多于退出的人员。
...
<table style="table-layout:auto">
  ...
  <tr style="border: 1;">
    ...
    <td>
      <p>用户档案的TTL将保持为30天（对于HIPAA为7天），与原始历程版本的发布时间一致。 对于强制重新进入的定期历程，TTL将与定期时段匹配。</p>
    </td>
  </tr>
  <tr style="border: 1;">
    <td>
...
您可以添加多个退出条件。 退出标准现在处于活动状态，将在历程的每个步骤进行评估。

![显示历程终止的受众条件的退出标准面板](assets/exitcriteria-sample.png){width="40%"}


### 基于配置文件属性的退出条件 {#profile-exit-criteria}

通过基于配置文件属性的退出标准，可让您定义在历程恢复之前自动删除特定配置文件的规则，从而更好地控制暂停的历程。 您可以根据配置文件属性（如位置、状态或首选项）设置退出条件，以确保恢复历程后，只有相关的配置文件才能继续历程。

例如，您可以[暂停历程](journey-pause.md)，添加退出条件以删除位于法国的所有用户档案，并在知道这些用户档案将在下一个操作步骤被排除的情况下继续历程。 此逻辑适用于历程中已存在的用户档案，以及历程恢复后符合条件的任何新用户档案。
...
以下护栏和限制适用于[历程退出标准](#exit-criteria-desc)功能：

* 退出条件仅在草稿状态下定义
* 事件和基于事件的退出标准之间的历程命名空间一致性

以下护栏适用于使用[基于配置文件属性的退出标准](#profile-exit-criteria)功能：

* **退出条件适用于操作级别**\
  “配置文件属性”退出标准仅在操作步骤中进行评估。 与其他退出标准类型不同，它们不会在整个历程中全局应用。\
  如果您恢复历程并且某些用户档案满足退出条件，则这些用户档案将在下一个操作节点被排除。\
  此外，对于在恢复后进入历程的新用户档案，如果符合条件，则也会在其第一个操作节点进行评估和排除。

* **每个历程一个基于配置文件的退出规则**\
  您只能为每个历程定义一个“配置文件属性”退出标准。 此限制有助于保持清晰度并避免旅程逻辑中的冲突。
...
>[!AVAILABILITY]
>
>此功能当前处于“有限可用”状态。 要获得访问权限，请与 Adobe 代表联系。 有关发行周期和可用性阶段的完整详细信息，请参阅 [Journey Optimizer 发行周期](../rn/releases.md)。
...
维持是随机选择的目标受众百分比，该受众不会进入历程且不会收到任何通信。 通过将保留用户档案的结果与进入历程的用户档案的结果进行比较，您可以测量增量提升，即历程对受众产生的实际影响。

### 配置维持 {#configure-holdout}
...
1. 输入&#x200B;**[!UICONTROL 保持百分比]**：将排除进入历程的受众份额。
   ...
   *在本例中，50%的受众将分配给保持组，并且不会进入历程。 对于其余受众，该历程将正常运行，并且Customer Journey Analytics将使用电子邮件打开次数(AJO)指标来衡量提升度。*
...
属性位于历程画布的右边栏中。 默认情况下，在您创建新历程时会显示它们。 对于现有历程，单击历程名称旁边的铅笔图标以将其打开。 对于实时历程，面板仅显示发布日期和发布历程的用户名称。 查看[访问历程的属性](#access-properties)。

**我能否更改实时历程的属性？**

历程处于活动状态后，大多数属性均为只读。 要修改它们，请创建新历程版本或复制历程，在草稿中进行更改，然后重新[发布](publish-journey.md)。

**重新进入设置与重新进入等待期之间有何区别？**

**[允许重入](#allow-reentrance)**&#x200B;控制用户档案是否可以多次进入历程。 **[重新进入等待期](#reentrance-wait)**（仅在允许重新进入时显示）定义同一配置文件可以重新进入单一历程之前的等待时间。 默认值为5分钟，最长为90天。 有关更多详细信息，请参阅[配置文件入口管理](entry-management.md)。

**个人资料可在历程中停留多长时间？**

[全局历程超时](#global_timeout)在个人资料&#x200B;**91天**&#x200B;之后停止该个人资料 — 个人历程不能超过该时间。 此超时未显示在界面中，无法更改。 因为用户档案数据在91天后会被删除，所以无法保证再进入阻止会超过该期限。 另请参阅[历程如何结束](end-journey.md#journey-finished-definition)。
...
它取决于历程类型：[读取受众](read-audience.md)和[受众资格](audience-qualification-events.md)历程使用受众的合并策略，[单一事件](../event/about-events.md)历程使用默认合并策略，而[业务事件](../event/about-creating-business.md)历程在以下读取受众活动中使用目标受众的合并策略。 相同的合并策略适用于整个历程。 如果更新了受众合并策略，则必须重新发布或复制引用该受众的任何活动历程。 请参阅[合并策略](#merge-policies)。

**91天历程超时与91天报告窗口有何区别？**

这些是不同的概念。 **[历程全局超时](#global_timeout)** （91天）是单个配置文件在历程中保持活动状态的最长时间，超过此时间后，配置文件将退出并删除其数据。 **报告窗口**（大约91天）是UI显示限制：超过~91天的性能数据不再可见，但历程继续运行且新配置文件继续进入。 有关TTL和数据保留的详细信息，请参阅[生存时间(TTL)和数据保留常见问题解答](#timeout-faq)。

## 相关主题 {#related-topics}

* [用户档案进入管理](entry-management.md) — 配置用户档案如何进入并重新进入历程
* [历程进入和退出标准指南](entry-exit-criteria-guide.md) — 包含真实示例和最佳实践的完整指南
* [历程如何结束](end-journey.md) — 了解历程的自然完成和配置文件退出
* [暂停历程](journey-pause.md) — 暂停并继续具有配置文件属性退出条件的历程
* [时区管理](timezone-management.md) — 配置历程和用户档案时区
* [冲突管理和优先顺序](../conflict-prioritization/conflicts.md) — 识别并解决历程和营销活动之间的冲突
