...
* **TL；DR：**&#x200B;本页介绍如何配置和管理历程的所有全局设置，包括进入规则、时区、开始/结束日期、超时行为、退出标准、有效负载大小和冲突管理。
...
* 规范名称：历程属性 — 缩写：无 — 变体：历程设置，历程配置面板
* 同义词： &quot;global journey timeout&quot; = &quot;TTL&quot; = &quot;Time-to-Live&quot;
* 请勿混淆：“全局历程超时（91天）”≠“报告窗口（约91天）” — 超时限制历程中的单个配置文件持续时间；报告窗口是分析数据的UI显示限制

**常见问题解答：**

* **问：个人资料可在历程中停留多长时间？**  — 最长91天（全局历程超时）；在此时段后，将自动退出用户档案并删除其数据。
* **问：历程处于实时状态时是否可以编辑历程属性？**  — 对于实时历程，属性面板仅显示发布日期和发布者名称；结构更改需要新版本。
* **问：如果配置了多个退出条件，会发生什么情况？**  — 在历程的每一步使用OR逻辑从上到下对它们进行评估；当满足任意一个条件时，就会退出用户档案。
* **问：如何阻止用户档案重新进入历程？**  — 取消选中历程属性中的“允许重新进入”选项；这适用于一次性体验，例如礼品选件。
* **问：历程超时与结束日期有何区别？**  — 结束日期将停止所有新条目，并在该特定日期自动退出活动用户档案；从用户档案进入的那一刻起，91天的全局超时适用于每个用户档案，而不考虑历程的结束日期。
* **问：如何确定历程的合并策略？**  — 取决于历程类型：读取受众和受众资格历程使用受众的合并策略；单一事件历程使用默认合并策略；业务事件历程在后续读取受众活动中使用来自目标受众的合并策略。
