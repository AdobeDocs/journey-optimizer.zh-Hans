...
1. **在历程中添加和配置读取受众活动**
   ...
   对于从CSV文件[&#128279;](https://experienceleague.adobe.com/docs/experience-platform/segmentation/ui/overview.html?lang=zh-Hans#import-audience){target="_blank"}导入的受众，如果您的CSV受众在每个配置文件ID中包含多行，请确保首先启用Express激活 — 请参阅[外部受众的补充标识符](#external-audiences)。
...
有关完整参考，请参阅[创建外部受众API文档](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/segmentation/tutorials/create-external-audience#create){target="_blank"}。
...
* **TL；DR：**&#x200B;本页介绍如何在Adobe Journey Optimizer历程中使用补充标识符，以允许单个配置文件拥有多个并发历程实例，每个实例的作用域都是不同的辅助ID，例如预订、订阅或策略ID。

**意图：**
* 了解何时及为何使用补充标识符而不是仅依赖用户档案ID
* 通过在事件架构中将属性标记为标识，在事件触发的历程中配置补充标识符
* 通过启用读取受众活动中的选项，在读取受众历程中配置补充标识符
* 使用表达式编辑器为消息个性化和条件逻辑引用补充标识符属性
* 应用正确的表达式语法对由补充ID键控的对象数组进行迭代
* 在历程中实施补充标识符之前识别护栏和限制

**术语表：**
* **补充标识符**：与配置文件ID一起使用的辅助标识符（例如，订单ID、预订ID、订阅ID）可将历程实例范围设置为特定记录，从而为每个配置文件&#x200B;*（产品特定）启用多个并发实例*
* **个人资料ID**：默认情况下用于执行历程的主要标识符；历程中活动的个人资料如果没有补充ID，则无法重新进入另一个历程
* **非人员标识符命名空间**：不代表人员的身份命名空间（补充ID所必需的）；必须与主身份命名空间不同
* **joai命名空间**：不适用于此页面（请参阅入站操作疑难解答）
* **DULE**：数据使用标签和执行 — Adobe Experience Platform中的数据治理策略验证框架；补充ID不受DULE检查

**护栏：**
* 仅事件触发和读取受众历程支持补充标识符；受众资格历程不支持补充标识符
* 用户档案的并发历程实例不能超过10个
* 每个历程实例都计入频率封顶，即使是通过补充标识符创建的
* 补充标识符必须是`string`类型；不支持字符串数组和映射
* 不得在架构中将补充ID属性标记为主标识
* 用于补充ID的命名空间必须是非人员标识符命名空间
* 将非人员身份命名空间应用于架构后，必须创建新的事件或字段组；无法刷新现有实体
* 对于具有补充ID的读取受众历程：每个历程实例的读取率限制为每秒500个配置文件；仅支持统一配置文件服务受众；补充ID必须是配置文件字段（而不是事件/上下文字段）
* 同一历程中的下游事件必须使用相同的补充ID和命名空间
* 对于使用业务事件的读取受众历程，补充数据ID被禁用

**术语：**
* 规范名称：补充标识符 — 首字母缩略词：none — 变体：补充标识符，辅助标识符
* 同义词： &quot;supplemental identifier&quot; = &quot;supplemental ID&quot;（在UI和文档中可互换使用）
* 请勿混淆：“supplemental identifier”≠“primary identity” — 在模式中，绝不能将补充ID标记为主要标识

**常见问题解答：**
* **问：用于什么的补充标识符？**  — 它允许单个配置文件同时多次输入和执行历程，每个实例的范围限定为不同的辅助记录，如预订、订阅或策略ID。
* **问：哪些历程类型支持补充标识符？**  — 事件触发的历程和读取受众历程。 受众资格历程不支持补充标识符。
* **问：配置文件可以有多少个具有补充标识符的并发历程实例？**  — 每个配置文件最多10个并发历程实例。
* **问：能否将补充ID属性用于消息个性化？**  — 是的。 通过表达式编辑器或个性化编辑器中的上下文属性菜单引用它们。
* **问：补充标识ID是否需要标记为架构中的主标识？**  — 不。 必须将其标记为身份，但不得设置为主要身份。
* **问：DULE治理策略是否应用于补充标识符？**  — 不。 对补充ID不执行DULE验证检查。
