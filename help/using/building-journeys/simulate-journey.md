...
>[!CAUTION]
>
>发送模拟用户会触发真正的消息发送。 如果受影响的数据集（例如反馈事件或跟踪事件）启用了配置文件，那么即便模拟运行，仍可以在Adobe Experience Platform中为这名用户创建一个永久性配置文件。

模拟用户是您在&#x200B;**[!UICONTROL 模拟设置]**&#x200B;中定义的临时配置文件类实体。 本节介绍如何创建缩览图、保存它们以供重用、在列表中调整或删除它们，并将它们发送到历程中。

如果您的历程使用外部受众，例如CSV或联合受众构成受众，并且您的条件、表达式或个性化依赖于其中的扩充属性，则您可以像配置文件属性一样模拟模拟用户的这些属性。 这样，您就可以根据设置的值将用户引导至特定分支，而无需运行实际受众。
...
1. 首先填充&#x200B;**[!UICONTROL 测试用户]**&#x200B;列表：
   ...
   1. 然后，从合并架构中选择要为该用户填充的属性。 请注意，此处仅提供当前历程中使用的属性，例如表达式或个性化中的属性。
      ...
      对于配置文件属性，属性按来源分组，并分为单独的部分；如果历程读取外部受众（如CSV或联合受众构成受众），则对该受众的扩充属性进行分组。
   ...
   请注意，**[!UICONTROL 从JSON创建]**&#x200B;和从JSON更新&#x200B;**[!UICONTROL 可让您访问存储在模拟用户上的每个属性，包括配置文件属性以及（如果适用）来自外部受众（如CSV或联合受众组合受众）的扩充属性。]**
...
1. 完成测试后，打开&#x200B;**[!UICONTROL 管理模拟]**&#x200B;菜单：
   ...
   * **[!UICONTROL 重置模拟]**&#x200B;以清除当前运行中的所有数据、选定的模拟用户、定义的事件值和其他测试设置，以便您可以从头开始新的模拟。
     ...
     ![使用“重置模拟”和“停止模拟”选项打开管理模拟菜单](assets/simulate-15.png)
...
* **模拟的用户**：模拟中使用的临时类似个人资料的实体。 发送模拟用户会触发真正的消息发送，这当前可能会导致在Adobe Experience Platform *（产品特定）*&#x200B;中创建永久配置文件
