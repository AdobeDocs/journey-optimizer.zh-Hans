...
在高级表达式编辑器中，您将找到 **历程属性** 类别，位于事件和数据源类别的下方。 此类别包含与给定用户档案的历程相关的技术字段。 这是系统从实时历程中检索到的信息，如历程 ID 或遇到的特定错误。

>[!NOTE]
>
>简单表达式编辑器中还提供历程属性。 请参阅此[章节](../condition-activity.md#about_condition)
...
* **记录丢弃的配置文件**：您可以将上限规则从消息中排除的所有用户档案发送到第三方系统以进行记录。 为此，您可以设置一个在发生超时和错误时的路径，并添加一个条件以筛选特定的错误类型，例如：“通过设置规则上限来放弃人员”。 然后，您可以通过自定义操作将已丢弃的用户档案推送到第三方系统。

* **发生错误时发送警报**：每次在消息中发生错误时，您都可以向第三方系统发送通知。 为此，您可以设置路径以防出错，添加条件和自定义操作。 例如，您可以通过Slack渠道发送包含所遇到错误说明的通知。

* **改进报告中的错误** ：您可以为每个错误类型定义条件，而不是让出错的消息只有一个路径。 这将允许您优化报告并查看所有错误类型数据。
...
| | lastErrorCode | 上一错误代码 | 出错的最新活动（节点）的错误代码。 可能的错误： <ul><li>HTTP错误代码</li><li>上限</li><li>timedOut</li><li>错误(示例：发生意外错误时的默认值。 不应该/极少数发生)</li></ul> |
| | lastExecutedActionErrorCode | 上次执行的操作错误代码 | 最新出错操作的错误代码 |
