...
>
>如果对所选客户细分进行流式处理，则属于此客户细分的个人可能会实时进入该历程。如果区段是批处理，则新近符合此区段资格的人员可能会在Adobe Experience Platform上执行区段计算时进入历程。
>
>以读取区段、区段鉴别或业务事件活动开始的历程中，无法使用体验事件字段组。


1. 展开 **[!UICONTROL 事件]** 类别并放置 **[!UICONTROL 区段鉴别]** 活动放入画布。
...
1. 在 **[!UICONTROL 行为]** 字段中，选择要侦听区段入口、出口，还是同时侦听两者。

   >[!NOTE]
   >
   >请注意 **[!UICONTROL 输入]** 和 **[!UICONTROL 退出]** 对应于 **已实现** 和 **已退出** 来自Adobe Experience Platform的区段参与状态。 有关如何评估区段的更多信息，请参阅 [分段服务文档](https://experienceleague.adobe.com/docs/experience-platform/segmentation/tutorials/evaluate-a-segment.html#interpret-segment-results){target="_blank"}.
...
在后跟的条件或操作中使用表达式编辑器时 **[!UICONTROL 区段鉴别]** 活动，则您有权访问 **[!UICONTROL 区段鉴别]** 节点。 您可以选择 **[!UICONTROL 上次资格取得时间]** 和 **[!UICONTROL 状态]** （进入或退出）。
...
此 **[!UICONTROL 区段鉴别]** 活动允许在Adobe Experience Platform区段中获得资格或被取消资格的个人在历程中立即进入。
...
请勿在中使用 **[!UICONTROL 区段鉴别]** 活动，即创建后立即包含的批处理客户细分。 它将避免第一个计算峰值。请注意，如果您要使用从未计算的客户细分，则历程画布中将显示黄色警告。
...
在生产历程中使用客户细分之前，请始终首先评估每天符合此客户细分条件的个人数量。为此，您可以检查 **[!UICONTROL 区段]** 菜单，打开区段，然后查看 **[!UICONTROL 随时间变化的配置文件]** 图表。
