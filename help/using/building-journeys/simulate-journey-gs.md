...
**术语表：**
* **模拟用户**：为模拟创建的类似配置文件的临时实体，不保留在Adobe Experience Platform *（产品特定）*&#x200B;中
...
* **问：模拟与测试模式有何不同？**  — 模拟使用动态创建的临时模拟用户，而不使用持久Adobe Experience Platform配置文件；测试模式使用在AEP中明确标记为测试配置文件的持久配置文件。
