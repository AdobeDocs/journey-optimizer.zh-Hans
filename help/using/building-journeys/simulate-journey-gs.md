...
| 入站渠道 | 您无法在&#x200B;**[!UICONTROL 模拟]**&#x200B;中运行包含入站渠道节点的历程。 |
...
| 配置文件持久性 | 将模拟用户发送到历程会触发通过标准投放管道发送的真实消息。 如果受影响的数据集启用了配置文件，则会导致在Adobe Experience Platform中为该模拟用户创建永久性配置文件，即使该运行被标记为模拟也是如此。 |
...
| AI生成的模拟用户保留 | 10天 | AI生成的模拟用户在创建10天后会自动删除。 |
...
* **TL；DR：**&#x200B;本页介绍了Adobe Journey Optimizer中的历程模拟功能，说明了它与“测试”模式的不同之处、它支持的旅程类型、如何启动模拟，以及它的节点级、功能和量化限制是什么。

**意图：**
* 了解验证历程的模拟模式和测试模式之间的差异
* 为批处理、单一或混合历程类型启动模拟会话
* 识别哪些历程节点阻止或限制模拟运行
* 确定模拟期间不支持哪些功能（例如，同意、频率封顶、STO）
* 围绕量化护栏进行规划，例如每个沙盒的最大模拟用户数
* 根据测试需求决定使用快速模拟还是手动模拟

**术语表：**
* **模拟用户**：为模拟创建的临时配置文件类实体。 发送模拟用户会触发真正的消息发送，这当前可能会导致在Adobe Experience Platform *（产品特定）*&#x200B;中创建永久配置文件
* **模拟**：历程状态（与草稿、测试模式和实时状态一起），用于模拟用户进行测试，而不是永久性测试配置文件&#x200B;*（产品特定）*
* **Journey Agent**：在快速模拟和AI辅助的手动模拟期间生成模拟用户、事件值和测试设置的AI组件&#x200B;*（产品特定）*
* **快速模拟**：生成用户和事件的自动端到端模拟运行，具有最少的手动输入&#x200B;*（产品特定）*
* **手动模拟**：一种逐步模拟模式，在此模式中，单独创建和触发用户和事件&#x200B;*（产品特定）*

**护栏：**
* 至少需要以下项之一： **模拟历程**、**发布历程**&#x200B;或&#x200B;**批准和发布历程**&#x200B;权限
* AI支持的模拟功能需要AI助手功能的&#x200B;**生成内容**&#x200B;权限
* 每个“发送全部”或“触发选定事件”批次最多20个模拟用户
* 每个AI生成请求最多50个模拟用户
* 每次模拟运行最多100个独特模拟用户
* 在一个沙盒中同时运行模拟的最多20个历程
* 一次一个沙盒中最多2,000个活动模拟用户
* 无法模拟业务事件触发的历程
* 无法模拟启用了多次重新进入的补充ID历程
* 在模拟期间不评估同意策略、频率封顶、选择退出、STO和免打扰时间
* 模拟用户不得包含真实的客户数据（不符合GDPR）

**术语：**
* 规范名称：模拟 — 缩写：无 — 变体：历程模拟，模拟模式
* 规范名称：模拟用户 — 缩写：无 — 变体：测试用户（在UI标签中）
* 同义词： &quot;Simulation&quot; = &quot;Simulation mode&quot;；&quot;simulated users&quot; = &quot;test users&quot;（仅限UI标签）
* 请勿混淆：“Simulation”≠“Test mode”（测试模式使用永久性AEP测试配置文件； Simulation使用临时模拟用户）

**常见问题解答：**
* **问：使用模拟需要什么权限？**  — 您至少需要以下之一：模拟历程、发布历程或批准并发布历程。 AI功能另外需要AI助手功能的“生成内容”权限。
* **问：模拟与测试模式有何不同？**  — 模拟使用动态创建的临时模拟用户，通常不在Adobe Experience Platform中预先创建配置文件；测试模式使用在AEP中明确标记为测试配置文件的持续配置文件。 发送模拟用户仍会触发真正的消息发送，这可能导致创建永久配置文件。
* **问：我能否模拟以商业活动开始的历程？**  — 不。 业务事件触发的历程无法在模拟中运行。
* **问：一次模拟运行中可以测试多少模拟用户？**  — 每次运行最多100个唯一的模拟用户；每个“全部发送”操作最多可一次发送20个用户。
* **问：模拟期间是否强制实施同意政策？**  — 不。 模拟期间不会评估同意策略评估、频率封顶、选择退出管理和免打扰时间。
* **问：如果我的旅程在AI生成期间有50多条路径，会发生什么情况？** — Journey Agent随机选择路径以生成最多50个模拟用户。
