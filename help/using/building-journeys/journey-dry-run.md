...
* 练习历程会在14天后自动退出练习模式并返回草稿状态。 无历程内容丢失；只有练习会话结束。
* 在试运行期间处理的用户档案将计入可参与用户档案和实时历程配额
* 在练习期间不执行渠道操作节点（电子邮件、短信、推送）和自定义操作
* 在练习中未启用跳转操作
* 练习期间不执行反应节点；配置文件成功退出，具有并行单一分支和反应分支的优先级规则
* 仅当练习处于活动状态时，报告数据才可用；一旦停止，数据将无法再访问
* 模拟历程不会影响业务规则
* 对于使用具有计划时间（每日、每周或每月）的&#x200B;**读取受众**&#x200B;活动的历程，练习不遵循配置的历程计划 — 该计划锚定到激活练习的时刻（例如，历程设置为10 AM，练习在上午8 AM激活→所有读取期间在上午8 AM执行）

**术语：**
* 规范名称：历程练习 — 首字母缩略词：none — 变体：练习模式，练习发布模式
* 同义词：“Dry run”=“smoke test”（非正式的）
* 请勿混淆：“练习”≠“测试模式”≠“模拟” — 练习使用实际生产数据，并计入可参与用户档案和实时历程配额；测试模式在草稿历程中使用永久性AEP测试用户档案；模拟使用在AEP中不永久性的临时模拟用户
...
* **问：练习是否遵循读取受众历程中配置的计划执行时间？**  — 不。 练习将计划锚定到激活时间，而不是配置的历程时间。 如果历程设置为在上午10点运行，但练习在上午8点激活，则练习期间的所有计划读取都会在上午8点执行。
