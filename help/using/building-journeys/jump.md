...
* **命名空间要求：** **[!UICONTROL 跳转]**&#x200B;活动仅适用于使用命名空间的历程。 您只能跳转到使用与起源历程相同的命名空间的历程。

* **目标历程约束：**&#x200B;您无法跳转到以&#x200B;**受众资格**&#x200B;事件或&#x200B;**读取受众**&#x200B;开始的历程。

* **受众资格支持（渐进式转出）：**&#x200B;如果历程以受众资格开始，并且跳转重定向到基于事件的开始历程，则可以在同一历程中具有&#x200B;**[!UICONTROL 跳转]**&#x200B;活动和&#x200B;**受众资格**&#x200B;事件。 此功能正在逐步推出。 如果您在环境中没有看到此内容，可能是因为您仍在受众资格中使用批量受众。

* **读取受众限制：**&#x200B;您无法在同一历程中拥有&#x200B;**[!UICONTROL 跳转]**&#x200B;活动和&#x200B;**读取受众**&#x200B;事件。

* **多个跳转：**&#x200B;您可以在历程中包含所需数量的跳转&#x200B;**[!UICONTROL 跳转]**&#x200B;活动。 执行&#x200B;**[!UICONTROL 跳转]**&#x200B;后，您可以添加任何需要的活动。 您可以根据需要设置任意数量的跳转级别（例如，历程A跳转到历程B，然后跳转到历程C）。 目标历程还可以包含所需数量的跳转&#x200B;**[!UICONTROL 跳转]**&#x200B;活动。

* **循环防止：**&#x200B;不支持循环模式。 无法将两个或更多历程链接在一起，从而导致无限循环。 **[!UICONTROL 跳转]**&#x200B;活动配置屏幕阻止您执行此操作。
...
1. 在&#x200B;**目标历程**&#x200B;字段中单击。
列表会显示草稿、实时或测试模式中的所有历程版本。 使用其他命名空间或以&#x200B;**读取历程**&#x200B;事件开头的受众不可用。 以&#x200B;**受众资格**&#x200B;事件开始的历程仅在您的原始历程也以Audience Qualification开始时才可用（贵组织中新的Audience Qualification节点已弃用批量受众）。 还会过滤掉会创建循环模式的目标历程。
...
* 无法在同一历程中使用跳转活动和读取受众事件
* 仅当历程以受众资格开始并跳转到基于事件的历程（取决于功能标志转出）时，才能在同一历程中使用跳转活动和受众资格事件
...
* **问：我是否可以跳转到“读取”受众历程？**  — 否；您无法跳转到以读取受众事件开始的历程。 您也无法跳转到以受众资格事件开始的历程。 但是，如果您的起源历程以受众资格事件开始，则可以跳转到基于事件的历程（此功能正在逐步推出，可能尚未在所有组织中提供）。
