...
* **TL；DR：**&#x200B;本页介绍Journey Optimizer旅程设计器画布，说明如何通过从面板拖放事件、编排和操作活动来构建多步旅程。

**意图：**

* 导航历程设计器界面（面板、画布、工具栏、活动配置窗格）
* 将事件、编排活动和操作活动添加到历程画布
* 在超时或错误时为条件和操作活动配置备用路径
* 在同一历程中或在同一实例的不同历程中复制和粘贴活动
* 使用事件触发器或读取受众入口点启动历程

**术语表：**

* **调色板**：历程设计器中的左侧面板，其中列出了可拖放到画布&#x200B;*（特定于产品）*&#x200B;上的所有可用事件、编排和操作活动
* **画布**：历程设计器的中心设计区域，在此区域中放置、连接和配置活动&#x200B;*（产品特定）*
* **活动配置窗格**：在画布上选择活动时打开的右侧面板，用于填写活动设置&#x200B;*（产品特定）*
* **历程片段**：预先构建的历程节点集可重复使用，可以直接插入画布以避免重建公共逻辑&#x200B;*（产品特定）*
* **反应事件**：在消息之后放置的事件活动，以根据收件人跟踪交互（打开、点击）分支历程&#x200B;*（产品特定）*

**护栏：**

* 操作、条件、等待活动和反应事件不能作为新历程的第一步。
* 仅支持在同一实例中复制/粘贴，不支持跨实例复制/粘贴。
* 您无法将事件复制/粘贴到使用其他命名空间的目标历程中。
* 从其他沙盒粘贴的活动可能引用目标历程中不存在的数据。
* 只有事件和等待活动可以并行设置；其他活动类型不能并行运行。
* 替代路径（超时/错误回退）仅可用于“条件”和“操作”活动。

**术语：**

* 规范名称：Designer — 缩写：none — 变体：旅程画布、编排画布
* 同义词： &quot;palette&quot; = &quot;activity panel&quot;；&quot;canvas&quot; = &quot;design area&quot;
* 请勿混淆：“events”（触发历程进入或分支）≠“actions”（客户发生的情况，例如发送消息）

**常见问题解答：**

* **问：用户档案如何进入历程？**  — 在收到配置的事件时统一实时输入配置文件，或在读取受众活动触发历程时批量输入配置文件。
* **问：我能否向历程添加多个事件？**  — 是，您可以添加多个事件，前提是它们都使用相同的命名空间。
* **问：如何在操作失败时定义回退？**  — 在活动属性中，启用“在超时或错误的情况下添加替代路径”选项以在活动后添加回退路径。
* **问：能否从只读历程复制活动？**  — 可以，您可以从任何历程复制活动，而不管其状态如何，但只能粘贴到同一实例中。
* **问：什么是历程片段？**  — 一组可重复使用的预建旅程节点（例如资格检查、欢迎序列），可直接插入画布上以避免从头开始重建公共逻辑。
