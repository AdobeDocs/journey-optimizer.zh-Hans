...
* 使用条件活动检查当天是星期六还是星期日
* 使用周末输入的自定义公式实施等待活动
* 将星期一投放的周末电子邮件在特定小时排入队列
* 立即发送工作日条目（星期一至星期五）的电子邮件
...
* 活动的Adobe Journey Optimizer实例
* 已配置的[电子邮件渠道表面](../configuration/channel-surfaces.md)
* 用于触发历程的[受众](../audience/about-audiences.md)或[事件](../event/about-events.md)
* 基本了解[历程条件](condition-activity.md)和[表达式](expression/expressionadvanced.md)
...
要增强您的工作流并处理更复杂的业务要求，请执行以下操作：

* **复杂的工作时间**：将公式扩展到除基本工作日检查之外的假日、时区或特定工作时间。

* **自定义投放时间**：调整等待公式中的小时参数(H)以匹配您的最佳发送时间。 例如，如果上午10点显示更好的参与率，则将公式更改为使用小时10。

* **多时区支持**：考虑为不同的地理区域创建单独的历程，以确保每个收件人的星期一送达都为当地时区。

### 时区管理

`now()`函数和历程执行使用在历程级别配置的时区。 请考虑以下事项：

* **时区**&#x200B;历程：确保历程时区与您的需求相匹配。 在发布之前，在历程属性中配置此设置。 [了解有关时区管理的更多信息](timezone-management.md)。

* **全局受众**：如果您的受众跨越多个时区，则星期几检查将在历程配置的时区中进行，而不是在收件人的本地时区中进行。

* **本地化计划**：对于特定于时区的投放，请为不同区域创建单独的历程，或在读取受众活动中使用时区设置。

### 历程输入和计时

* **读取受众历程**：对于批处理历程，[安排读取受众](read-audience.md#schedule)在对受众有意义的时间触发。 清晨执行（例如，上午6:00）在业务通信中很常见。

* **基于事件的历程**：收到事件后将立即评估条件。 周末输入的配置文件将自动等待到星期一。 [了解有关事件的更多信息](../event/about-events.md)

* **等待超时注意事项**：确保您的[历程超时设置](journey-properties.md#timeout)满足最长等待时间（从星期六到星期一，最长2天）。

### 测试至关重要

正如实施指南中强调的那样，请始终测试历程逻辑以确认一切都按预期运行：

* 使用&#x200B;**测试模式**&#x200B;模拟不同的进入方案，而不发送真正的电子邮件
* 测试所有三个路径：星期六条目、星期日条目和工作日条目
* 验证等待持续时间计算是否正确
* 确认星期一的投放在指定时间发生
* 检查历程可视化以确保正确的路径路由
...
* 对于定期营销活动，请正确配置&#x200B;**[!UICONTROL 重新进入]**&#x200B;设置。 [了解有关重新进入设置的更多信息](entry-management.md)

* 如果用户档案可以重新进入历程，则每次都将接受星期几检查，确保周末条目始终排队等待星期一。

* 考虑添加[频率上限规则](../conflict-prioritization/journey-capping.md)，以避免用户档案可以频繁地重新输入时过度发送消息。
...
您可以使用不同的等待公式为不同的周末行为创建多个路径：

* **星期六→星期三投放**：使用`nowWithDelta(4, "days")`
* **星期日→星期二投放**：使用`nowWithDelta(2, "days")`
