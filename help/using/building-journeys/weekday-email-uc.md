...
* **多时区支持**：为不同的地理区域创建单独的历程，以确保星期一按照每个收件人的当地时区交付。

### 时区管理

`now()`函数和历程执行使用在历程级别配置的时区。 考虑以下要点：

* **时区**&#x200B;历程：在发布之前，通过在历程属性中配置此时区，确保历程时区与您的需求相匹配。 [了解有关时区管理的更多信息](timezone-management.md)
...
* **读取受众历程**： [安排读取受众](read-audience.md#schedule)在对受众有意义的时间触发。 清晨执行（例如，上午6:00）在业务通信中很常见。
...
[了解有关测试历程的更多信息](testing-the-journey.md)

### 重返大气层及其频率

对于周期性活动，请仔细管理用户档案重新输入：

* **配置重新进入**：正确设置&#x200B;**[!UICONTROL 重新进入]**&#x200B;设置。 [了解有关重新进入设置的更多信息](entry-management.md)
* **一致的行为**：如果配置文件可以重新进入历程，则每次都将接受星期几检查，确保周末条目始终排队等待星期一。
* **频率封顶**：考虑添加[频率封顶规则](../conflict-prioritization/journey-capping.md)，以避免在配置文件可以频繁重新输入时过度发送消息。

## 高级变量

### 特定日期定位

仅在特定日期（例如，星期二和星期四）发送电子邮件：
...
2. **添加等待活动**，用于计算到下个星期二或星期四之前的天数。

### 不同天的不同发送时间

使用不同的等待公式创建多个路径，以实现灵活计划：
...
1. **调整等待公式中的小时参数**。 例如，对于在下午2点（而不是上午9点）投放：

   ```javascript
   setHours(nowWithDelta(1, "days"), 14)
   ```

2. **添加时间检查**（可选）：在等待之后添加第二个条件，以验证当前时间在发送前的营业时间内。

### 假日排除

要从电子邮件发送中排除假日，请执行以下操作：
...
2. **如果条件与假日匹配，请添加“等待”活动**，以延迟到下一工作日。

[了解有关日期比较函数的更多信息](functions/date-functions.md)
