...
* **测试模式**：一种历程验证模式，使用测试配置文件模拟执行，以在启动前捕获问题。 *（产品特定）*
* **练习**：一种旅程预览模式，模拟执行而不影响实际的配置文件数据或发送消息。 *（产品特定）*
...
* 请勿混淆：“练习”≠“测试模式” — 练习预览执行，但不接触实际数据；测试模式使用实际测试配置文件模拟历程
...
* **问：如何在不发送真实消息的情况下验证历程？**  — 使用模拟运行模式，该模式可在不影响实际数据或发送消息的情况下预览历程执行。
