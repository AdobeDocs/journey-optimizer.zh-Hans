...
历程片段是可重用的历程节点集，您可以一次性生成此节点，然后将其放到沙盒的任意历程中。 无论是资格检查、首选渠道路由逻辑还是欢迎序列，片段都可以帮助团队更快地移动并保持一致，而无需每次从头开始重建相同的逻辑。 [查看用例示例。](#examples)

创建后，片段会被存储在专用的&#x200B;**[!UICONTROL 片段清单]**&#x200B;中，并可使用&#x200B;**[!UICONTROL 历程片段]**&#x200B;活动将其插入任何历程。
...
>title="另存为一个历程片段"
>abstract="保存之前需输入唯一的片段名称。 所选节点将保存为可重复使用的片段，并可在片段库存中使用。"
...
>title="历程片段属性"
>abstract="从库存中打开片段后，可以修改其节点、属性、标记或标签。 活跃的片段必须先停用，然后才能进行编辑。"
