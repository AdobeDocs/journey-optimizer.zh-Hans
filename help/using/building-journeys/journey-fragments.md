...
>[!NOTE]
>对于此第一个版本，历程片段使用&#x200B;**复制行为**：将片段插入历程中创建原始节点的静态副本。 对原始片段所做的任何更新都不会自动反映在已使用该片段的历程中。
...
1. 单击 **[!UICONTROL Save]**。片段将另存为草稿。

>[!TIP]
>如果从历程创建片段，请在保存片段之前[测试您的历程](testing-the-journey.md) ****，以确保所选节点按预期运行。
...
>[!CAUTION]
>测试模式在片段编辑器中不可用。 这意味着在激活片段并将其插入到历程之前，无法验证配置的活动的行为。 对于逻辑准确性至关重要的片段，请考虑先在完整历程[中构建和测试节点](testing-the-journey.md)，然后从上面的画布选项卡中将它们另存为片段。
...
>
>* 测试模式在片段编辑器中不可用。 在将节点另存为片段之前，在完整历程中测试任何历程级别逻辑。
>
...
要激活&#x200B;**[!UICONTROL 草稿]**&#x200B;片段，请将其打开并使用&#x200B;**[!UICONTROL 激活]**&#x200B;图标。 要停用&#x200B;**[!UICONTROL 活动]**&#x200B;片段，请将其打开并使用&#x200B;**[!UICONTROL 停用]**&#x200B;图标。
...
* **[!UICONTROL 复制]**：从&#x200B;**[!UICONTROL 更多操作]** (...)图标创建片段的副本。
* **[!UICONTROL 删除]**：从&#x200B;**[!UICONTROL 更多操作]** (...)图标的活动清单中删除片段。
* **[!UICONTROL 编辑标记]** — 从&#x200B;**[!UICONTROL 更多操作]** (...)图标添加或删除片段的标记。
...
>title="添加历程片段"
>abstract="选取器中只有&#x200B;**[!UICONTROL 活动]**&#x200B;片段可用。 插入片段会创建其节点的&#x200B;**静态副本** — 对原始片段的任何后续更新都不会反映在历程中。"

要将片段插入历程，请执行以下操作：

1. 打开您的旅程，然后从左边栏拖动&#x200B;**[!UICONTROL 历程片段]**&#x200B;活动。
1. 将其拖放到现有分支中。 出现片段选取器。
...
* 可以使用[历程片段](../start/search-filter-categorize.md)类别下的&#x200B;**[!UICONTROL 统一搜索]**&#x200B;栏找到片段。
...
标准进入模式（例如[读取受众](read-audience.md)节点，后跟资格过滤器）可以封装到片段中。 这允许团队保持用户档案进入历程方式的一致性，同时减少设置时间。 片段只能是[Condition](condition-activity.md)，也可以同时是Read Audience和Condition。

![资格检查片段示例](assets/journey-fragments-uc-eligibility-check.png)

**首选渠道**

片段可以评估用户档案的首选通信渠道（电子邮件、推送或短信），并相应地路由用户档案。 此逻辑可以在任何涉及出站消息传递的历程中重复使用，以确保一致的渠道首选项管理。 片段可以包含[条件](condition-activity.md)和所有三个通道分支。
