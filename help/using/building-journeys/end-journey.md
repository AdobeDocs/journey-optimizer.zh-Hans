...
1. 当&#x200B;**最后一个活动用户档案退出**&#x200B;时，历程进入安全缓冲期并保持&#x200B;**[!UICONTROL 实时]**&#x200B;状态。
1. 安全缓冲区经过后（历程的计划运行时间后约96小时），历程会在下次扫描器通过时自动转换为&#x200B;**[!UICONTROL 已停止]**&#x200B;状态。

此行为仅适用于&#x200B;**非循环读取受众历程**。 周期性历程不受影响。

* **自动停止计时：**&#x200B;安全缓冲区包含两个窗口：一个是&#x200B;**24小时空闲窗口**，允许完成任何飞行中的发送；另一个是&#x200B;**72小时免打扰时间** （免打扰时间最多可延迟发送72小时，扫描仪看不到该时间）。 在历程的计划运行时间后，缓冲区的总时间约为&#x200B;**96小时（~4天）**。 在此期间，历程保持在&#x200B;**[!UICONTROL 实时]**&#x200B;状态。 这是预期行为，并不表示存在问题。

* **排除基于批次的历程：**&#x200B;此自动停止行为不适用于基于批次的历程，包括使用发送时间优化的历程。 这些历程在所有计划批次中保持活动状态，并且仅通过标准[91天全局超时](../building-journeys/journey-properties.md#global_timeout)停止，除非手动关闭或停止这些历程。
...
|--------------|------------|---------------|--------------------------|
| 读取受众 | 否 | 不适用 | 最后一个配置文件退出后约96小时（自动停止缓冲区） |
...
* **TL；DR：**&#x200B;本页介绍了实时历程的不同结束方式（包括全局91天超时、手动关闭新入口和紧急停止）及其对正在进行中的用户档案的影响。

**意图：**

* 将实时历程关闭到新入口，同时允许当前用户档案完成该历程
* 立即停止历程以停止所有正在处理的用户档案
* 了解“已关闭”、“已停止”和“已完成”历程状态之间的区别
* 根据历程的类型和配置确定历程何时视为“已完成”
* 历程达到“已完成”状态后将其删除

**术语表：**

* **结束标记**：在创作过程中，每个历程路径的末尾会显示一个自动生成的不可移除节点；其标签可以更改&#x200B;*（产品特定）*
* **关闭新入口**：手动操作，阻止新配置文件进入历程，同时允许现有配置文件完成其路径&#x200B;*（产品特定）*
* **全局历程超时**：历程最长为91天，超过此时间后，历程会自动切换到“已完成”状态并删除所有配置文件数据&#x200B;*（产品特定）*
* **已停止状态**：历程状态，在该状态中，所有正在进行的配置文件将立即停止；仅用于紧急情况&#x200B;*（产品特定）*

**护栏：**

* 无法重新启动或删除已关闭和已停止的历程；只能创建新版本或创建重复项。
* 只能删除处于已完成状态的历程。
* 停止历程需要“管理历程”权限；具有内联营销活动或消息传送节点的历程还需要“营销活动”>“发布营销活动”权限。
* 在91天的全局超时之后，所有配置文件历程数据都会被删除，剩余配置文件会自动退出。
* 没有长时间运行的等待、反应或事件触发节点的非循环读取受众历程会在最后一个配置文件退出后约96小时（约4天）自动转换为已停止。 在此缓冲期间，历程保持实时状态。 基于批次的历程（包括发送时间优化用例）将从此自动停止中排除，并保持在91天全局超时范围内，除非手动关闭或停止。

**术语：**

* 规范名称：关闭新入口 — 首字母缩写：n/a — 变体：关闭历程，手动关闭
* 同义词：“已停止”历程≠“已关闭”历程 — 已停止立即暂停所有配置文件；已关闭仅阻止新进入
* 请勿混淆：“结束标记”≠“结束活动” — “结束”标记是自动生成的，无法删除；“结束”活动是可投放的画布节点

**常见问题解答：**

* **问：关闭历程与停止历程有何不同？**  — 关闭会阻止新入口，但允许完成现有用户档案；停止会立即停止其跟踪中的所有用户档案。
* **问：为什么非循环历程在运行时数天保持实时状态？**  — 这是正常情况。 在最后一个配置文件退出后，AJO将应用约96小时（约4天）的安全缓冲：24小时允许飞行中发送完成，72小时允许安静时间延迟。 历程在缓冲区经过后在下一次扫描程序通过时转换为“已停止”。
* **问：基于批次的历程在约96小时后是否自动停止？**  — 不。 基于波次的历程（包括使用发送时间优化的历程）会从此自动停止中排除，以便它们可以在所有计划的波次中保持活动状态。 除非已关闭或手动停止，否则它们将遵循标准91天历程超时。
* **问：读取受众历程何时达到“已完成”状态？**  — 对于非循环读取受众历程：它会在最后一个配置文件退出后约96小时（约4天）自动停止到已停止（安全缓冲区：24小时空闲窗口+ 72小时免打扰时间）。 在此缓冲期间，历程保持实时状态。 如果等待、反应或事件节点使配置文件保持活动状态，则应用标准的91天全局超时。 当已关闭的历程点击91天的全局超时时，或按finished-definition表中的循环历程规则时，会达到finished 。
