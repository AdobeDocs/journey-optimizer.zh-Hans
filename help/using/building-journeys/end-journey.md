...
* 没有长时间运行的等待、反应或事件触发的节点的非循环读取受众历程在其计划运行后约96小时（约4天）自动转换为已停止。 在此缓冲期间，历程保持实时状态。 基于批次的历程和使用发送时间优化的历程将从此自动停止中排除，并保持受91天全局超时的限制，除非手动关闭或停止。
...
* **问：为什么非循环历程在运行时数天保持实时状态？**  — 这是正常情况。 AJO应用了大约96小时（大约4天）的安全缓冲：24小时允许飞行中发送完成，72小时允许安静时间延迟。 历程在缓冲时间过后不久过渡到“停止”。
* **问：基于批次的历程在约96小时后是否自动停止？**  — 不。 基于批次的历程和使用发送时间优化的历程将从此自动停止中排除，以便他们可以在所有计划的批次中保持活动状态。 除非已关闭或手动停止，否则它们将遵循标准91天历程超时。
* **问：读取受众历程何时达到“已完成”状态？**  — 对于非循环读取受众历程：它会在计划运行后约96小时（约4天）自动停止到已停止（安全缓冲：24小时空闲时段+ 72小时免打扰时间）。 在此缓冲期间，历程保持实时状态。 如果等待、反应或事件节点使配置文件保持活动状态，则应用标准的91天全局超时。 当已关闭的历程点击91天的全局超时时，或按finished-definition表中的循环历程规则时，会达到finished 。
