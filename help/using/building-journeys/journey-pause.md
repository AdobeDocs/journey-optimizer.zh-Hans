...
| 历程活动 | 轮廓管理 | 注释 |
|-------------------------|--------------------------------------------------|------------------------|
| [受众资格](audience-qualification-events.md) | 在第1个节点中：已丢弃<br>在其他节点中：与实时历程中的行为相同，但是，如果受众资格在操作活动之后并且用户在该操作上暂停，则受众资格将被丢弃。 |          |
| [商业活动](general-events.md) | 已丢弃 |    |
| [单一事件](general-events.md) | 在第1个节点中：已丢弃<br>在其他节点中：与实时历程中的行为相同，但是，如果事件在操作活动之后并且用户在该操作上暂停，则该事件将被丢弃。 | 精心制作您的消息 |
| [读取受众](read-audience.md) | 与实时历程中的行为相同，但有几个特性：<br>如果在读取受众活动开始后按下暂停，则进入历程的用户档案将继续（直到下一次操作活动）。 当历程以特定速度读取受众时，如果整个受众尚未进入，则将丢弃队列中的剩余配置文件。 |  — 对于单次执行：如果计划日期早于恢复日期，则在恢复时不会显示任何错误。 该时间表将被忽略。 <br> — 对于增量历程： <br>如果暂停发生在第一次发生之前，则在继续时，将播放整个受众。 <br>例如，如果发生暂停，则在每日重复周期的第4天，历程保持暂停状态直到第9天，则在恢复时，将包含从第4天到第9天输入的所有用户档案 |
| [反应](reaction-events.md) | 与实时历程中的行为相同，但是，如果反应在操作活动之后并且用户在该操作上暂停，则将丢弃事件。 |
...
| [更新配置文件](update-profiles.md)和[跳转](jump.md) |  |       |
