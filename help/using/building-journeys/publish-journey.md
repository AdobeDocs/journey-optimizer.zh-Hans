...
使用其中一个可用的测试选项验证您的历程。 每个报表包使用不同类型的数据，并适用于内部版本的不同阶段：
...
如果您的历程受审批策略的约束，请单击&#x200B;**[!UICONTROL 发布]**&#x200B;提交该策略以供审批，而不是立即发布。 一旦审批者签发，历程就会自动发布 — 之后无需执行单独的发布步骤。 [了解有关批准的更多信息](../test-approve/gs-approval.md)。
