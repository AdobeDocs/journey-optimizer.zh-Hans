...
* 请勿混淆：模拟（临时模拟用户，不需要AEP测试用户档案）≠测试模式（持久AEP测试用户档案，仅限草稿历程）≠试运行（实际生产受众数据，无联系人，无配置文件更新，绕过操作节点）
...
* **问：发布前是否需要批准？**  — 仅当您的历程受审批策略约束时；在这种情况下，发布会提交历程以供审批，而不是立即发布，并且会在审批者签署后自动发布。
