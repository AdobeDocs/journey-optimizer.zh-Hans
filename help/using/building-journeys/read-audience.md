...
使用读取受众活动，开始具有已定义受众的历程。 您可以选择受众及其运行时间；然后使用[条件来分段、排除或合并分支](#audience-targeting-in-journeys)、计时器和操作来个性化每个配置文件的路径。
...
**键限制：**&#x200B;每个历程一个读取受众（必须是第一个活动或业务事件后的第二个受众）；每个活动一个受众；每个组织最多运行五个并发读取受众；每个沙盒每秒有20,000个配置文件；12小时作业超时。 [护栏和建议](#must-read)中的完整详细信息。
...
* 历程中只能使用一个&#x200B;**[!UICONTROL 读取受众]**&#x200B;活动。 它必须是画布中的第一个活动，但以&#x200B;**业务事件**&#x200B;活动开始的历程除外，在这种情况下，它是第二个活动。
...
>[!TIP]
>
>要在时间范围内批量投放出站消息而不是一次投放所有出站消息，您可以在历程计划中配置波次发送。 [了解如何在历程中使用批次发送](send-using-waves.md)
...
>[!NOTE]
>
>[**优化**&#x200B;活动](optimize.md)为高级路径路由提供了替代方法，包括实验和定位规则。 它当前处于“有限可用”状态 — 请联系您的Adobe代表以了解详情。
...
如果计数不匹配或零配置文件运行在执行以下步骤后持续存在，[请与Adobe支持部门联系](../start/user-interface.md#support-ticket-guidelines)。 准备就绪：受众名称/ID、历程名称/ID、计划运行时间、沙盒和差异的简短描述（例如“受众展示已实现10,000项，只有2,000项在[日期]进入历程”）。
