...
* **`inAudience()`条件计时：**&#x200B;在读取受众历程的条件节点中使用`inAudience()`时，将从用户档案的批量投影中读取区段成员资格。 此投影中的数据新鲜度在引入后最多可保持&#x200B;**2小时**&#x200B;的SLT。 有关传播计时方案的完整详细信息，请参阅[inAudience函数文档](functions/functioninaudience.md#propagation-timing)。
