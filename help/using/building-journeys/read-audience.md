...
>title="读取受众活动"
>abstract="将所选 [!DNL Adobe Experience Platform] 受众中的所有符合条件的轮廓添加到此历程中。 可执行一次，也可按计划执行。"
...
>
>**真实示例**
>* **每周新闻稿**→读取受众。 您的受众是每日批次快照。 您计划每个星期一早上9点到达。 所有符合条件的配置文件一起输入。
>* **忠诚度级别升级**→受众资格。 用户档案一在流受众中达到“金级”状态，就会立即进入历程并收到祝贺电子邮件。
>* **重新参与系列**→读取受众。 您每30天运行一次定期历程，以长达90天以上的不活跃用户档案为目标。

**键限制：**&#x200B;每个历程一个读取受众（必须是第一个活动）；每个活动一个受众；每个组织最多并发运行五个读取受众；每个沙盒每秒有20,000个配置文件；12小时作业超时。 [护栏和限制](../start/guardrails.md#read-segment-g)中的完整详细信息。

**先决条件：**&#x200B;已生成并评估的[!DNL Adobe Experience Platform]受众（已实现状态）、为历程选择的基于人员的身份命名空间，以及对于定期运行，了解[计划和吞吐量限制](../start/guardrails.md#read-segment-g)。

例如，在[生成受众](../audience/about-audiences.md)用例中创建的`Luma app opening and checkout`受众可以用作入口点。 所有符合条件的用户档案都会使用条件、计时器、事件和操作进入旅程并经过个性化路径。
...
>title="受众"
>abstract="其轮廓将进入此历程的 [!DNL Adobe Experience Platform] 受众。 将读取所有符合条件的轮廓。 建议使用批量受众以获得可靠且一致的计数结果，并且每个活动只能读取一个受众。"

>[!CONTEXTUALHELP]
>id="ajo_journey_read_segment_namespace"
>title="命名空间"
>abstract="用于识别进入历程个人的身份标识（例如电子邮件地址、ECID）。 仅支持基于个人的命名空间，缺少此身份标识的轮廓无法进入历程。 默认情况下，这个字段会预填充为上次使用的命名空间。"
...
1. 在&#x200B;**[!UICONTROL 受众]**&#x200B;字段中，选择将进入历程的[!DNL Adobe Experience Platform]受众，然后单击&#x200B;**[!UICONTROL 保存]**。 您可以选择使用[区段定义](../audience/creating-a-segment-definition.md)生成的任何[!DNL Adobe Experience Platform]受众。
...
>title="开始日期/时间"
>abstract="历程开始读取受众并允许轮廓进入的日期和时间。 可结合下方的重复执行选项来安排定期运行。"

>[!CONTEXTUALHELP]
>id="ajo_journey_read_segment_scheduler_repeat_until"
>title="重复直到"
>abstract="定期运行停止的日期。 在此日期之后，历程将不再读取受众，也不再允许新的轮廓进入。"

>[!CONTEXTUALHELP]
>id="ajo_journey_read_segment_scheduler_repeat_every"
>title="重复每一次"
>abstract="定义历程重新读取受众并再次运行的频率，例如每天或每周。 确定两次运行之间的重复间隔，直至达到“重复至”日期。"
...
>title="强制重入"
>abstract="在每次重新读取受众之前，清除历程中的所有参与者，以便每次运行都从头开始，并允许轮廓在每次执行时重新进入。"
...
>**受众快照的使用方式**
>
>每个读取受众执行都使用运行执行时可用的受众成员资格。 对于批处理受众，[!DNL Journey Optimizer]从最新的可用批处理受众快照中读取。 它不会在历程开始时实时重新计算受众。
>
>对于定期历程，每个发生次数都使用可用于该发生次数的快照。 如果您希望历程在运行时等待最新的批次受众评估，请启用&#x200B;**[!UICONTROL 在批次受众评估后触发]**。
...
使用此选项时，系统会从[!DNL Adobe Experience Platform]的分段服务执行的上次受众评估作业的时间开始，回顾&#x200B;**24小时**。
...
>[!CAUTION]
>
>对于[自定义上传受众](../audience/custom-upload.md) （CSV上传）和其他外部受众（例如，联合受众合成），**[!UICONTROL 当前在功能上不支持增量读取]**。 在每个循环中，都会处理&#x200B;**整个受众**，而不管增量读取切换设置如何。
>
>若要控制周期性条目，请使用[强制重复重入](#schedule)。
...
+++**如何将[!UICONTROL 增量读取]和[!UICONTROL 在重复时强制重新进入]协同工作**

这两个选项控制旅程执行的不同部分：

* **[!UICONTROL 增量读取]**&#x200B;控制&#x200B;**从受众**&#x200B;中为下一次定期运行选择哪些配置文件。
* **[!UICONTROL 强制循环重入]**&#x200B;控制&#x200B;**当下一次循环运行开始时，在历程**&#x200B;中仍处于活动状态的用户档案会发生什么情况。

请使用下表了解下次运行时的组合行为。

| [!UICONTROL 增量读取] | [!UICONTROL 在重复时强制重入] | 下次运行时的行为 |
| ------------------------------ | ------------------------------------------- | ------------------------ |
| 关闭 | 关闭 | [!DNL Journey Optimizer]读取该运行的完整受众。 在历程中仍处于活动状态的用户档案不会自动重置。 |
| 于 | 关闭 | [!DNL Journey Optimizer]仅读取自上次执行以来添加到受众的用户档案。 在历程中仍处于活动状态的用户档案不会自动重置。 |
| 关闭 | 于 | [!DNL Journey Optimizer]在开始下一次运行之前从当前历程执行中删除活动参与者，然后再次读取完整受众。 这允许用户档案在新发生次数时重新开始。 |
| 于 | 于 | [!DNL Journey Optimizer]在开始下一次运行之前从当前历程执行中删除活动参与者，然后仅读取自上次执行以来添加到受众的用户档案。 强制重新进入会重置活动历程参与，但增量读取仍会将选择限制为新添加的受众成员。 |

换句话说，**[!UICONTROL 在重复时强制重入]不会禁用[!UICONTROL 增量读取]**。 如果同时启用这两个选项，则在下一次发生开始之前，会从其活动历程实例中删除用户档案，但下一次发生时仍会仅选择自上次执行以来被视为新的受众成员。

>[!IMPORTANT]
>
>**[!UICONTROL 强制重复登录]**&#x200B;删除的配置文件不会自动视为&#x200B;**[!UICONTROL 增量读取]**&#x200B;的新受众成员。 受众选择仍取决于自上次执行以来是否将用户档案新添加到受众。
...
## 历程中的受众定位 {#audience-targeting-in-journeys}
...
* **`inAudience()`条件计时：**&#x200B;在读取受众历程的条件节点中使用`inAudience()`时，将从用户档案的批量投影中读取区段成员资格。 此投影中的数据在摄取后&#x200B;**2小时**&#x200B;内刷新。 有关传播计时方案的完整详细信息，请参阅[inAudience函数文档](functions/functioninaudience.md#propagation-timing)。
...
有关读取受众护栏（包括重试和吞吐量限制）的完整列表，请参阅[护栏和限制](../start/guardrails.md#read-segment-g)。
...
* **TL；DR：**&#x200B;本页介绍如何在Adobe Journey Optimizer中配置和使用读取受众活动，以一次或按重复计划将Adobe Experience Platform受众中的配置文件添加到历程中，并提供有关计划、吞吐量、故障排除和最佳实践的指导。

**意图：**
* 将读取受众活动配置为历程的入口点
* 选择历程的Adobe Experience Platform受众和身份命名空间
* 设置读取率以控制每秒输入的配置文件数
* 安排历程运行一次、每天、每周或按自定义周期运行
* 启用增量读取以在定期运行时仅处理新的受众成员
* 解决受众规模不匹配、零配置文件运行和条目延迟的问题
* 根据批处理与实时需求，在读取受众和受众资格之间做出选择

**术语表：**
* **读取受众活动**：从选定的Adobe Experience Platform受众中读取所有符合条件的用户档案并将其添加到历程&#x200B;*（产品特定）*&#x200B;的历程入口点活动
* **读取率**：每秒可以进入历程的最大配置文件数（500-20,000；默认为5,000） *（产品特定）*
* **增量读取**：周期性历程选项，仅处理自上次历程执行&#x200B;*（产品特定）*&#x200B;以来新添加到受众的用户档案
* **在重复时强制重新进入**：计划选项，用于在每次新运行之前删除所有活动的历程参与者，以便配置文件可以重新输入新的&#x200B;*（产品特定）*
* **在批次受众评估后触发**：一个计划选项，它将历程的执行延迟到新的批次受众快照可用时为止（最多6个小时）*（产品特定）*
* **补充标识符**：辅助标识符（例如，订单ID），当标识符不同时&#x200B;*（产品特定）*，允许同一配置文件多次进入历程

**护栏：**
* 每个历程只允许一个读取受众活动，且必须是第一个活动。
* 每个读取受众活动只能选择一个受众。
* 每个组织最多运行五个并发读取受众。
* 最大读取速率为每个沙盒每秒20,000个配置文件（所有并发读取受众活动的总和）。
* 使用补充标识符时，读取率限制为每秒500个用户档案。
* 只有状态为“已实现受众参与”的用户档案才会进入历程。
* 只有基于人员的身份命名空间可用；没有选定命名空间的配置文件无法进入。
* 12小时作业超时适用于读取受众导出作业。
* 每10分钟重试一次失败的导出作业，重试时间最长为1小时。
* 对于启用了增量读取的自定义上传受众，仅在第一次重复时检索用户档案（这些受众是固定的）。
* 缩放入选者不适用于读取受众历程（路径实验）。

**术语：**
* 规范名称：读取受众 — 缩写：none — 变体：区段触发器、基于受众的历程条目、读取区段（旧版API名称）
* 同义词：“读取受众”=“区段触发器”=“受众触发的历程”
* 请勿混淆：“读取受众”≠“受众资格”（读取受众是批量/计划的；受众资格是实时流式传输的）

**常见问题解答：**
* **问：何时应使用“读取受众”而非“受众资格”？**  — 将读取受众用于批处理、计划用例（例如，每周新闻稿、重新参与活动）。 当用户档案必须实时进入历程且符合条件时，请使用受众资格。
* **问：为什么进入历程的用户档案少于受众规模？**  — 常见原因包括：配置文件没有选定的命名空间、在历程运行之前尚未完成批量分段作业，或者配置文件未处于“已实现”状态。 启用“批量受众评估后触发”并检查命名空间配置。
* **问：增量读取在第一次运行时做什么？**  — 在首次执行时，所有受众配置文件都会进入。 在后续运行中，仅处理自上次执行以来新添加到受众的用户档案。
* **问：如果导出作业失败，会发生什么情况？**  — 系统每10分钟重试一次，最长可重试1小时。 故障会在“警报”中报告。 1小时未成功后，该运行会被视为失败。
* **问：同一配置文件能否多次进入读取受众历程？**  — 是，如果配置了补充标识符且条目之间不同，或者启用了重复时强制重入。 如果没有这些变量，配置文件不能同时出现多次。
* **问：一次性读取受众历程保持多长时间？**  — 当最后一个用户档案退出时，它会自动停止到“已停止”，除非历程包括等待、反应或事件触发的过渡，在这种情况下，将应用91天全局超时。 默认情况下，在91天的“已完成”后才会保持活动状态。
