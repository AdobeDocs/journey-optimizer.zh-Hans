...
1. 在&#x200B;**[!UICONTROL 优化量度]**&#x200B;中，默认[!DNL Customer Journey Analytics] [数据视图](https://experienceleague.adobe.com/zh-hans/docs/analytics-platform/using/cja-dataviews/data-views){target="_blank"}中的所有量度都显示在列表中。 选择要优化模型的量度。

   ![包含名称和描述字段的AI模型详细信息窗格](assets/journey-model-metrics.png){width="80%"}
...
## 为排名公式选择AI模型 {#select-ai-model-for-ranking-formula}

您现在可以将AI模型设置为引用，以构建排名公式。 请按照以下步骤操作。

1. 创建排名公式。 [了解如何操作](journey-ranking-formulas.md#create-journey-ranking-formula)

1. 使用&#x200B;**[!UICONTROL 选择AI模型]**&#x200B;按钮选择要使用的AI模型。

   ![历程排名公式详细信息窗格，包含AI模型选择](assets/journey-formula-ai-model.png){width="80%"}
...
要使用AI模型为您的历程排名，您需要将引用此AI模型的公式分配给规则集。
