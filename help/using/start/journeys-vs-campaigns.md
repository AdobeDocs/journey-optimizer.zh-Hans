...
| 功能 | 历程 | 操作营销活动 | API触发的营销活动 | 编排的营销活动 |
|---------|----------|------------------|------------------------|----------------------|
...
| **分段** | 预建受众+实时条件 | Experience Platform中的预建受众 | 有效负载驱动的定位（无计划受众） | 在画布中构建的按需受众具有确切计数 |
| **配置文件正在处理** | 单个实时（当事件发生时） | 批次，一次完成 | 每个API调用，有效负载驱动 | 批处理，以及多实体支持 |
| **Personalization** | 实时上下文数据+配置文件属性 | 配置文件属性 | 有效负载数据+可选配置文件属性 | 用于精确定位的多实体数据 |
...
| 渠道 | 历程 | 操作营销活动 | API触发的营销活动 | 编排的营销活动 |
|---------|:--------:|:----------------:|:-----------------------:|:---------------------:|
| 电子邮件 | ✅ | ✅ | ✅ | ✅ |
| 推送 | ✅ | ✅ | ✅ | ✅ |
| 短信 | ✅ | ✅ | ✅ | ✅ |
...
是的。 在[!DNL Adobe Experience Platform]中创建的受众可用于历程、操作营销活动和编排的营销活动（还可以在画布中按需构建受众逻辑）。 API触发的营销活动是有效负载驱动的，不使用相同方式的预建受众。
