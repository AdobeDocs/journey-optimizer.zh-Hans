...
* 对于所有有效的[历程版本](../building-journeys/publish-journey.md#create-a-new-version-of-a-journey-journey-create-new-version)，一个轮廓不能在同一历程中同时多次出现。如果启用了重新进入，则用户档案可以重新进入历程，但只有在完全退出该历程的上一个实例后才能重新进入历程。[了解详情](../building-journeys/end-journey.md)
