...
## 关键限制概览 {#quick-reference}

在构建或发布之前，请使用此表查找最重要的数字限制。 以下各节将介绍全部详细信息和相关内容。

| 面积 | 限制 | 值 |
|---|---|---|
| **历程** | 每个历程最多[个活动](#journeys-guardrails-journeys) | **50** |
| **历程** | [最大实时/暂停/试运行旅程](#journeys-guardrails-journeys) | **100** |
| **历程** | [历程实例大小](#journeys-guardrails-journeys) | **1 MB** |
| **历程** | [有效负载大小（发布）](#journey-payload-size)历程 | **2 MB** （90%时警告） |
| **历程** | [全局历程超时](#journeys-guardrails-journeys) | **91天** |
| **历程** | [每个配置文件的挂起事件队列](#journeys-guardrails-journeys) | **10个事件** |
| **历程** | [并发读取受众实例](#read-segment-g) | 所有沙盒中的&#x200B;**5** |
| **历程** | [读取受众沙盒吞吐量](#read-segment-g) | **20,000个配置文件/秒**（共享） |
| **历程** | [读取受众作业超时](#read-segment-g) | **12小时** |
| **渠道** | [每秒入站请求数](#inbound-guardrails) | **5,000 RPS** |
| **渠道** | [最大活动入站操作数](#inbound-guardrails) | **500** |
| **渠道** | [事务性消息/秒（营销活动）](#transactional-message-guardrails) | **500** |
| **渠道** | [每秒入站历程事件](#events-g) | **5,000** |
| **自定义操作** | 每分钟[次呼叫（响应&lt; 0.75秒）](#custom-actions-g) | **300,000/分钟**&#x200B;每个主机/沙盒 |
| **自定义操作** | 每30秒[次呼叫（响应> 0.75秒）](#custom-actions-g) | **150,000 / 30 s**&#x200B;每个主机/沙盒 |
| **内容** | [电子邮件发布内容](#message-content-size) | **2 MB** （作者在1 MB以下） |
| **内容** | [应用程序内消息内容](#in-app-activity-limitations) | **2 MB** |
| **内容** | [可视片段大小](#fragments-guardrails) | **100 KB** |
| **内容** | [表达式片段大小](#fragments-guardrails) | **200 KB** |
| **内容** | [历程片段节点](#fragments-journey-g) | **20个节点/片段**，200个活动/沙盒 |
| **受众** | 每个沙盒的[受众合成](#audience) | **10** |
| **数据集** | [配置文件存储TTL（新组织/沙盒）](#datasets-guardrails) | **90天** |
| **数据集** | [数据湖TTL（新组织/沙盒）](#datasets-guardrails) | **13个月** |
...
* 默认情况下，同一时间的实时/暂停/练习历程数限制为&#x200B;**100**。 当前历程数显示在历程画布上方。
...
>[!TIP]
>
>**这对您意味着什么：** **50活动限制**&#x200B;和&#x200B;**100实时历程限制**&#x200B;是大多数团队在缩放时首先遇到的两个护栏。 提前规划历程拆分，并将读取受众开始时间至少相隔5-10分钟以避免沙盒吞吐量竞争。
...
| 护栏 | 值 | 注释 |
|---|---|---|
| 上限限制 — 响应&lt; 0.75秒 | 每个主机和每个沙盒&#x200B;**300,000次调用/分钟** | 滑动窗口。 每个沙盒和每个端点强制执行。 |
| 上限限制 — 响应> 0.75秒 | **150,000次调用/每个主机和每个沙盒30次** | 滑动窗口。 当终结点延迟超过0.75秒时应用的单独限制。 |
| URL类型 | 仅限静态 | 不支持自定义操作URL中的动态参数。 |
| 支持的HTTP方法 | POST， PUT， GET | 不支持其他方法。 |
| 查询参数/标题名称格式 | 不得以`.`或`$`开头 | 以这些字符开头的名称将被拒绝。 |
| URL中的IP地址 | 不允许 | 请使用主机名而不是IP地址。 |
| Adobe内部地址 | 不允许 | URL和API中不允许使用`.adobe.*`地址。 |
| 内置自定义操作 | 无法删除 | 内置自定义操作是永久性的。 |
| 有效负载格式 | 仅限JSON | 请求和响应负载均需要JSON格式。 请参阅[此页](../action/about-custom-action-configuration.md#custom-actions-limitations)。 |
| 最小端点吞吐量 | 200 TPS | 自定义操作所定向的任何端点都必须支持至少200个TPS。 |

>[!TIP]
>
>**这对您意味着什么：**&#x200B;默认的300,000次调用/分钟上限可保护您的外部端点不被历程吞吐量淹没。 如果您的端点可以处理更多负载，则可以使用[上限API](../configuration/capping.md)或[限制API](../configuration/throttling.md)提高此限制。 如果您需要更高的组织限制，请联系您的Adobe代表。

此限制是根据客户使用情况设置的，用于保护自定义操作所针对的外部端点。 如果需要，您可以通过我们的“上限/限制 API”定义较大的上限或限制来覆盖此设置。 请参阅[此页](../configuration/external-systems.md)。
...
| 护栏 | 值 | 注释 |
|---|---|---|
| 并发实例（所有沙盒+历程） | **5** | 避免计划超过5个读受众同时开始的历程；将它们分散开5-10分钟。 |
| 沙盒吞吐量 | **20,000个配置文件/秒**（共享） | 在沙盒中的所有读取受众活动中共享。 如果达到限制，作业可能会排入队列。 |
| 每个活动可配置的吞吐量 | **500-20,000个配置文件/秒** | 在沙盒共享限制内为每个活动配置。 |
| 作业处理超时 | **12小时** | 无法在12小时内处理的作业将自动清除，并且不会执行。 |
| 重试窗口 | 最多&#x200B;**1小时**（10分钟间隔） | 在检索导出作业时应用重试。 历程最多可在计划时间后1小时执行。 |
| 补充ID读取率 | 每个历程实例每秒&#x200B;**500个配置文件** | 在使用补充ID时适用。 |
| 每个历程读取受众实例 | **1** | 历程只能有一个读取受众活动。 |
| 受众类型 | 流式处理受众始终保持最新 | 每天在每日批量评估时间仅评估一次批量受众。 |
| 配置文件属性刷新 | 已在&#x200B;**等待**&#x200B;活动中刷新 | 在历程条目中，配置文件使用批次快照值。 当配置文件达到等待活动时，属性会刷新。 |
| 在历程中的定位 | 第一个活动或业务事件后 | 读取受众活动只能用作历程中的第一个活动，或用作业务事件活动之后。 |
| 与Adobe Campaign一起使用 | 不受支持 | 读取受众活动不能与Adobe Campaign活动一起使用。 |
| 多个受众 | 不直接支持 | 读取受众活动在每个历程中只能定位一个受众。 要使用多个受众，请先将其合并。 [了解如何操作](../audience/get-started-audience-orchestration.md) |
