...
| 面积 | 限制 | 值 | 类型 |
|---|---|---|---|
| **历程** | 每个历程的最大活动数 | **50** | 硬限制 |
| **历程** | 实时/暂停/模拟历程的最大值 | **100** | 软限制（可扩展） |
| **历程** | 历程实例大小 | **1 MB** | 硬限制 |
| **历程** | 历程的有效负载大小（发布） | **2 MB** （90%时警告） | 硬限制 |
| **历程** | 全局历程超时 | **91天** | 硬限制 |
| **历程** | 每个配置文件的挂起事件队列 | **10个事件** | 硬限制 |
| **历程** | 并发读取受众实例 | 所有沙盒中的&#x200B;**5** | 硬限制 |
| **历程** | 读取受众沙盒吞吐量 | **20,000个配置文件/秒**（共享） | 硬限制 |
| **历程** | 读取受众作业超时 | **12小时** | 硬限制 |
| **渠道** | 每秒入站请求数 | **5,000 RPS** | 硬限制 |
| **渠道** | 最大活动入站操作数 | **500** | 硬限制 |
| **渠道** | 事务性消息数/秒（营销活动） | **500** | 硬限制 |
| **渠道** | 每秒入站历程事件 | **5,000** | 硬限制 |
| **自定义操作** | 每分钟呼叫数（响应小于0.75秒） | **300,000/分钟**&#x200B;每个主机/沙盒 | 硬限制（可配置） |
| **自定义操作** | 每30秒调用数（响应> 0.75秒） | **150,000 / 30 s**&#x200B;每个主机/沙盒 | 硬限制（可配置） |
| **内容** | 发布时的电子邮件内容 | **2 MB** （作者在1 MB以下） | 硬限制 |
| **内容** | 应用程序内消息内容 | **2 MB** | 硬限制 |
| **内容** | 可视片段大小 | **100 KB** | 硬限制 |
| **内容** | 表达式片段大小 | **200 KB** | 硬限制 |
| **内容** | 历程片段节点 | **20个节点/片段**，200个活动/沙盒 | 硬限制 |
| **受众** | 每个沙盒的受众组合 | **10** | 硬限制 |
| **数据集** | 配置文件存储TTL（新组织/沙盒） | **90天** | 硬限制 |
| **数据集** | 数据湖TTL（新组织/沙盒） | **13个月** | 硬限制 |

>[!NOTE]
>
>可以通过联系您的Adobe代表或Adobe客户关怀团队来提高标记为&#x200B;**硬限制（可配置）**&#x200B;的限制。
