...
* 对于每个用户档案和历程版本，历程运行时在处理一个挂起事件时保持内部队列最多10个。 如果达到此限制，则会以`maxInstanceStackEventsReached`原因丢弃其他事件，直到栈栈耗尽为止。 查看由于受阻的历程实例[而丢弃的](../building-journeys/troubleshooting-execution.md#max-instance-stack-events-reached)个事件。
