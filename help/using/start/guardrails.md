...
* Journey Optimizer要求将事件流式传输到数据收集核心服务(DCCS)才能触发历程。 批量摄取的事件或来自内部Journey Optimizer数据集（消息反馈、电子邮件跟踪等）的事件 无法用于触发历程。 对于无法获取流式处理事件的用例，请根据这些事件构建一个区段，然后使用 **读取区段** 活动。 从技术上讲，区段鉴别可以使用，但可能会根据使用的操作在下游造成挑战。
