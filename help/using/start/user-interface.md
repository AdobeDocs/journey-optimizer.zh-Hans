...
* **环境详细信息** — 沙盒名称、组织ID和产品区域（例如，历程、营销活动、决策）
* **影响级别** — 问题的严重性（例如，生产受阻、功能受限、轻微）
* **复制步骤** — 清晰、分步说明以重现问题
...
>title="处理过的数据"
>abstract="在“已处理数据”下，选择如何从早期运行中排除记录。 使用先前执行的“排除结果”，活动会保留先前运行中返回的记录列表；每次运行都将排除这些记录，并仅返回新记录。 通过使用日期字段选项，活动使用选定的日期字段而不是跟踪单个ID，并且每次运行只返回日期在最后一次执行之后的行。"
