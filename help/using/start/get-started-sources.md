...
>[!NOTE]
>
>**协调营销活动的数据摄取** — 对于与协调营销活动一起使用的基于文件的变更数据捕获源，`_change_request_type`字段是必需的。 支持的值为`u` （更新插入）或`d` （删除）。 这些值必须为小写`u`和`d`，而不是大写`U`和`D`。 [了解关于协调的活动护栏和限制的更多信息](../orchestrated/guardrails.md)
