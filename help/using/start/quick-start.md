...
Journey Optimizer提供两种主要方法来吸引和吸引客户：

* **历程** — 实时、一对一的编排，其中每个客户均按照自己的进度移动，由行为或事件触发。 最适合载入顺序、购物车放弃和生命周期参与。
* **营销活动** — 基于受众的消息传递，根据您的用例有三种传递模式：
   * **操作营销活动** — 一次性发送给定义受众的计划或定期消息。 最适合于新闻稿、促销公告和产品发布。
   * **API触发的营销活动** — 由外部系统通过API触发的按需消息。 最适合用于订单确认、装运警报和帐户通知等事务性消息。
   * **协调的营销活动** — 具有多实体分段和基于画布执行的复杂批处理工作流。 最适合季节性促销、多步骤批处理项目和需要准确预发送计数的营销活动。
...
成功的实施从准备开始。 在配置Journey Optimizer之前，请根据以下要求调整您的团队：

* **首先定义您的用例** — 确定您将处理的客户方案并确定其优先级。 这会指导从[数据管理](../data/gs-data.md)到[渠道设置](../configuration/get-started-configuration.md)的每个配置决策。
* **涉及所有接触客户体验的团队** — Journey Optimizer实施通常跨越营销、IT、数据和操作。 团队间的早期对齐可防止返工。
* **建立共享的客户标识符** — 商定所有数据源中存在的通用标识符（如CRM ID或电子邮件地址）。 这是[统一客户配置文件](../audience/get-started-profiles.md)的基础。
* **验证数据隐私合规性** — 确保您计划连接的所有数据源在引入之前都遵守适用的[隐私法规](../privacy/get-started-privacy.md)。
* **在上线之前规划测试** — 验证[事件触发器、历程条件和渠道操作](../building-journeys/journey-gs.md)在开发或暂存沙盒中是否按预期运行。
* **准备您的品牌内容和资源库** — 识别您的团队将在历程和营销活动中使用的数字资源、模板和品牌准则。 在Launch启动之前，将它们加载到Journey Optimizer的[内置资源库](../integrations/assets.md)中，可加快消息创建速度并确保从第一天起就保持品牌一致性。
