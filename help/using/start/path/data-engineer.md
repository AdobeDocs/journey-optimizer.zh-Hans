...
**在此页面上：**&#x200B;构建支持 Adobe Journey Optimizer 的架构、数据集、标识和数据源，以便您的团队能够提供实时、个性化的客户体验。
...
>[!TIP]
>
>首次接触 Journey Optimizer 中的数据？ 从[数据管理快速入门](../../data/gs-data.md)概述开始，在深入研究配置之前了解架构、数据集、身份标识、轮廓片段模型和完整的数据就绪性清单。
...
1. **消息导出数据集**（可选）。 在渠道配置级别启用消息导出后，发送的电子邮件和短信内容会自动导出到专用的 Experience Platform 数据集，以便进行合规性、存档或下游分析。 了解[消息导出](../../configuration/message-export.md)。
