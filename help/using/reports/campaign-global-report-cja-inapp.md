...
**在此页面上：**&#x200B;了解如何在Adobe Journey Optimizer中阅读应用程序内促销活动报告，以分析应用程序内消息的显示和点击趋势、跟踪数据和跟踪链接。
...
* **[!UICONTROL 发送]**：应用程序请求应用程序内促销活动的次数。 如果未缓存营销活动数据，则每个用户会话产生的多个请求（例如，启动或重新加载时）可能会导致此值超过独特用户计数。

* **[!UICONTROL 入站触发]**：应用程序考虑显示应用程序内消息的次数。 如果应用程序端规则阻止消息显示，则此数字可能低于发送总数。
