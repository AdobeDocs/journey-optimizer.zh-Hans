...
**在此页面上：**&#x200B;了解如何阅读历程报告中的电子邮件量度，包括投放与点击趋势、投放状态、发送和跟踪统计数据、电子邮件域、跟踪链接、主题以及退信和排除原因。
...
## 发送统计数据 {#email-sending-statistics}
...
* **[!UICONTROL 预计 CTR]**（点进率）：计算为预计点击次数与已投放消息总数的比率。

* **[!UICONTROL 预计 CTOR]**（点击打开率）：计算为预计点击次数与预计打开总数的比率。
