...
历程步骤事件是自动生成的事件，用于捕获用户档案在Adobe Journey Optimizer中完成历程时执行的每个步骤的详细信息。 这些事件提供了历程性能的全面可见性，并支持强大的分析功能。

## 什么是历程步骤事件？ {#what-are-step-events}

历程步骤事件是系统生成的XDM(Experience Data Model)事件，每当配置文件在旅程中从一个节点移动到另一个节点时，Adobe Journey Optimizer会自动创建这些事件并将其发送到Adobe Experience Platform。 每个事件对应于客户历程体验中的特定操作或过渡。
...
>[!NOTE]
>
>默认情况下，将在所有实例上激活历程步骤事件。 您无法修改或更新在预配步骤事件期间创建的架构和数据集。 这些架构和数据集处于只读模式。
...
- **历程性能跟踪**：实时监视用户档案如何在您的历程中流动
- **转化分析**：了解流失点和成功的转化路径
- **错误检测**：发现问题并排除其故障

### 数据集成和见解 {#data-integration}

- **跨平台分析**：将历程数据与其他Adobe Experience Platform数据源相结合
- **Customer 360视图**：创建包含历程交互的综合客户配置文件
- **归因建模**：将历程接触点与下游业务结果连接起来

### 优化机会 {#optimization}

- **A/B测试见解**：分析不同历程路径的性能
- **Personalization增强功能**：使用历程行为数据来改进未来的体验
- **运营效率**：识别瓶颈并优化历程设计
...
1. **Data Lake查询**：使用SQL查询`journey_step_events`数据集
2. **Customer Journey Analytics**：通过高级分析工具分析旅程数据
3. **实时报表**：通过Journey Optimizer的内置报表功能访问数据
...
- **历程标识**：历程ID、版本和名称
- **配置文件信息**：配置文件ID和关联的身份
- **步骤详细信息**：节点名称、步骤类型和执行状态
- **时间戳**：每个历程步骤的精确计时
- **操作结果**：成功/失败状态和执行详细信息
- **错误信息**：出现问题时提供详细的错误代码和描述
...
- 请使用`journeyVersionID`而不是`journeyVersionName`以获得更好的查询性能
- 按日期范围过滤，提高大型数据集的查询速度
- 利用与您的历程命名空间配置匹配的配置文件身份
...
可以使用Customer Journey Analytics分析历程步骤事件，用于：
...
- **[内置架构词典](https://experienceleague.adobe.com/tools/ajo-schemas/schema-dictionary.html?lang=zh-Hans)**：完整XDM架构引用
...
历程步骤事件构成了Adobe Journey Optimizer中advanced journey analytics的基础。 通过有效地了解和利用这些事件，您可以深入了解客户行为、优化历程性能并为客户创建更加个性化的体验。
