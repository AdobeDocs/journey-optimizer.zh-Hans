...
title: 使用拆分活动
description: 了解如何在多步营销活动中使用拆分活动
hide: true
hidefromtoc: true
...
>title="拆分活动的区段"
>abstract="添加所需数量的子集以划分传入的群体。<br/></br>执行&#x200B;**拆分**&#x200B;活动时，将群体按将其添加到活动的顺序划分为不同的子集。在开始多步骤营销活动之前，请确保您已使用箭头按钮按照符合您需求的顺序为子集排序。"
...
1. 活动配置窗格随即打开，其中包含默认子集。单击&#x200B;**添加区段**&#x200B;按钮以添加所需数量的子集，对传入群体进行分段。
   ...
   >
   >在开始多步骤营销活动之前，请确保已按照适合您需求的顺序订购了子集。 要实现此目的，请使用箭头按钮更改子集的位置。

1. 添加子集后，活动将显示与子集一样多的输出转换。我们强烈建议更改每个子集的标签，以便在多步骤营销活动画布中轻松识别它们。
...
1. 配置每个子集筛选传入群体的方式。为此，请执行以下步骤：
   ...
   1. 要在传入群体为空时禁用过渡，请打开&#x200B;**[!UICONTROL 跳过空过渡]**&#x200B;选项。 如果没有与子集匹配的用户档案，则多步营销活动将不会转换为下一个活动。
...
现已配置该活动。在多步骤营销活动执行中，群体将按照其添加到活动的顺序划分为不同的子集。
