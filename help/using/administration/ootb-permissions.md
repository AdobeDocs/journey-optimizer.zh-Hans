...
| 数据卫生 | **[!DNL View data hygiene]**：数据卫生的只读访问。</br>**[!DNL Manage data hygiene]**：读取、创建、编辑和删除数据保健。 |
...
| 目标 | **[!DNL Manage destinations]**：读取、创建和删除目标激活流和目标帐户。</br>**[!DNL View destinations]**：对“目录”选项卡中的可用目标和“浏览”选项卡中的经过身份验证的目标进行只读访问。</br>**[!DNL Activate destinations]**：允许用户将区段激活到现有目标。</br>**[!DNL Activate segment without mapping]**：允许用户将区段激活到现有目标，而不显示映射步骤。 用户可以在激活工作流程中添加和删除区段，但无法添加或删除映射的属性或身份。</br>**[!DNL Manage and activate dataset destination]**：读取、创建、编辑和禁用数据集导出流程。 还能将数据激活到已创建的活动数据集。</br>**[!DNL Destination authoring]**：使用Adobe Experience Platform Destination SDK的创作目标。 |
...
| 沙盒管理 | **[!DNL Manage sandboxes]**：读取、创建、编辑和删除沙箱。</br>**[!DNL View sandboxes]**：对属于您组织的沙盒的只读访问权限。</br>**[!DNL Reset sandboxes]**：能够重置沙盒。</br>**[!DNL Export sandboxes]**：导出沙盒的功能。 |
