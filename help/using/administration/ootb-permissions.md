...
| 目标 | **[!DNL Manage destinations]**：读取、创建和删除目标激活流和目标帐户。</br>**[!DNL View destinations]**：对“目录”选项卡中的可用目标和“浏览”选项卡中的已验证目标的只读访问。</br>**[!DNL Activate destinations]**：允许用户将区段激活到现有目标。</br>**[!DNL Activate segment without mapping]**：允许用户将区段激活到现有目标，而不显示映射步骤。 用户可以在激活工作流中添加和删除区段，但无法添加或删除映射的属性或标识。</br>**[!DNL Manage and activate dataset destination]**：读取、创建、编辑和禁用数据集导出流。 能够将数据激活到已创建的活动数据集。</br>**[!DNL Destination authoring]**：使用Adobe Experience Platform Destination SDK创作目标。 |
