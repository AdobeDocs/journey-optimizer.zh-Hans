...
title: 短信配置
description: 了解如何配置环境以通过Journey Optimizer发送短信
role: Admin
level: Intermediate
...
发送短信之前，请配置您的实例。 您需要 [集成提供程序设置](#create-api) 和 [创建短信界面](#message-preset-sms) （即短信预设）。 这些步骤必须由 [Adobe Journey Optimizer系统管理员](../start/path/administrator.md).

>[!IMPORTANT]
>
>Adobe Journey Optimizer当前与第三方提供商（如Sinch和Twilio）集成，这些提供商提供的短信服务独立于Adobe Journey Optimizer。  在短信配置之前，您必须与这些短信提供商之一创建一个帐户，以接收API令牌和服务ID，以便您能够在Adobe Journey Optimizer与适用的短信提供商之间建立连接。 您使用短信服务将受适用短信提供商的其他条款和条件的约束。 由于Sinch和Twilio是Adobe Journey Optimizer用户可通过集成获得的第三方产品，因此对于与短信服务相关的任何问题或查询，Sinch或Twilio的用户将需要联系适用的短信提供商以获取帮助。 Adobe不控制第三方产品，也不负责第三方产品。

## 创建新API凭据 {#create-api}

>[!CONTEXTUALHELP]
>id="ajo_admin_sms_api_header"
>title="使用Journey Optimizer配置您的短信供应商"
>abstract="选择您的供应商并填写您的短信API凭据。"

>[!CONTEXTUALHELP]
>id="ajo_admin_sms_api"
>title="使用Journey Optimizer配置您的短信供应商"
>abstract="在发送短信之前，您必须将提供程序设置与Journey Optimizer相集成。 完成后，您需要创建短信界面。 这些步骤必须由Adobe Journey Optimizer系统管理员执行。"
...
1. 选择 **[!UICONTROL SMS vendor]**:
   ...
   * [!DNL Sinch]. 查找 **[!UICONTROL Service ID]** 和 **[!UICONTROL API Token]**，从您的Sinch帐户访问短信> API菜单。
   * [!DNL Twilio]. 查找 **[!UICONTROL Service ID]** 和 **[!UICONTROL API Token]**，访问控制台功能板页面的帐户信息窗格。
...
现在，您已准备好通过Journey Optimizer发送短信消息。
