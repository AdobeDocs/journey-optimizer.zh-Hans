...
>title="委派登陆页面子域"
>abstract="您将设置子域以供登陆页面使用。 您可以使用已委派给Adobe的子域，也可以配置其他子域。"

>[!CONTEXTUALHELP]
>id="ajo_admin_subdomain_lp"
>title="委派登陆页面子域"
>abstract="您必须配置子域以用于登陆页面，因为您需要此子域才能创建登陆页面预设。 您可以使用已委派给Adobe的子域，也可以配置新的子域。"
...
您可以使用已委派给Adobe的子域，也可以配置其他子域。 了解有关将子域委派到Adobe的更多信息，请参阅 [此部分](../configuration/delegate-subdomain.md).
...
1. 从列表中选择委派的子域。
   ...
   请注意，不能使用同一父域的多个委派子域。 例如，如果已将“marketing1.yourcompany.com”委派给Adobe用于您的登陆页面，则您将无法使用“marketing2.yourcompany.com”。 但是，登陆页面支持多级子域，您可以继续使用“marketing1.yourcompany.com”的子域（如“email.marketing1.yourcompany.com”），或使用其他父域。
...
1. 提交后，子域将显示在列表中，其中 **[!UICONTROL Processing]** 状态。 有关子域状态的更多信息，请参阅 [此部分](../configuration/about-subdomain-delegation.md#access-delegated-subdomains).<!--Same statuses?-->

   ![](assets/lp_subdomain-processing.png)

   >[!NOTE]
   >
   >要使用该子域发送消息，您必须等到Adobe执行所需的检查（最长可能需要4小时）。<!--Learn more in [this section](delegate-subdomain.md#subdomain-validation).-->
...
1. 提交子域委派后，子域将显示在列表中，其中 **[!UICONTROL Processing]** 状态。 有关子域状态的更多信息，请参阅 [此部分](../configuration/about-subdomain-delegation.md#access-delegated-subdomains).<!--Same statuses?-->

   >[!NOTE]
   >
   >要使用该子域发送消息，您必须等到Adobe执行所需的检查（最长可能需要4小时）。<!--Learn more in [this section](#subdomain-validation).-->
