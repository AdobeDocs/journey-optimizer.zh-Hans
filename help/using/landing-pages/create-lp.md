...
>[!NOTE]
>
>在创建您的第一个登陆页面之前，请完成[配置登陆页面](../../rp_landing_pages/lp-configuration-landing-page.md)设置。 这包括配置子域以托管登陆页面，并定义至少一个指定子域和其他渠道设置的预设。 创建登陆页面时，您将需要选择预设。
...
>id="ajo_lp_create"
>title="定义和配置登陆页面"
>abstract="要创建登陆页面，您需要选择一个预设，然后配置主页面和子页面，最后在发布页面之前对其进行测试。"
...
1. 选择要创建的登陆页面的类型。
   ...
   * **[!UICONTROL 订阅/退订]**：使用此类型为订阅或退订用例创建登陆页面。 [了解详情](lp-use-cases.md#subscription-to-a-service)
   * **[!UICONTROL 数据捕获]**：使用此类型为数据捕获用例创建登陆页面。 [了解详情](get-started-lp.md#data-capture-lp)
...
>[!CAUTION]
>
>* 要发布登陆页面，您必须具有&#x200B;**[!UICONTROL 发布消息]**&#x200B;权限。
>* 发布之前，请检查并解决所有警报。 [了解详情](#check-alerts)
