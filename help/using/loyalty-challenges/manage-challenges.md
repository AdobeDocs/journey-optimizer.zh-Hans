...
* **已计划**：已发布质询，质询将在指定的开始日期自动生效
* **实时**：挑战当前处于活动状态，客户可以参加
* **已完成**：挑战已结束 — 已超过结束日期或已实现所有目标
* **已停止**：挑战在达到其自然完成之前已手动停止
* **已存档**：质询已存档用于组织目的，并且不再显示在主清单中

### 编辑挑战 {#edit-challenges}

您可以根据挑战的当前状态来编辑挑战：

* **草稿挑战**：完全编辑功能 — 所有属性都可以修改
* **已计划/实时挑战**：编辑受限 — 您可以更新内容、发送消息和延长日期，但不能更改核心挑战结构（类型、受众或任务定义）
...
1. 根据质询状态进行更改：
   * **草稿挑战**：修改任何属性、任务、内容或消息
   * **已计划/实时挑战**：根据需要更新内容卡、消息传送或延长结束日期

1. 保存您的更改。对于计划的或实时挑战，更改将立即生效或根据您的更新计划生效。

>[!NOTE]
>
>对于需要重大修改的更改（如更改质询类型、受众或任务结构），请复制质询并创建新版本，而不是编辑现有版本。
...
1. 选择该挑战旁边的更多操作菜单（三个圆点）。

1. 选择&#x200B;**[!UICONTROL 复制]**。

1. 已创建挑战副本，并在其名称后附加“[副本]”。
