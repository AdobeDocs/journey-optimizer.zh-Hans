...
>[!NOTE]
>
>营销类型的电子邮件必须包含 [选择退出链接](../messages/consent.md#opt-out-management)，事务型消息不需要此参数。 消息类别（**[!UICONTROL Marketing]** 或 **[!UICONTROL Transactional]**）在[渠道平面](../configuration/channel-surfaces.md#email-type)（即消息预设）级别和[创建消息](../messages/get-started-content.md#create-new-message)时定义。
