...
* 选项过滤器
   ...
   **字段：** 过滤器
   **标题：** 选项过滤器
   **描述：** 对基于集合限定符（以前称为“标记”）的过滤器的引用，该过滤器使用其附加的集合限定符匹配库存中的选项。 该值是引用的决策规则的URI(@id)。 请参阅架构https://ns.adobe.com/experience/decisioning/filter 。
   **类型：**&#x200B;字符串
