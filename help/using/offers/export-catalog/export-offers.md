...
**字段：**特性
**标题：**决策选项特性
**描述：**特性是属于特定决策选项的优惠的附加特性或特性。 这些属性是键值对，这意味着它们包含具有关联值的属性名称（有时称为键），并用于区分一个决策选项和其他选件。 例如，对于属性名称“color”，特定选件的值可能为“green”。
特征会用作内容中的值，表示此决策选项，并用作分析和优化优惠性能的功能。 当每个实例具有相同的属性或属性时，该方面应被建模为从决策选项详细信息派生的扩展模式。
**类型：**&#x200B;对象
