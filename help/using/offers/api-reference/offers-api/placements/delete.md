...
有時可能需要移除(DELETE)位置。 只能刪除您在租使用者容器中建立的版位。 這是透過對「 」執行DELETE請求來完成 [!DNL Offer Library] API使用您要刪除之位置的例項ID。
...
您可以嘗試向位置查詢(GET)請求以確認刪除。 您需要在請求中加入Accept標頭，但應該會收到HTTP狀態404 （找不到），因為位置已從容器中移除。
