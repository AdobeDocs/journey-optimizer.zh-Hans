...
有时可能需要删除(DELETE)标记。 只能删除您在租户容器中创建的标记。 为此，请使用要删除的标记的$id对[!DNL Offer Library] API执行DELETE请求。
...
您可以通过尝试对标记进行查找(GET)请求来确认删除。 您需要在请求中包含一个“接受”标头，但应接收HTTP状态404（“未找到”），因为标记已从容器中删除。
