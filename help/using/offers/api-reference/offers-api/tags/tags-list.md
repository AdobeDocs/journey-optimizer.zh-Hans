...
收藏集限定符（以前称为“标记”）允许您更好地组织和排序选件。 例如，您可以使用“黑色星期五”集合限定符来标记“黑色星期五”选件。 然后，您可以使用选件库中的搜索功能，通过该收藏集限定符轻松找到所有选件。

收藏集限定符还可用于将选件分组到收藏集中。 有关更多信息，请参阅 [创建收藏集](../../../offer-library/creating-collections.md).
...
| `limit` | 限制返回的集合限定符数量。 | `limit=5` |

**响应**

成功的响应会返回一个集合限定符列表，这些限定符存在于您有权访问的容器中。
