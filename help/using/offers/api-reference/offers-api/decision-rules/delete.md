...
有時可能必須移除(DELETE)決定規則。 只能刪除您在租使用者容器中建立的決定規則。 這是透過對「 」執行DELETE請求來完成 [!DNL Offer Library] API使用您要刪除之決定規則的例項ID。
...
您可以嘗試向決定規則查詢(GET)請求以確認刪除。 您需要在請求中加入Accept標頭，但應該會收到HTTP狀態404 （找不到），因為決定規則已從容器中移除。
