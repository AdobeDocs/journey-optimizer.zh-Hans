...
有时可能有必要删除(DELETE)决策规则。 只能删除您在租户容器中创建的决策规则。 DELETE这是通过对 [!DNL Offer Library] API使用要删除的决策规则的实例ID。
...
您可以通过尝试向决策规则查找(GET)请求来确认删除。 您需要在请求中包含“接受”标头，但应会收到HTTP状态404（未找到），因为决策规则已从容器中删除。
