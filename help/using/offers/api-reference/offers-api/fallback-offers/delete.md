...
有时可能有必要删除(DELETE)备用选件。 只能删除您在租户容器中创建的后备优惠。 DELETE这是通过对 [!DNL Offer Library] API使用要删除的备用选件的$id。
...
您可以通过尝试对后备优惠进行查找(GET)请求来确认删除。 您需要在请求中包含“接受”标头，但应会收到HTTP状态404（未找到），因为已从容器中删除后备选件。
