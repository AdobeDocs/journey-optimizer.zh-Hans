...
您可以通过对容器执行单个GET请求，查看容器中所有个性化优惠的列表。 [!DNL Offer Library] API。
...
| 参数 | 描述 | 示例 |
| --------- | ----------- | ------- |
| `q` | 在选定字段中搜索的可选查询字符串。 查询字符串应为小写，并且可以用双引号括起来，以防止对其进行标记化并对特殊字符进行转义。 字符 `+ - = && || > < ! ( ) { } [ ] ^ \" ~ * ? : \ /` 具有特殊含义，在查询字符串中出现时应当使用反斜杠进行转义。 | `discounted offers` |
