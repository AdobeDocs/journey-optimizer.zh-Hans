...
* **动态收藏集** 是基于集合限定符的选件集合（以前称为“标记”）。 这些收藏集会自动更新。 例如，如果使用选定的收藏集限定符创建了新选件，则该选件将自动添加到收藏集。
...
1. 要创建动态收藏集，请使用左窗格选择要添加到收藏集的选件的收藏集限定符，然后单击 **[!UICONTROL 保存]**. 包含选定收藏集限定符的所有选件都将保存在收藏集中。

   有关创建集合限定符的更多信息，请参阅 [创建集合限定符](../offer-library/creating-tags.md).

   ![](../assets/dynamic_collection.png)

1. 要创建静态收藏集，请使用左窗格筛选选件列表（状态、收藏集限定符、日期、渠道、内容类型），然后选择要添加到收藏集的选件。
