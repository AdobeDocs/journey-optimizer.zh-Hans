...
>
>发布优惠后，您将无法更改为该频率选择的时间段（每月、每周或每日）。 如果选件具有 **[!UICONTROL 草稿]** 状态，之前从未发布并启用了频率封顶。
...
例如，我们将贵组织的中心区域视为 *NLD2*，并且您正在发送来自欧洲的决策请求(*IRL1* 边缘区域)。 在这种情况下，决策请求将递增用户档案的计数器，因为相关规则在（爱尔兰）中可用 *IRL1* 区域。 但是，如果决策请求来自日本等地区(*日文3*)，不是绑定到的边缘区域（荷兰） *NLD2* 在中心区域，不会创建计数器，也不会强制执行频率上限规则。

有关哪些中心地区和边缘地区与贵公司关联的更多信息，请联系您的Adobe代表。
