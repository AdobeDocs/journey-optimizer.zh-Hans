...
**在此页面上：**&#x200B;通过创建将标签链接到营销操作的治理策略，然后将它们应用于历程、营销活动和自定义操作，以便受限制的字段无法在外部共享，从而保护敏感数据。
...
创建治理策略后，您可以将营销操作应用于历程/营销活动和历程自定义操作。[了解如何在 Journey Optimizer 中应用营销操作](#apply-marketing-actions)

>[!IMPORTANT]
>
>DULE策略实施仅适用于&#x200B;**配置文件属性**。 DULE策略不支持基于事件的字段（上下文属性），例如历程事件字段。 应用于UI中事件字段的标签将没有任何效果，并且不会限制数据使用。
...
1. 在左侧菜单中的&#x200B;**数据管理**&#x200B;下，单击&#x200B;**架构**，然后单击&#x200B;**应用访问和数据治理标签**&#x200B;按钮。 选择您的架构和字段（例如“血型”），然后选择之前创建的标签，在我们的例子中为 _ePHI1_。
   ...
   >[!NOTE]
   >
   >标签只能应用于&#x200B;**配置文件属性字段**。 不支持基于事件的字段（例如历程上下文事件字段）：应用于这些字段的标签将不会限制数据使用。
...
1. 创建历程并添加自定义操作。 在历程中添加自定义操作时，有多个选项可用于对数据治理工作进行管理。 单击&#x200B;**显示只读字段**&#x200B;以显示所有参数。
   ...
   * 您可以定义&#x200B;**其他营销操作**&#x200B;以设置自定义操作类型。 这允许您定义此历程中自定义操作的用途。 除了必需的营销操作（通常特定于渠道）之外，您还可以定义附加营销操作，该操作将特定于此历程中的自定义操作。 例如：训练通信、新闻稿、健身通信等。必需的营销操作和附加营销操作都将适用。 在我们的示例中，我们不使用其他营销操作。
