...
实时活动提供对锁屏界面和Dynamic Island的持久实时更新，无需用户解锁其设备。 推送通知是临时警报，在关闭后便会消失。 实时活动将保持可见状态，并且可多次更新，直至明确结束。

+++

+++一次可以激活多少个实时活动实例？

iOS应用程序可以同时运行多个实时活动实例，包括使用相同`ActivityAttributes`类型的多个实例。

开发人员对给定属性类型的实时活动实例的数量没有硬性限制。 例如，您可以根据应用程序逻辑的要求启动任意数量的投放，每个正在进行的投放或历程启动一次。 但是，iOS对一次可以处于活动状态或可见的实时活动实例数量实施了系统级别限制。

实践中：

* iOS通常支持每个应用程序最多五个并发实时活动实例。

* 如果超过此数量，系统可能会停止显示某些活动实例，或终止旧活动实例以节约资源。

* 每个实时活动实例都有一个唯一的`Activity.id`，可让您单独更新或结束活动。
...
* iOS 18+：基于受众的直播活动支持广播频道
+++

+++实时活动可以保持活动状态多长时间？

Apple将实时活动限制为&#x200B;**8小时的活动更新**。 之后，系统会自动结束活动，不过在移除之前，它最多可以在静态状态下再显示&#x200B;**12小时**。 您也可以通过在应用程序中设置一个`dismissalDate`或显式调用`activity.end()`来更快地结束实时活动。

+++

+++ 什么是速率限制？

营销活动的默认速率限制为每秒500条事务性消息，跨所有渠道（包括iOS实时活动）进行传输。 此限制适用于合并的所有渠道，并且没有专门针对iOS Live活动的单独速率限制。
...
可以。 相同的属性类适用于本地启动和远程启动（推送启动）的实时活动。 您需要确保向`Messaging.registerLiveActivity()`注册它。

+++

+++如果我发送了不存在的实时活动的更新，会发生什么情况？

如果您发送不存在的`liveActivityID`或`channelID`的更新或结束事件，则请求将在设备上静默失败。 始终确保您跟踪每个用户处于活动状态的实时活动实例。
...
* **远程**：若要远程测试Live活动功能，请将我们的消息传送SDK集成到您的应用程序中，并使用提供的执行API发送远程启动、更新和结束到您的测试设备或iOS模拟器。 与当前通过Adobe SDK集成测试推送通知的方式类似。

+++

+++当应用程序处于后台时，如何处理更新？

SDK会自动处理此任务。 注册后，即使应用程序终止，实时活动也会接收更新。 不需要其他背景模式。
+++

+++`liveActivityID`和`channelID`之间有何区别？

* `liveActivityID`：用于针对特定用户的个人（单一）实时活动。 每个ID表示一个唯一的实时活动实例。
* `channelID`：用于向受众发送广播实时活动。 受众中的所有用户都会在同一渠道中接收相同的更新。
+++
...
* **支持频繁更新**：在应用程序的`Info.plist`中包含`NSSupportsLiveActivitiesFrequentUpdates`，并在需要频繁更新时将它设置为&#x200B;**是**。
...
* 您可以在请求中包含`x-request-id`标头。 理想情况下，每个`liveActivityID`与其对应的`x-request-id`之间应该有一对一的关系。 这样可以确保在使用相同的`x-request-id`和`liveActivityID`组合发出多个请求时，设备上将只启动一个实时活动实例，并将忽略重复请求。

* 如果忽略`x-request-id`标头，则会单独处理每个请求，这可能会导致使用同一`liveActivityID`创建多个实时活动实例。 在这种情况下，未来的更新可能会失败或只适用于其中一个活动实例。

* 不应在单独的API请求中跨不同的`liveActivityIDs`重复使用`x-request-id`值。
...
Use Unix epoch time in **seconds** not milliseconds. 例如：`1759937682`

+++

+++Can I use the same `requestId` for multiple API calls?

不是。 Each API request should have a unique `requestId` to ensure idempotency and proper tracking. Use UUIDs or similar unique identifiers.
...
不是。 You can use the same campaign and change the `event` field in the payload. However, some organizations prefer separate campaigns for better analytics tracking.
...
>[!TIP]
>
>有关全面的疑难解答指南，请参阅[实时活动疑难解答](troubleshoot-mobile-live.md)。
...
+++我可以为用户删除或清除所有实时活动实例吗？

您需要为每个活动的实时活动实例发送“结束”事件。 跟踪系统中每个用户处于活动状态的实时活动实例，以便正确清理它们。
