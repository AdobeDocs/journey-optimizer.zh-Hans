...
Unlike unitary events, business events are not linked to a specific profile. 事件ID类型始终基于规则。 请阅读 [此部分](../event/about-events.md).
...
* 测试业务事件时，必须传递事件参数以及将进入测试旅程的测试用户档案的标识符。 此外，在测试基于业务事件的历程时，您只能触发单个用户档案进入。 请参阅[此小节](../building-journeys/testing-the-journey.md#test-business)。In test mode, there is no &quot;Code view&quot; mode available.
...
Business events follow re-entrance rules in the same way as for unitary events. 如果历程允许重新进入，则将处理下一个业务事件。
...
1. 在 **[!UICONTROL Event ID condition]** 字段。 Using the simple expression editor, define the condition that will be used by the system to identify the events that will trigger your journey.
