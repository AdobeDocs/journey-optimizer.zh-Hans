...
1. 从Adobe Experience Platform API获取入口URL。 在 [流摄取API概述](https://experienceleague.adobe.com/docs/experience-platform/ingestion/streaming/overview.html){target=&quot;_blank&quot;}。
1. 从 **[!UICONTROL Event]** 菜单。 在 [本页](../event/about-creating.md#define-the-payload-fields).

然后，您需要配置数据系统，以使用您复制的有效负载将事件推送到流摄取API:

1. 为流摄取API URL设置POST API调用（称为入口）。
