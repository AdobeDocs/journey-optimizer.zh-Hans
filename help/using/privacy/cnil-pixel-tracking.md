...
CNIL提供了自建议之日起三个月的时间，要求公司通知其电子邮件收件人（“用户”）跟踪像素的存在、其目的以及用户选择退出的权利。 在此过渡期间，客户应通知用户像素跟踪信息，并在必要时提供选择退出选项。 **CNIL预计将在2026年7月14日之后开始执行活动。**
...
电子邮件类型在[渠道配置](../email/email-settings.md#email-type)级别设置。 在历程或营销活动中创作电子邮件时，作者必须选择电子邮件类型与通信性质匹配的渠道配置。 此分类会告知在投放之前应用同意检查。
...
要禁用特定电子邮件的打开跟踪，请在创建邮件时取消选中&#x200B;**[!UICONTROL 电子邮件打开次数]**&#x200B;选项。 禁用后，选项会阻止为该投放收集打开的跟踪数据。 对于范围内的组织，请在实施日期之前查看所有活动历程和营销活动的打开跟踪设置。
...
除了每条消息的打开跟踪切换之外，Adobe Journey Optimizer的Email Designer还精确控制要跟踪的URL。 使用Email Designer中的&#x200B;**[!UICONTROL 链接]**&#x200B;面板，作者可以查看邮件中的所有跟踪URL并单独设置每个链接的跟踪模式。
...
[了解如何在Email Designer中管理跟踪](../email/message-tracking.md#manage-tracking)

### 同意捕获和管理 {#consent-management}

Adobe Journey Optimizer通过Adobe Experience Platform (AEP) [同意和偏好设置架构](https://experienceleague.adobe.com/docs/experience-platform/xdm/field-groups/profile/consents.html?lang=zh-Hans){target="_blank"}处理同意。 同意首选项存储在用户档案级别，并在历程和活动执行期间自动实施。
...
[了解如何配置列表取消订阅](../email/list-unsubscribe.md)
...
[了解如何添加一键式选择退出链接](../email/email-opt-out.md#one-click-opt-out)
...
[了解如何管理客户的偏好设置](../action/preference-center.md)
...
[了解如何管理禁止列表](../configuration/manage-suppression-list.md)
