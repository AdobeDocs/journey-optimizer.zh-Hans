...
>additional-url="https://experienceleague.adobe.com/docs/journey-optimizer/using/privacy/consent.html?lang=en#one-click-opt-out" text="一键式选择退出"
...
根据行业标准和法规，所有短信营销消息都必须包含一种方式，以便收件人轻松取消订阅。 取消订阅后，用户档案将自动从未来营销消息的受众中删除。

>[!NOTE]
>
>对于事务型消息，不必添加退订链接。

Adobe Journey Optimizer会自动处理传入消息中的以下关键词： **开始**, **停止**&#x200B;和 **UNSTOP**. 这些关键词会触发短信提供商的标准自动回复。
