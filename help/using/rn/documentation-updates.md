...
* 与Journey Optimizer一起提供的Adobe集成现在列在连接您的系统和环境部分中。 [了解详情](../integrations/ajo-integrations.md)

* 内容集成现在分组到内容管理部分。 [了解详情](../integrations/content-integrations.md)

* Adobe Experience Platform和Journey Optimizer的架构图已更新。 [了解详情](../start/get-started.md#architecture-architecture)

* 添加了有关个性化编辑器游乐场的视频，以帮助您了解如何使用示例数据编写和测试个性化代码。 [了解详情](../personalization/personalize.md#how-to-videosvideo-perso)

* 种子列表中的最大地址数已从50个增加到300个。 [了解详情](../configuration/seed-lists.md#create-seed-list)

* 在“创建决策策略”页面中添加了一个新步骤，其中详细说明了在基于代码的体验编辑器中使用决策策略时如何封装代码。 [了解详情](../experience-decisioning/create-decision.md#use-decision-policy)

* 向基于代码的体验文档中添加了注释，以指定当您在同一个表面上运行多个基于代码的体验操作时，如果最终用户符合多个操作的条件，则营销活动或历程的优先级分数将决定向最终用户交付的内容。 [了解详情](../code-based/code-based-surface.md#surface-definition)

* 有关对历程中的入站操作进行故障排除的新页面提供了分步指南，以便在联系支持人员之前独立识别并解决问题。 [了解详情](../building-journeys/troubleshooting-inbound.md)
...
* 目前，您无法通过 [!UICONTROL Journey Optimizer] 用户界面取消委派子域，您必须联系 Adobe 代表。现在，已为[电子邮件](../configuration/delegate-subdomain.md#undelegate-subdomain)、[短信](../sms/sms-subdomains.md#undelegate-a-subdomain-undelegate-subdomain)、[Web体验](../web/web-delegated-subdomains.md#undelegate-a-subdomain-undelegate-subdomain)和[登陆页面](../landing-pages/lp-subdomains.md#undelegate-subdomain).<!--[Read more](../configuration/delegate-subdomain.md#undelegate-subdomain)-->详细说明了取消委派子域的步骤
