...
* 在[护栏和限制](../start/guardrails.md#profile-management-inbound)页面中添加了针对具有入站渠道的假名配置文件的护栏。 此护栏指定定向未经身份验证或未知的用户档案会增加您的总可参与用户档案计数，并且可能影响您的帐单。 这就是为什么Adobe建议为自动删除用户档案设置存留时间(TTL)以管理可参与用户档案计数和相关成本的原因。
