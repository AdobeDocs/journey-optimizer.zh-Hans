...
* 为更加清晰明了，内容优化文档进行了重组。 主优化页面已拆分为四个重点子页面：一个入门页面、一个用于定位的专用页面、一个用于实验，另一个用于组合两种方法。 [了解详情](../campaigns/gs-message-optimization.md)

* 已从三个旅程警报(已发布历程、已完成历程和已触发自定义操作上限设置)中删除有限可用性说明，因为这些功能现已正式可用。 [了解详情](../reports/alerts.md)

* 对“测试、验证和批准”登陆页面进行了增强，添加了新部分，包括测试功能概述、常见问题解答、包含导航链接的决策树，以及包含文档链接的增强术语。 [了解详情](../../rp_landing_pages/test-landing-page.md)
...
* 更新了营销活动页面中的“优化”部分，以反映优化功能现在也可用于历程的事实。[了解详情](../campaigns/gs-message-optimization.md)

* 添加了教程视频链接，描述如何在营销活动中利用消息优化。[了解详情](../campaigns/gs-message-optimization.md)
