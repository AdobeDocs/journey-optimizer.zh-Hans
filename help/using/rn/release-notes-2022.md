...
<thead>
<tr>
<th><strong>数据治理和隐私</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
...
<tr>
<td>
<p>作为 Journey Optimizer 用户，您现在可以通过用户界面访问系统警报，在历程运行异常时获得通知。 您可以查看可用的警报并订阅它们。 如果读取受众活动在定义的时间范围内未处理任何轮廓，则此版本中提供的第一个警报将会警告您。 解锁此工作流后，将会有更多内容。</p>
...
* 借助Journey Optimizer，您可以识别用户在系统中对各种服务和功能（如营销策划、历程、消息、登陆页面等）执行的操作。审核日志资源现在包括对各种其他操作所做的更改，并会在活动发生时自动进行记录。 请参阅[此页面](../privacy/audit-logs.md)以了解详情。
...
<tr>
<td>
<p>Adobe Stock和Adobe Journey Optimizer Email Designer集成插件为客户提供了一种简单的方式来导航、许可和保存图像，以用于消息创作。</br> 新的<b>查找类似Stock照片</b>选项还允许您查找与图像的内容、颜色和合成匹配的照片库。 </p>
...
* **另存为模板** – 现在，您可将电子邮件内容另存为模板，并在创建其他消息时重复使用。 [了解详情](../content-management/content-templates.md#video-templates)
...
* **选择启用/选择禁用复选框的新选项** - 您现在可以在订阅登陆页面中为选择启用/选择禁用插入一个复选框。 用户需要选中复选框来表示同意（选择启用），取消选中该复选框以取消同意（选择禁用）。 [了解详情](../landing-pages/design-lp.md#design-lp)
