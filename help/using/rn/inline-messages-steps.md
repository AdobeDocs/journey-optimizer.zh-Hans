...
有关在Adobe Journey Optimizer中创作消息的新过程，请参阅 [页面](../rn/inline-messages.md). 将为您执行历程的自动转换。 话虽如此，我们需要你的帮助，只需几步。
...
1. 查看草稿版本迁移中的错误。 [了解详情](../rn/inline-messages-steps.md#migration-step-2-5)
...
开 **非生产沙箱**，停止所有实时和已关闭的历程。 这将使自动迁移过程能够从这些沙箱迁移所有历程，而无需您执行任何操作。 迁移后，您将能够复制并使用已停止的历程版本。
...
要查找这些历程，请导航到 **历程** 菜单中，并过滤“状态=实时”和“类型=读取区段”上的列表。 您还可以按时间顺序从最早到最新的“已发布”日期进行排序。
...
* 检查历程是否包含消息。
* 检查它们不是重复历程。 这些不是临时的。 你最可能想让他们活下去。 例如，此历程属于重复历程（而非临时历程）：

   ![](assets/inline-migration-steps2.png)

* 如果在这些历程中使用了等待侦听器或事件侦听器，则用户档案可能仍位于内部。 查看历程执行日期，并添加您在等待或事件侦听器中定义的任何小时/天，以推断内部没有用户档案的实际日期。 如果日期已过，您可以停止历程。 否则，此历程将在历程执行日期30天后自动移至“已完成”状态。
...
* 如果您的消息未在历程中使用且要保留，请将其另存为模板。 请注意，在弃用之前，您仍然可以访问这些组件。

## 迁移首次迭代后（7月25日）{#migration-step-2}

迁移分两个阶段进行：自动化阶段（在夜间，7月25日至7月26日）和手动阶段（从7月26日开始），需要行动项目。

对于自动化阶段，请参阅 [页面](../rn/inline-messages.md#process).
...
### 5.查看草稿版本迁移中的错误{#migration-step-2-5}

单击 **检查状态** 按钮，并检查自动迁移期间没有错误，以及没有要迁移的内容。 请注意，任何出错历程（包含消息）将在9月5日之后（在所有沙箱上）被弃用。
...
如果您尚未发布较新的历程版本(请参阅 [部分](../rn/inline-messages-steps.md#migration-step-2-3))，然后发布较新版本和 **停止之前的版本，否则您将丢失** 及其相关报告。
