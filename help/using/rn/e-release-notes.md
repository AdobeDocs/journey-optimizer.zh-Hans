...
* **按历程设置频率上限** — 您现在可以创建应用于历程的规则集，从而限制每日、每周或每月的历程数，并控制同时运行的并行历程数。

* **优先级得分** — 您现在可以为营销活动或历程分配从0到100的优先级得分。 数字越大，表示优先级越高。当两个营销活动或历程使用相同的渠道配置时，Journey Optimizer将选择具有最高优先级分数的营销活动或历程。 如果促销活动具有相同的得分，则将选择最近修改的促销活动。 优先级得分适用于营销活动中的所有入站渠道，以及历程中的应用程序内渠道。
