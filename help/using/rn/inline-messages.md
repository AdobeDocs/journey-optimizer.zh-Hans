...
* **产品更改**:在GA（7月25日），将在每个历程中创建和管理渠道内容。 的 **消息** 菜单（在左侧导航中）将不再可用([了解更多](../rn/inline-messages.md#change))。 我们将继续为您的现有历程进行迁移。

* **时间轴**:每个区域在夜间(通过多个 [迭代](../rn/inline-messages.md#iterations).

   ![](assets/inline-migration-timeline.png)

* **必需操作**:将为您执行历程的自动转换。 话虽如此，我们需要你的帮助，只需几步。 详细了解此 [页面](../rn/inline-messages-steps.md).

* **弃用**:9月6日后，所有仍在使用旧版消息的历程都将停止并删除。

## 优点和产品更改{#change}

Adobe的愿景是不断简化产品，以提供高效和优化的用户流。 这种创建消息的新方式将简化用户流程。

我们设计了这个新工作流，用于将内容直接放在使用位置。

现在，内容的创建操作将直接在历程中执行。 立即 **好处** 您将获得：
...
需要进行迁移，才能使用 **消息** 包含内联创作操作的历程。 将为您执行历程的自动转换。 话虽如此，我们需要你的帮助，只需几步。
...
这个改变将在一夜之间被运用，经过几次迭代。 了解详情 [迭代](../rn/inline-messages.md#inline-authoring).
...
对于非实时或已关闭的历程，迁移是完全自动的。 我们不希望影响实时或已关闭的历程，以避免任何生产影响。 我们将要求您发布我们为您创建的新版本。
...
消息的草稿版本将在迁移期间进行修改。 他们将不再引用消息。 的 **消息** 活动将替换为相应的渠道操作活动。 每个渠道参数和内容都将包含在内。
...
迁移期间将创建此历程的新草稿版本。 此新草稿版本将是实时版本的副本，但消息将被内联创作的渠道操作替换。 每个渠道操作活动将包括渠道参数和内容。 内容不会丢失。 报告不会丢失
...
在查看历程报表时，报告将会更丰富，以包含之前在消息报表中提供的信息级别。
...
将为您执行历程的自动转换，但需要执行一些步骤。 详细了解此 [页面](../rn/inline-messages-steps.md).
...
迁移将在夜间进行。 您将能够使用产品。 但请勿创建、编辑或删除历程。
