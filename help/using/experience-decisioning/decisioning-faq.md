...
**在此页面上：**&#x200B;查找有关决策功能的常见问题解答，包括上限规则、排名公式和AI模型行为，以便您能够自信地配置决策并排除其故障。
...
自动优化模型会根据过去14天的流量数据，将流量提供给下一个最佳可用优惠，无论个性化优化模型是否使用过去30天的流量数据。

当同时删除多个选件且剩余选件在14天或30天窗口内具有最小的流量数据时，模型可能表现出次优行为，包括基于有限展示数据的随机分布模式或偏向具有较高转化率的选件。
