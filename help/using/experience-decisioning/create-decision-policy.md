...
**在此页面上：**&#x200B;在营销活动或历程内容中添加并配置决策策略，以便您可以在支持的渠道中为每个用户档案返回最佳决策项目并进行排名。
...
在Email Designer中使用&#x200B;**[!UICONTROL 编码您自己的]**&#x200B;模式时，您还可以添加决策策略。 为此，请导航到&#x200B;**[!UICONTROL 决策策略]**&#x200B;以插入决策策略代码 — 将显示投放位置选择UI，允许您为决策策略选择投放位置。 [了解如何为自己的电子邮件内容编码](../email/code-content.md)。
...
在内容中添加新的决策策略后，将打开决策策略配置屏幕。 按照以下步骤配置决策策略：

1. 提供决策策略的名称，然后选择目录（当前仅限于默认的&#x200B;**[!UICONTROL 优惠]**&#x200B;目录）。

   ![](assets/decision-code-based-details.png)

1. **[!UICONTROL 项数]**&#x200B;字段允许您定义要随决策策略返回的决策项数。 例如，如果选择 2，则将为当前配置显示最佳的 2 个合格优惠。
