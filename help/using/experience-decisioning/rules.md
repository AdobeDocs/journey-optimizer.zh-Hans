...
1. 选择规则实体，以指定要为其创建规则的对象类型。
   ...
   * **[!UICONTROL 定位]** — 在生成[定位](#targeting-rules)规则时可使用该规则，该规则可作为营销活动中[内容优化](../campaigns/campaigns-message-optimization.md#targeting)的一部分或历程（在[优化历程活动](../building-journeys/path-targeting.md)中）使用。
