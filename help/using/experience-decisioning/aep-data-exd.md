...
**在此页面上：**&#x200B;在决策规则、排名公式和上限规则中使用Adobe Experience Platform数据集属性，以便使用频繁更改的动态外部数据（如库存或定价）扩充决策逻辑。
...
>title="添加数据集"
>abstract="使用 [!DNL Adobe Experience Platform] 数据集属性，根据动态外部属性定义上限标准。 单击&#x200B;**[!UICONTROL 添加数据集]**&#x200B;以选择包含所需属性的数据集。 如果数据集未显示在列表中，请确保您已启用了该数据集的查找功能，并已创建了数据集查找映射。"
...
要使用 Adobe Experience Platform 中的属性做出决策，您需要创建一个映射来定义 Adobe Experience Platform 数据集如何与 [!DNL Journey Optimizer] 中的数据连接。 为此，请执行以下步骤：
