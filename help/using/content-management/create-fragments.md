...
**在此页面上：**&#x200B;了解如何创建、设计、锁定、预览和发布可视化及表达式片段，以便在Adobe Journey Optimizer营销活动和历程中重用内容。
...
➡️ [管理您的Adobe Experience Manager内容片段](../integrations/aem-fragments-manage.md)以查看在&#x200B;**[!UICONTROL AEM片段]**&#x200B;选项卡上同步的片段、引用、标记和手动同步。
...
* 对于可视化片段，请根据需要编辑您的内容，就像处理历程或营销活动中的任何电子邮件一样。 [了解详情](../email/get-started-email-design.md)
  ...
  >[!CAUTION]
  >
  >片段在使用主题和手动样式设置模式之间不交叉兼容。 在电子邮件内容中使用片段时，请确保应用为此片段定义的主题。 [了解详情](../email/apply-email-themes.md#leverage-themes-fragment)
...
* 对于表达式片段，利用[!DNL Journey Optimizer]个性化编辑器及其所有个性化和创作功能来构建片段内容。 [了解详情](../personalization/personalization-build-expressions.md)
  ...
  >[!NOTE]
  >
  >JSON类型表达式片段在保存时进行语法验证，任何错误都显示为警告警报。
...
### 锁定可视片段 {#lock-visual-fragment}

如果您创建或编辑的是可视化片段，则可以将其锁定以防止编辑者在电子邮件中使用它时对其进行修改或分离。

选择此选项可确保片段在使用的任何位置都保持同步，从而帮助强制实施品牌、保持一致性和遵守法律要求。

要锁定可视片段，请执行以下步骤。

1. 在片段内容版本屏幕中，转到&#x200B;**[!UICONTROL 设置]**&#x200B;选项卡。

1. 默认情况下，片段处于解锁状态。 选择&#x200B;**[!UICONTROL 防止继承中断]**&#x200B;以锁定片段。

1. 单击&#x200B;**[!UICONTROL 确认]**。

   >[!NOTE]
   >
   >可以随时更新此设置。 但是，更改仅适用于未来使用情况。 不修改使用此片段的现有电子邮件。

![](assets/fragment-lock.png){width="70%" align="center"}

现在，在电子邮件中使用此片段时，该片段将被锁定并且无法与原始片段分离。 [了解详情](../email/use-visual-fragments.md#locked-fragments)

对原始锁定片段的新更新将自动传播到使用该片段的所有电子邮件。
