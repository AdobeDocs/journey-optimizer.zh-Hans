...
title: 将图像转换为电子邮件内容模板
description: 了解如何使用AI支持的图像到HTML转换器将图像设计转换为可编辑的电子邮件内容模板
...
* **比手动编码快** — 转换器在几分钟内将图像转换为可编辑内容，因此您可以跳过耗时的手动模拟HTML工作流。
* **无需技术技能** — 营销人员无需设计或开发支持即可生成和调整模板。
...
* **平台迁移**：是否从其他电子邮件营销平台迁移？ 无需从头重新构建，即可将您现有的电子邮件设计转换为可用于[!DNL Journey Optimizer]的HTML模板。
...
要将图像设计转换为完全可自定义的电子邮件内容模板，请执行以下步骤。
...
1. 在&#x200B;**[!UICONTROL 将图像转换为模板]**&#x200B;部分中，执行以下步骤：

   * （可选）如果贵组织在Journey Optimizer中定义了品牌主题，则可以选择一个主题作为输入，以便根据品牌主题参数设置生成的HTML的样式。 [了解有关主题的更多信息](../email/apply-email-themes.md)
...
现在，内容模板库中提供了您的模板，并可在历程或营销活动中创建电子邮件时使用。 [了解如何使用内容模板](../email/use-email-templates.md)

## 最佳实践 {#best-practices}

要在将图像转换为电子邮件内容模板时获得最佳结果，请遵循以下建议。

+++开始之前

* **保存现有内容**：转换图像将替换电子邮件模板中的所有现有内容。 在使用此功能之前，请始终保存您当前所做的工作。
* **规划您的工作流**：在电子邮件创建过程的开始时使用此功能，或确保准备好替换所有当前内容。
...
+++将图像转换为内容模板时，现有电子邮件内容会发生什么情况？

上传要进行转换的图像时，电子邮件中的所有现有内容都将被删除并替换为新生成的模板。 使用此功能之前，请确保保存所有重要内容。 此功能最好在电子邮件创建过程开始时实现。

+++

+++支持哪些文件格式？

转换器支持JPEG (.jpg、.jpeg)和PNG (.png)图像格式。
...
当前专门为电子邮件内容模板设计的图像到HTML转换器。 对于其他内容类型，请使用电子邮件Designer中提供的标准设计和导入选项。
