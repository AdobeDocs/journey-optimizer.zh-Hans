...
>id="ajo_ab_test_mab"
>title="试验类型"
>abstract="试验类型决定在测试期间流量如何在不同处理版本之间分配。 请选择最符合您目标的方法：</br><b>A/B 试验</b>：按照您的定义在不同处理版本之间分配流量，并持续衡量效果，直至结果达到统计显著性。 适用于在受控对比环境中确定哪个版本表现更优。</br><b>多臂老虎机试验</b>：随着数据的收集，将流量动态分配给表现更佳的版本，在速度与优化之间取得平衡。 如果您希望最大限度提高试验期间的转化率，就很有帮助。</br><b>自带多臂老虎机</b>：使用您自己的算法来决定流量分配，如果您有一个自定义的模型或策略，这将为您提供灵活性。"
