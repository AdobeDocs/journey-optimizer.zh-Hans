...
>title="营销活动类型"
>abstract="选择营销活动的类型。 可用渠道根据所选类型而不同。 <br>**计划的营销活动**（操作营销活动）——非常适合简单的、一次性的批量通信，您可以计划在特定时间运行。<br>**API 触发的营销活动**——API 调用后激活，这样就可以直接从外部系统自动发送基于事件的消息。<br>**编排的营销活动** – 提供一个可视化的拖放式画布，用于设计和自动化复杂、多步骤的营销工作流，包括从受众细分到跨渠道的个性化消息投放。"
...
>title="营销活动"
>abstract="提供单次或重复的出站事务性操作。 支持的渠道：电子邮件、短信、推送通知。"
...
* **权限** – 您需要根据营销活动类型获得相应的权限。 若您无法访问营销活动功能，请联系您的管理员。 [了解内置角色→](../administration/ootb-product-profiles.md)
  ...
  | 营销活动类型 | 权限 |
  |-------------|---------------|
  | **操作营销活动**&#x200B;和 **API 触发的营销活动** | 营销活动管理员<br>营销活动审批者<br>营销活动经理<br>营销活动查看者 |
...
* **目标是什么？** （例如，提高转化率、增加参与度、通知客户）
* **受众是谁？** （例如，从Adobe Experience Platform中构建或选择）
* **适合哪个营销活动类型？** （请参阅上述[营销活动类型](#campaign-types)）
...
3. **选择您的受众**
4. 如果使用冲突管理，请&#x200B;**设置优先级**
5. **配置计划**（对于操作营销活动）或API详细信息（对于API触发）。 对于行动营销活动，您还可以[使用批次](send-using-waves.md)发送，以随着时间推移批量投放消息。
...
[配置免打扰时间](../conflict-prioritization/quiet-hours.md)
:::
