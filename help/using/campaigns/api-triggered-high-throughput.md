...
**在此页面上：**&#x200B;为API触发的营销活动激活高吞吐量模式，以便您能够以每秒最多5000个事务的速度发送非常大规模的实时事务性消息（电子邮件和推送），而无需依赖用户档案。
...
* **用于反馈和跟踪的数据集** — 高吞吐量营销活动的反馈和跟踪数据存储在未为配置文件启用的专用数据集中。 因此，即使存在匹配的用户档案，这些事件也不会与用户档案拼合。
  ...
  * **AJO消息反馈事件数据集 — 非配置文件** — 用于电子邮件和推送反馈。
  * **AJO电子邮件跟踪体验事件数据集 — 非配置文件** — 仅用于电子邮件跟踪。 推送的专用非用户档案跟踪数据集尚不可用。
...
* **推送通知**：
  ...
  * **Web推送** — 高吞吐量Web推送营销活动在共享基础架构上运行，不会从为高吞吐量移动推送保留的专用容量中获益。
  * **有效负载大小** — 最大有效负载大小为5 KB。
  * **阻止列表行为** — 当消息点击时，将保留投放反馈，但不生成配置文件更新反馈。 这意味着阻止列表响应不会在Adobe Experience Platform中触发配置文件更新或令牌失效。
