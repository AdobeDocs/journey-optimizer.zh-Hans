...
+++如果多个策略可以匹配，[!DNL Journey Optimizer]如何确定适用的审批策略？

当多个活动审批策略可以应用于同一历程或营销活动时，最近&#x200B;**激活的策略**&#x200B;优先。 该策略中定义的批准者用户组是通知的组，负责控制请求。
...
否。 请求者无法手动选择哪个用户组接收或路由审批请求。 在根据[策略优先权](approval-policies.md#multiple-policies)应用的审批策略中指定的用户组会自动收到通知。
