...
1. 单击属性旁边的 ![显示“更多操作”按钮的图像](assets/do-not-localize/rule-builder-icon-more.svg) 按钮，访问此类其他选项：
   ...
   您还可以使用显示表达式编辑器图标![图标的](assets/do-not-localize/activity-copy.svg)图像在剪贴板上复制值，例如将其粘贴到表达式编辑器中。
...
* **计算**：更新并显示规则所针对的记录数。
* **选择或保存筛选器**：选择要在画布中使用的现有预定义筛选器，或将查询另存为预定义筛选器以供将来重用。 如果过滤器包含参数，请在应用之前提供值。 有关详细信息，请参阅[使用预定义过滤器](predefined-filters.md)。
