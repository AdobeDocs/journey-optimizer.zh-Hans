...
1. 单击&#x200B;**添加条件**&#x200B;按钮，创建查询的第一个条件。
   ...
   您也可以使用预定义过滤器来启动查询。 为此，请单击&#x200B;**[!UICONTROL 选择或保存筛选器]**&#x200B;按钮，然后选择&#x200B;**[!UICONTROL 选择预定义筛选器]**。 有关详细信息，请参阅[使用预定义过滤器](predefined-filters.md)。
...
1. 单击属性旁边的 ![显示“更多操作”按钮的图像](assets/do-not-localize/rule-builder-icon-more.svg) 按钮，访问此类其他选项：
   ...
   通过值的分布可以标识在数据库中如何使用值。 此变量有助于了解可用值、其计数和百分比。 它还有助于避免在构建查询或创建表达式时出现大小写或拼写不一致等问题。
...
* **计算**：更新并显示规则所针对的记录数。
* **选择或保存过滤器**：选择要在画布中使用的现有预定义过滤器，或将查询另存为预定义过滤器以供将来重用。 如果过滤器包含参数，请在应用之前提供值。 有关详细信息，请参阅[使用预定义过滤器](predefined-filters.md)。
