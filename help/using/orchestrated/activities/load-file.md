...
>title="加载文件活动"
>abstract="**加载文件**&#x200B;活动是&#x200B;**数据管理**&#x200B;活动。 使用它在“编排的营销活动”画布上使用存储在外部文件中的用户档案和数据并定义营销活动受众。 文件数据在执行时消耗，并且不作为Adobe Experience Platform数据集保留。 使用标识列和定向维度将行协调到现有收件人。 请联系您的Adobe代表以请求获取访问权限。"
...
>
>此活动当前不可用于&#x200B;**Healthcare Shield**&#x200B;或&#x200B;**Privacy and Security Shield**。
...
在配置&#x200B;**[!UICONTROL 加载文件]**&#x200B;活动之前：

1. 创建协调所需的&#x200B;**[!UICONTROL 定向维度]**（例如，收件人）。 [了解如何创建定向维度](../target-dimension.md)

1. 确保文件中的标识值与该维度的现有记录相匹配。 上传文件中的行将协调到现有收件人，活动不会从文件创建新配置文件。

## 配置加载文件活动 {#load-file-configuration}

分两部分配置活动：使用样例文件定义预期的文件结构，然后指定在营销策划运行时加载的文件以及如何将行协调到定向维度。

按照以下步骤配置&#x200B;**[!UICONTROL 加载文件]**&#x200B;活动：
...
1. 在&#x200B;**[!UICONTROL 列]**&#x200B;部分中，展开每列并配置其属性。
   ...
   以下属性适用于每一列。 选择&#x200B;**[!UICONTROL 数据类型]**&#x200B;后，将显示该类型的其他选项。 展开以下部分，了解每种数据类型的完整列表。
   ...
   * **[!UICONTROL 分隔符]** — 日期和时间组件之间的字符，如列的UI中所示。
...
1. 在&#x200B;**[!UICONTROL 目标文件]**&#x200B;部分中，选择提供文件的方式，例如&#x200B;**[!UICONTROL 从本地计算机上传文件]**，以便在此版本中手动上传。

1. 选择要上载的CSV或TXT文件。

   >[!CAUTION]
   >
   > 确保目标文件遵循与示例文件相同的格式、列结构和列数。 不匹配可能会导致执行期间出错。

1. 选择文件中的标识列 — 用于将每行与现有收件人匹配的字段（例如，电子邮件地址或客户ID）。

1. 选择要协调的&#x200B;**[!UICONTROL 定向维度]**。

1. 配置完成后，如果UI提供映射行示例，则预览该示例，然后确认。
...
无法与现有收件人协调的行将从受众中排除。 排除的行会记录在营销活动执行日志中；营销活动不仅仅因为某些行不匹配而失败。

## 在投放中使用文件受众 {#downstream}

在&#x200B;**[!UICONTROL 加载文件]**&#x200B;解析受众后，您可以使用标准编排的营销活动：

* **[渠道活动](channels.md)** — 电子邮件、短信、推送通知或直邮。

* **[扩充](enrichment.md)**&#x200B;或&#x200B;**[协调](reconciliation.md)** — 如果需要，进一步细化或链接工作表数据。

[了解如何编排营销活动](../orchestrate-activities.md)
...
* 排除被拒绝或未协调的行；计数和原因显示在&#x200B;**执行日志**&#x200B;中（例如，加载的总行数、接受的行数、被拒绝的行数）。

对于标准编排的营销活动基础结构下的&#x200B;**100,000行** CSV，受众解析被设计为在大约&#x200B;**60秒**&#x200B;内完成。

## 相关内容 {#related}

* [创建定位维度](../target-dimension.md)
* [生成受众活动](build-audience.md)
* [读取受众活动](read-audience.md)
* [协调活动](reconciliation.md)
* [护栏和限制](../guardrails.md)
