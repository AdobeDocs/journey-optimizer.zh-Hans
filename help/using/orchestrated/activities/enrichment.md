...
将扩充数据添加到编排的营销活动后，即可将其用在在&#x200B;**扩充**&#x200B;活动之后添加的活动中，根据客户的行为、偏好和需求将客户划分到不同的组中，或创建更可能引起目标受众共鸣的个性化营销消息和营销活动。

例如，您可以将与客户购买相关的活动工作表信息添加到编排的营销活动工作表中，并使用此数据来个性化包含客户最新购买或购买额的电子邮件。
...
1. 添加&#x200B;**扩充**&#x200B;活动。
1. 如果您在编排的活动中配置了多个过渡，则可以使用&#x200B;**[!UICONTROL 主集]**&#x200B;字段定义应将哪个过渡用作主集以扩充数据。
...
1. 确定要创建协调链接的数据。
   ...
   * 若要使用来自输入过渡的数据创建协调链接，请选择&#x200B;**临时架构**，然后选择存储目标数据的协调营销活动过渡。
...
以下示例显示了一个经过编排的营销活动，该营销活动配置为在Journey Optimizer用户档案表和生成&#x200B;**加载文件**&#x200B;活动的临时表之间创建链接。 在此示例中，**扩充**&#x200B;活动使用电子邮件地址作为协调条件来协调两个表。
...
以下示例显示了一个精心设计的营销活动，该活动配置为在两个过渡之间创建链接。 第一个过渡使用&#x200B;**查询**&#x200B;活动定位配置文件数据，而第二个过渡包括存储到通过加载文件活动加载的文件中的购买数据。
...
* 添加了第二个&#x200B;**扩充**&#x200B;活动，以便使用来自&#x200B;**加载文件**&#x200B;活动的购买数据扩充编排的营销活动表中的数据。 这样，我们便可以在进一步的活动中使用这些数据，例如，将发送给客户的消息与他们的购买信息个性化。
