...
您可以通过检查 **[!UICONTROL Email opens]** 和/或 **[!UICONTROL Click on email]** 选项。
...
1. 选择要创建的链接类型：
   ...
   * **[!UICONTROL Landing page]**:插入指向登陆页面的链接。 在 [此部分](../landing-pages/get-started-lp.md)

   * **[!UICONTROL One click Opt-out]**:插入链接，使用户能够快速退订您的通信，而无需确认选择退订。 在 [此部分](../privacy/opt-out.md#one-click-opt-out).

   * **[!UICONTROL External Opt-in/Subscription]**:插入一个链接以接受从您的品牌接收通信。

   * **[!UICONTROL External Opt-out/Unsubscription]**:插入链接以取消订阅从您的品牌接收通信。 了解有关选择退出管理的更多信息，请参阅 [此部分](../privacy/opt-out.md#opt-out-management).
...
>[!NOTE]
>
>营销类型的电子邮件必须包含 [选择退出链接](../privacy/opt-out.md#opt-out-management)，而事务型消息不需要此参数。 消息类别(**[!UICONTROL Marketing]** 或 **[!UICONTROL Transactional]**) [通道表面](../configuration/channel-surfaces.md#email-type) （即消息预设）级别和创建消息时。
