...
对于短信，使用个性化编辑器中的[Url](../personalization/functions/helpers.md#url)帮助程序功能创作深层链接。 在[本节](../mobile/design-mobile.md#sms-content)中了解如何添加指向短信内容的链接。
...
>[!IMPORTANT]
>
>通过Adobe基础架构的深层链接适用于为您的消息启用链接跟踪的情况 — 在[电子邮件跟踪设置](message-tracking.md#enable-tracking)中或短信营销活动的&#x200B;**[!UICONTROL 操作跟踪]**&#x200B;部分中。 跟踪的深层链接点击使用`/ee/v1/mclick/*`下的URL，Adobe会托管这些URL并对其进行解析。
>
>对于&#x200B;**非跟踪**&#x200B;链接，不会通过Adobe系统重写URL。 您必须在自己的域和托管上配置通用链接或应用程序链接，以便这些链接能够按预期打开您的应用程序。

## 移动应用程序实施 {#mobile-implementation}

本节介绍如何使用[!DNL Adobe Journey Optimizer]实施移动设备深层链接，以便在典型的&#x200B;**HTTPS**&#x200B;设置中（通用链接和应用程序链接），单个URL可以：
...
是的。 如果您想要深层链接每个委派的子域，请为应支持该功能的每个子域请求AASA和`assetlinks.json`配置。
