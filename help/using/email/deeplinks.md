...
是的。 如果您希望对每个委派的子域进行深层链接，请为应支持该功能的每个子域请求AASA和`assetlinks.json`配置。
...
是的。 当您的应用程序对`mclick` URL执行GET时，在[!DNL Journey Optimizer]中配置的UTM参数将包含在`Location`标头中返回的最终URL中，以便您将其用于应用程序内分析。
