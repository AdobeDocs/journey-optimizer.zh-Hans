---
solution: Journey Optimizer
product: Journey Optimizer
title: 快速入门
description: Adobe Journey Optimizer入门
redpen-status: CREATED_||_2025-08-11_20-05-24
exl-id: fee218bf-8c60-442d-ac41-a3856fc71b70
source-git-commit: a83be759a20b8b1592164119762a6dbaa5ac469a
workflow-type: tm+mt
source-wordcount: '588'
ht-degree: 9%

---

# Adobe Journey Optimizer 快速入门{#get-started}

Adobe Journey Optimizer 使您能够跨每个渠道提供个性化的实时客户体验。在正确的时间向客户传达正确的消息，与他们接触。

无论您是Journey Optimizer的新手还是希望扩展您的技能的经验丰富的用户，本节都提供了快速有效地开始使用所需的一切。

## 开始使用Essentials {#essentials}

了解Adobe Journey Optimizer的基础知识以及它如何实现大规模数据驱动个性化。

:::: landing-cards-container
:::
![icon](https://cdn.experienceleague.adobe.com/icons/book.svg?lang=zh-Hans)

什么是Journey Optimizer？

了解核心功能、用例以及Journey Optimizer如何提供个性化的客户体验。

[了解基础知识](../using/start/get-started.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/code-branch.svg?lang=zh-Hans)

了解其工作方式

了解Journey Optimizer如何与Adobe Experience Platform集成及其关键功能领域如何协同工作。

[探索架构](../using/start/understanding-ajo.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/list.svg?lang=zh-Hans)

关键术语

帮助您自信地导航Journey Optimizer的基本术语和概念。

[查看术语](../using/start/terminology.md)
:::

::::

## 选择您的学习路径 {#by-role}

开始使用根据您的责任和目标定制的特定于角色的指南。

:::: landing-cards-container
:::
![icon](https://cdn.experienceleague.adobe.com/icons/user.svg?lang=zh-Hans)

面向营销人员

设计客户历程、创建营销活动并个性化内容以吸引受众。

[营销人员入门](../using/start/path/marketer.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/database.svg?lang=zh-Hans)

面向数据工程师

配置数据结构，管理架构和数据集，并确保数据质量。

[起步数据工程师](../using/start/path/data-engineer.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/gear.svg?lang=zh-Hans)

适用于管理员

为您的组织设置环境、管理权限并配置渠道。

[以管理员身份启动](../using/start/path/administrator.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/code.svg?lang=zh-Hans)

面向开发人员

集成移动应用程序、实施API，以及构建自定义操作和集成。

[开发人员入门](../using/start/path/developer.md)
:::

::::

[不确定哪个职位适合你？ 参加我们的快速评估→](../using/start/quick-start.md)

## 高效工作 {#work-efficiently}

掌握界面并利用智能功能加快您的工作。

:::: landing-cards-container
:::
![icon](https://cdn.experienceleague.adobe.com/icons/layout.svg)

导航界面

了解如何使用Journey Optimizer界面、访问关键功能并快速找到所需的内容。

[浏览UI](../using/start/user-interface.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/search.svg?lang=zh-Hans)

搜索和组织

使用搜索、筛选器和标记高效地管理历程、营销活动和内容。

[主搜索](../using/start/search-filter-categorize.md)
:::

:::
![icon](https://cdn.experienceleague.adobe.com/icons/ai.svg)

AI和智能功能

了解AI如何帮助您创建内容、优化发送时间和做出更明智的决策。

[探索AI功能](../using/start/ai-features.md)
:::

::::

## 其他资源 {#additional-resources}

### 参考和最佳实践

* **[护栏和限制](../using/start/guardrails.md)** — 了解有效使用Journey Optimizer功能的限制和建议做法
* **[辅助功能](../using/start/accessibility.md)** — 了解辅助功能和WCAG合规性，确保所有用户的包容性
* **[疑难解答指南](../using/start/troubleshooting.md)** — 查找常见问题的解答和常见问题的解决方案

### 通过实践学习

* **[教程视频](https://experienceleague.adobe.com/docs/journey-optimizer-learn/tutorials/overview.html?lang=zh-Hans){target="_blank"}** — 介绍核心功能的分步视频教程
* **[用例行动手册](../using/start/ai-features.md#playbooks)** — 用于常见方案（例如放弃的购物车恢复和欢迎营销活动）的预建工作流
* **[文档更新](../using/rn/documentation-updates.md)** — 随时了解文档的最新更改和添加内容

### 获取支持

* **[监视和故障排除](/help/rp_landing_pages/troubleshoot-journey-landing-page.md)** — 用于跟踪性能、识别问题和快速解决问题的工具
* **[新增功能](../using/rn/release-notes.md)** — 及时了解最新功能、改进和修复
* **[社区论坛](https://experienceleaguecommunities.adobe.com/t5/journey-optimizer/ct-p/journey-optimizer){target="_blank"}** — 与其他用户联系、提出问题并共享最佳实践

## 快速链接 {#quick-links}

使用这些重要指南立即开始创建：

| 任务 | 指南 |
|------|-------|
| 创建您的第一个历程 | [历程创建指南](../using/building-journeys/journey-gs.md) |
| 发送您的第一个营销活动 | [营销活动创建指南](../using/campaigns/get-started-with-campaigns.md) |
| 设计电子邮件 | [电子邮件设计指南](../using/email/get-started-email-design.md) |
| 构建受众 | [受众创建指南](../using/audience/creating-a-segment-definition.md) |
| 设置渠道 | [渠道配置指南](../using/configuration/get-started-configuration.md) |
| 创建个性化产品建议 | [决策管理指南](../using/offers/get-started/starting-offer-decisioning.md) |

&#x200B;---

## 准备好开始了吗？

选择适合您的路径：

* **是Journey Optimizer的新用户？**&#x200B;以[开始。什么是Journey Optimizer？](../using/start/get-started.md)以了解基础知识
* **知道您的角色吗？**&#x200B;跳转到与您职责匹配的[特定于角色的指南](../using/start/quick-start.md)
* **想探索吗？**&#x200B;浏览[AI功能](../using/start/ai-features.md)或尝试[用例行动手册](../using/start/ai-features.md#playbooks)
* **需要帮助？**&#x200B;查看[疑难解答指南](../using/start/troubleshooting.md)或访问[社区论坛](https://experienceleaguecommunities.adobe.com/t5/journey-optimizer/ct-p/journey-optimizer){target="_blank"}

>[!NOTE]
>
>环境中可用的组件和功能取决于您的[权限](../using/administration/permissions.md)和[许可包](https://helpx.adobe.com/cn/legal/product-descriptions/adobe-journey-optimizer.html){target="_blank"}。 有关特定设置的问题，请联系您的Adobe客户成功经理。
