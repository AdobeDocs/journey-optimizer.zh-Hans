...
使用特定配置文件测试历程，以便在发布之前验证历程，确保事件、条件和操作按预期运行。 适用于使用命名空间的草稿历程。 不确定哪种方法合适？请参阅[选择验证方法](../using/building-journeys/choose-validation-method.md)。
...
* 使用[历程模拟](../using/building-journeys/simulate-journey-gs.md)，借助临时模拟用户快速测试分支，无需用到测试轮廓
* 使用[测试模式](../using/building-journeys/testing-the-journey.md)，借助真实的 AEP 测试轮廓手动走查分支和消息逻辑（仅适用于草稿历程，需要命名空间）
* 使用[试运行](../using/building-journeys/journey-dry-run.md)，在发布之前大规模检查实际受众触达范围和分支逻辑，而不发送消息
...
不确定该使用三种历程验证方法中的哪一种？有关完整对比，请参阅[选择验证方法](../using/building-journeys/choose-validation-method.md)。

**对于复杂历程：**

* 在迭代分支逻辑时使用历程模拟，然后结合测试模式和试运行，在发布前全面验证分支逻辑与执行路径
* 系统性地测试不同的进入条件与轮廓属性
...
1. **使用样本数据测试内容变体** – 通过使用 CSV 或 JSON 文件中的[样本输入数据](../using/test-approve/simulate-sample-input.md)、手动输入或 AI 自动生成，无需创建大量测试轮廓即可测试多达 30 种个性化场景。

1. **使用利益相关者监控用种子列表** – 配置[种子列表](../using/configuration/seed-lists.md)，在执行时自动包含将接收所有发送内容副本的内部利益相关者，用于质量监控与合规性验证（仅限电子邮件渠道）。

1. **在创建测试轮廓之前，使用历程模拟进行迭代** – 使用[历程模拟](../using/building-journeys/simulate-journey-gs.md)，可以借助临时模拟用户快速验证新的分支或决策策略路径，并将[测试模式](../using/building-journeys/testing-the-journey.md)留到需要借助真实 AEP 测试轮廓手动走查历程时再使用。
...
| **3. 历程逻辑** （仅限历程） | 进入条件、流程、分支 | 使用[历程模拟](../using/building-journeys/simulate-journey-gs.md)进行快速迭代，使用[测试模式](../using/building-journeys/testing-the-journey.md)借助真实测试轮廓走查逻辑，使用[试运行](../using/building-journeys/journey-dry-run.md)进行大规模最终检查 – 请参阅[如何选择](../using/building-journeys/choose-validation-method.md) |
...
**[历程模拟](../using/building-journeys/simulate-journey-gs.md)** – 一种验证方法，使用手动创建或自动生成的临时模拟用户来运行历程，而非使用真实 AEP 测试轮廓。用例：历程设计期间的快速迭代。[历程模拟快速入门](../using/building-journeys/simulate-journey-gs.md)

**[测试模式](../using/building-journeys/testing-the-journey.md)** – 通过历程路径发送真实 AEP 测试轮廓的历程验证功能。限制条件：仅适用于草稿历程、需命名空间、仅限测试轮廓。 [请参阅测试模式文档](../using/building-journeys/testing-the-journey.md)
...
**[示例输入数据](../using/test-approve/simulate-sample-input.md)** - CSV 或 JSON 文件、手动输入或 AI 自动生成可用于测试个性化效果。 最多支持 30 种变体。 [如何模拟内容变体](../using/test-approve/simulate-sample-input.md)
...
* [模拟内容变体](../using/test-approve/simulate-sample-input.md) – 使用 CSV 或 JSON 文件、手动输入或 AI 自动生成测试最多 30 种个性化场景。
...
**主要解决方案：**&#x200B;使用 CSV/JSON 文件的[样本输入数据](../using/test-approve/simulate-sample-input.md)（最多支持 30 种变体）
...
**历程模拟：**使用临时模拟用户运行历程，无需用到真实的 AEP 测试轮廓。向模拟用户的已配置地址发送真实消息。
**测试模式：**让真实的 AEP 测试轮廓走完整个历程，触发实际操作，并生成测试消息。需要草稿历程 + 命名空间。
**试运行：**基于真实生产受众数据跟踪执行路径，且不发送任何内容。不发送消息，不执行操作。
**搭配使用：**&#x200B;历程模拟可用于进行设计迭代，测试模式可用于借助真实测试轮廓对消息/逻辑进行手动走查，试运行可用于进行大规模最终逻辑检查。有关完整对比，请参阅[选择验证方法](../using/building-journeys/choose-validation-method.md)。
